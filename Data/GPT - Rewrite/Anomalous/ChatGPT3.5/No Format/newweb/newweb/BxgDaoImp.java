...
public class BxgDaoImp implements BxgDao {
    ...
    public List<Bxg> selectAllBxgs() {
        Query query = sessionFactory.getCurrentSession()
                .createSQLQuery("select * from t_bxg where valid=true order by buycount desc,CONVERT(name USING GBK),norms desc")
                .addEntity(Bxg.class);
        return query.list();
    }
    ...
    public List<Bxg> selectBxgByLikeNorms(String condition) {
        Query query = sessionFactory.getCurrentSession()
                .createSQLQuery("select * from t_bxg where valid=true and norms like ? order by buycount desc,CONVERT(name USING GBK)")
                .addEntity(Bxg.class);
    ...
    public List selectAllBxgs(int start, int limit) {
        ...
        Query query = sessionFactory.getCurrentSession()
                .createSQLQuery("select * from t_bxg order by valid DESC,buycount desc,CONVERT(name USING GBK)")
                .addEntity(Bxg.class);
    ...
    public Bxg selectBxgByCondition(String name, double thickness, String norms, int brandID) {
        Query query = sessionFactory.getCurrentSession()
                .createSQLQuery("select * from t_bxg where name=? and thickness=? and norms=? and materialbrandID=?")
                .addEntity(Bxg.class);

...
public class BillDaoImp implements BillDao {
    ...
    public List selectBillByStatusACreateDateACustomerLikeCondition(String status, String startDate, String endDate,
        ...
        Query query1 = sessionFactory.getCurrentSession()
                .createSQLQuery("select * from t_bill where status != '等待结帐' and status like '送货结帐' and createDate < ? order by lastModifyDate DESC,lastModifyTime DESC")
                .addEntity(Bill.class);
        Query query2 = sessionFactory.getCurrentSession()
                .createSQLQuery("select * from t_bill where status != '等待结帐' and status like ? and createDate between ? and ? and customerID like ? order by lastModifyDate DESC,lastModifyTime DESC")
                .addEntity(Bill.class);
    ...
    @SuppressWarnings("unchecked")
    @Override
    public List<Integer> selectDistinctCustomerByDateACustomer(String startDate, String endDate, String customer) {
        Query query = sessionFactory.getCurrentSession()
                .createSQLQuery("select distinct customerID from t_bill where status != '等待结帐' and createDate between ? and ? and customerID like ?");
    ...
    @SuppressWarnings("unchecked")
    @Override
    public List<Bill> selectBillByStatusACreateDateACustomerLikeCondition(String status, String startDate,

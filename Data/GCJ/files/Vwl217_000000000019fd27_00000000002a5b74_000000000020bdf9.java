...
public class Solution {
  public static void main(String[] args) {
    ...
    for (int numCase = 1; numCase <= t; numCase++) {
        ...
        HashMap<List<Integer>, List<Integer>> startEndToIndex = new HashMap<>();
        for(int i = 0; i < n; i++){
            ...
            if(startEndToIndex.containsKey(startEnd)){
                timeList = startEndToIndex.get(startEnd);
            }else{
                timeList = new ArrayList<>();
            }
            timeList.add(i);
            startEndToIndex.put(startEnd, timeList);
        }
        ...
        for(int i = 0; i < startTimeArray.length; i++){
            ...
            startEnd.add(start);
            startEnd.add(end);
            List<Integer> indexes = startEndToIndex.get(startEnd);
            int index;
            if(indexes.size() > 1){
                index = indexes.remove(0);
                startEndToIndex.put(startEnd, indexes);
            }else{
                index = indexes.get(0);
            }
            if(cEndTime <= start){

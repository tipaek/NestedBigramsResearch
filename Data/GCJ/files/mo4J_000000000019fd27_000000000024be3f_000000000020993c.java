...
public class Solution {
    ...
    private static boolean hasUniqueRowValues(int[][] matrix, int rowNumber, int n) {
        ...
        for(int i=0; i<n; i++) {
            if(existingValues.contains(matrix[rowNumber][i])) {
                return false;
            } else {
                existingValues.add(matrix[rowNumber][i]);
            }
        }
        return true;
    }
    ...
    private static boolean hasUniqueColumnValues(int[][] matrix, int n, int columnNumber) {
        List<Integer> existingValues = new ArrayList<>();
        for(int i=0; i<n; i++) {
            if(existingValues.contains(matrix[i][columnNumber])) {
                return false;
            } else {
                existingValues.add(matrix[i][columnNumber]);
            }
        }
        return true;
    }
    ...
    private static int countRowsWithRepeatedElements(int[][] matrix, int n) {
        ...
        for (int i=0; i<n; i++) {
            countRepeatedElementsRows = hasUniqueRowValues(matrix, i, n)?
                    countRepeatedElementsRows ++:countRepeatedElementsRows;
        }
    ...
    private static int countColumnsWithRepeatedElements(int[][] matrix, int n) {
        ...
        for (int i=0; i<n; i++) {
            countRepeatedElementsColumns = hasUniqueColumnValues(matrix, n, i)?
                    countRepeatedElementsColumns ++:countRepeatedElementsColumns;
        }

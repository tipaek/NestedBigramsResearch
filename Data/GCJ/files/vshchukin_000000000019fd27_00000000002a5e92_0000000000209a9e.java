...
public class Solution {
    ...
    static class ESAbATAd {
        public void solve(int testNumber, Scanner in, PrintWriter out) {
            ...
            for (int _i = 0; _i < numTests; _i++) {
                ...
                if (halfPairs > 0) {
                    ...
                    ESAbATAd.EntangledBits first = merge(toMerge, out);
                    List<ESAbATAd.EntangledBits> toMergeLast = new ArrayList<>();
                    for (int i = halfPairs; i < numPairs; i++) {
                        ESAbATAd.EntangledBits bits = list.get(i);
                        bits.setState(5 * i, b, out, in);
                    }
                    ESAbATAd.EntangledBits last = merge(toMergeLast, out);
                    first.setState(0, b, out, in);
                    last.setState(b / 2, b, out, in);
                    finalBits = merge(Arrays.asList(first, last), out);
                }
        ...
        private ESAbATAd.EntangledBits merge(List<ESAbATAd.EntangledBits> toMerge, PrintWriter out) {
            ...
            EntangledBits entangledBits = new EntangledBits(first, last, len);
        ...
        static class EntangledBits {
            ...
            public String toString() {
                return curFirst.toString() + curLast.toString();
            }
            ...
            public void setState(int start, int b, PrintWriter out, Scanner in) {
                out.println(Math.max(flippingBit, 0)+1);
                out.flush();
                int currFlipping = in.nextInt();
                out.println(Math.max(sameBit, 0)+1);
                out.flush();

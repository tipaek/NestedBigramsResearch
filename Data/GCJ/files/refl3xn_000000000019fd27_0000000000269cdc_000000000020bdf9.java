...
public class Solution {
    ...
    private static String calculate(ArrayList<ArrayList<Integer>> arr, int i) {
        ...
        if (cisnotbusy(arr.get(i))){
            C.add(arr.get(i));
            return "C" + calculate(arr, i+1);
        } else if (jisnotbusy(arr.get(i))){
            J.add(arr.get(i));
            return "J" + calculate(arr, i+1);
        } else {
            return "IMPOSSIBLE";
        }
    }
    ...
    private static boolean jisnotbusy(ArrayList<Integer> p) {
        for(int i=0;i<J.size();i++){
            if (p.get(0) > J.get(i).get(0) && p.get(0) < J.get(i).get(1)){
                return false;
            } else if (p.get(1) > J.get(i).get(0) && p.get(1) < J.get(i).get(1)){
                return false;
            }
        }
        return true;
    ...
    private static boolean cisnotbusy(ArrayList<Integer> p) {
        for(int i=0;i<C.size();i++){
            if (p.get(0) > C.get(i).get(0) && p.get(0) < C.get(i).get(1)){
                return false;
            } else if (p.get(1) > C.get(i).get(0) && p.get(1) < C.get(i).get(1)){
                return false;
            }
        }
        return true;

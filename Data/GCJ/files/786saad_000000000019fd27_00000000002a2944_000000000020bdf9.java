...
public class Solution {
    ...
    public static void main (String [] args) {
        ...
        for (int i = 0; i < numberOfCases; i++) {
            outputs[i] = "Case #" + (i + 1) + ": " + solutionOutput(scanner);
        }
    ...
    public static String solutionOutput(Scanner scanner) {
        String [] activities = new String[Integer.parseInt(scanner.nextLine())];
        //save all inputs
        for (int i = 0; i < activities.length; i++) {
            activities[i] = scanner.nextLine();
        }
        return makeSchedule(activities);
    }
    public static String  makeSchedule(String [] activities) {
        ...
        for (int i = 0; i < activities.length ; i++) {
            ...
            for (int[] cameronJob : cameronsJobs) {
                ...
                if (startingTime >= cameronJob[0] && startingTime < cameronJob[1]
                        ...
                        || startingTime < cameronJob[0] && endingTime > cameronJob[0]) {
                    cameronBusy = true;
                    break;
                }
            }
            if (!cameronBusy) {
                ...
                for (int[] jamiejob : jamiesJobs) {
                    ...
                    if (startingTime >= jamiejob[0] && startingTime < jamiejob[1]
                            ...
                            || startingTime < jamiejob[0] && endingTime > jamiejob[0]) {
                        jamieBusy = true;
                    }
                }
                if (jamieBusy) {

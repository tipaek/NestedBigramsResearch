		...
		while (t-->0)
		{
		    ...
		    for (int i=1;i<aa.length-1;i++)
		    {
		        ...
		        else if (s1[i]!=-1)
		        {
    		        ...
    		        if(x-aa[i]==0 && x==1)
    		        {
    		            c[s1[i]]='C';c1=true;k1=e1[i];
    		            //o.println(Arrays.toString(c)+" "+x);
    		        }
    		        else if(x-aa[i]==1 && x==2)
    		        {
    		            c[s1[i]]='J';c2=true;k2=e1[i];
    		            //o.println(Arrays.toString(c)+" "+x);
    		        }
    		        else if(x-aa[i]==0 && x==2)
    		        {
    		            c[s1[i]]='C';c1=true;k1=e1[i];
    		            c[s2[i]]='J';c2=true;k2=e2[i];
    		            //o.println(Arrays.toString(c)+" "+x);
    		        }
    		        else if(x-aa[i]==1 && x==1)
    		        {
    		            c[s1[i]]='C';c1=true;k1=e1[i];
    		            //o.println(Arrays.toString(c)+" "+x);
    		        }
    		        else if(x-aa[i]==2 && x==2)
    		        {
    		            if (c1)
    		            {c[s1[i]]='J';c2=true;k2=e1[i];}
    		            else {c[s1[i]]='C';c1=true;k1=e1[i];}
    		            //o.println(Arrays.toString(c)+" "+x);
    		        }
    		        //o.println(k1+" "+k2);

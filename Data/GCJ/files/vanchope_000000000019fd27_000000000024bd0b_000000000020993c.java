 ...
 */
public class GCJ2020_Q_A implements Runnable {
    ...
    public static class Solver extends  SolverAbstract{
        ...
        public void readInput() throws IOException {
            ...
            A = new int[N][N];
            for (int i = 0; i < N; i++) {
                A[i] = scanner.loadIntArray(N);
            }
        }

        @Override
        protected void solve() throws IOException {
            int k = IntStream.range(0,N).map(i -> A[i][i]).sum();
            int r = 0;
            for (int i = 0; i < N; i++) {
                final int ii = i;
                int distinct = IntStream.range(0,N).map(j->A[ii][j]).boxed().collect(Collectors.groupingBy(e->e, Collectors.counting())).size();
                if (distinct<N){
                    r++;
                }
            }
            ...
            for (int j = 0; j < N; j++) {
                final int jj = j;
                int distinct = IntStream.range(0,N).map(i->A[i][jj]).boxed().collect(Collectors.groupingBy(e->e, Collectors.counting())).size();
                if (distinct<N){
                    c++;
                }
            }

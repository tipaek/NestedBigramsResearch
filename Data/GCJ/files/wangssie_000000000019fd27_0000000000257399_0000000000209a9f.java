...
public class Solution {
    ...
    public static String minElementArray(int[] array, int level) {
        ...
        if (array.length == 1) {
            ...
            return repeatString("(",min) + array[0]+")".repeat(min);
        }
        if (array.length == 0) {
            return "";
        }
        ...
        if (minIndex == 0) {
            ...
            return "(".repeat(min) + array[minIndex] +minElementArray(arrayRight, level-min)+")".repeat(min);
        }
        else if (minIndex == array.length-1) {
            int[] arrayLeft = Arrays.copyOfRange(array, 0, minIndex);
            return "(".repeat(min) +minElementArray(arrayLeft, level -min) + array[minIndex] +")".repeat(min);
        }
        else {
            int[] arrayLeft = Arrays.copyOfRange(array, 0, minIndex);
            int[] arrayRight = Arrays.copyOfRange(array, minIndex+1, array.length);
            return "(".repeat(min) + minElementArray(arrayLeft, level - min) +  array[minIndex] +  minElementArray(arrayRight, level - min) + ")".repeat(min);
        }
    }

...
public class Solution {
	...
	private void solve() {
		...
		for (int num = 1; num <= T; num++) {
			int N = sc.nextInt();
			PriorityQueue<Schedule> queue = new PriorityQueue<>((s1, s2) -> {
				if (s1.time < s2.time) {
					return -1;
				}
				if (s1.time > s2.time) {
			...
			for (int i = 0; i < N; i++) {
				int S = sc.nextInt();
				int E = sc.nextInt();
				queue.add(new Schedule(i, S, true));
				queue.add(new Schedule(i, E, false));
			}
	...
	class Schedule {
		...
		int time;
		boolean isStart;
		public Schedule(int id, int time, boolean isStart) {
			this.id = id;
			this.time = time;
	...
	class PrimeNumberUtils {
		...
		public PrimeNumberUtils(int limit) {
			if (limit > 10000000) {
				System.err.println("limit is too big");
			}

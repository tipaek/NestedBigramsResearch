		...
		for(i=0;i<N;i++)
		{
			for(j=0;j<N;j++)
			{
				if(mapr.containsKey(arr[i][j]))
					mapr.replace(arr[i][j],'b');
				else
					mapr.put(arr[i][j],'a');
				
				if(mapc.containsKey(arr[j][i]))
					mapc.replace(arr[j][i],'b');
				else
					mapc.put(arr[j][i],'a');
			}
			
			for(Entry<Integer,Character> valr:mapr.entrySet()) 
			if(valr.getValue().equals('b'))
			{
				r++;
				break;
			}
			
			for(Entry<Integer,Character> valc:mapc.entrySet()) 
			if(valc.getValue().equals('b'))
			{
				c++;
				break;
			}

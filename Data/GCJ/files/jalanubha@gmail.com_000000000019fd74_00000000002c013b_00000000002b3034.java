	...
	public static void main(String[] args) {
		...
		for(int x = 0; x < T; x++) {
			...
			for (int i = 0; i < N; i++) {
				...
				if(inputs[i][0].length() > maxPrefix.length() && !inputs[i][0].contentEquals("_")) {
					maxPrefix = inputs[i][0];
				}
				if(inputs[i][inputs[i].length - 1].length() > maxSuffix.length() && !inputs[i][inputs[i].length - 1].contentEquals("_")) {

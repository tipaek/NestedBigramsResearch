...
public class Solution {
	...
	public static void joestar(int kamamam) throws IOException {
		...
		for (Pair p : fakequeue) {
			...
			for (long cake : cakes) {
				if ((cake*p.y)%p.x==0) {
					if (D-counter<cake*p.y/p.x) {
						cuts+=D-counter;
						counter = D;
						
						if (counter >= D) {
							//if (p.x==4&&p.y==2)
							//System.out.println(cuts+" "+p.x+" "+p.y+"AAAAA1AAAAA");
					...
					cuts+=cake*p.y/p.x-1;
					//System.out.println(counter+" "+cuts+" "+cake);
				}
				if (counter >= D) {
					//if (p.x==4&&p.y==2)
					//System.out.println(cuts+" "+p.x+" "+p.y+"AAAAA2AAAAA");
					mincuts = Math.min(cuts, mincuts);
					break;
				}
			}
			for (long cake : cakes) {
				if (!((cake*p.y)%p.x==0)) {
					...
					if (counter >= D) {
						//if (p.x==4&&p.y==2)
						//System.out.println(cuts+" "+p.x+" "+p.y+"AAAAA3AAAAA");
						mincuts = Math.min(cuts, mincuts);
						break;
					}
					counter+=cake*p.y/p.x;
				...
				if (counter >= D) {
					//if (p.x==4&&p.y==2)
					//System.out.println(cuts+" "+p.x+" "+p.y);
					mincuts = Math.min(cuts, mincuts);
					continue;
				}
			}

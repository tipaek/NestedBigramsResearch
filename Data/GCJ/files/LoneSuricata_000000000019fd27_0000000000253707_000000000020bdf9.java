...
public class Solution {
    ...
    public static void main(String args[]) {
        ...
        for (int i = 0; i < numberOfTests; i++) {
            ...
            for (Activity a : activies) {
                ...
                if(stupidScheduler(activies.get(0), c, j)){
    ...
    public static boolean stupidScheduler(Activity a, List<Activity> c, List<Activity> j) {
        if (a == null) {
            return true;
        }
        if (c.isEmpty() || c.get(c.size() - 1).endTime <= a.startTime) {
        ...
        return stupidScheduler(a.nextActivity, c, j);
    }

    public static boolean isSchedulable(List<Activity> activies) {

        for (Activity a : activies) {
            List<Activity> overlappedActivity = new ArrayList<>();
            int overlapped = 0;
            Activity next = a.nextActivity;
            while (next != null) {
                if ((next.startTime >= a.startTime && next.startTime < a.endTime) || (next.startTime > a.startTime && next.startTime <= a.endTime)) {

                    for (Activity o : overlappedActivity) {
                        if (next.startTime>= o.startTime && next.startTime<o.endTime) {
                            return false;
                        }
                    }
                    overlappedActivity.add(a);
                }

...
public class Solution {
    ...
    private static void formLatinMatrix(int n, int sum) {
        ...
        for (int i = 1; i <= n; i++) {
            matrix[0][0] = i;
            if (findSolution(matrix, n, 0, 1,sum) == true) {
                int diagSum = 0;
                for (int j = 0; j < n; j++) {
                    diagSum += matrix[j][j];
                }
    ...
    private static boolean findSolution(int[][] matrix, int n, int row, int col, int sum) {
       // System.out.println("entering findsolution"+"row:"+row+" col:"+col);
        ...
        for (int num = 1; num <= n; num++) {
            ...
            if (col < n - 1) {
                if (findSolution(matrix, n, row, col + 1, sum))
                    return true;
            } else {
                
                if (row<n-1 && findSolution(matrix, n, row + 1, 0, sum))
                    return true;
            }}}
            matrix[row][col] = 0;

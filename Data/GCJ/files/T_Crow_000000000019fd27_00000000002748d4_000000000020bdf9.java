...
public class Solution {
	...
	static class Pareja implements Comparable<Pareja> {
		int in,index;
		int fin;
		public Pareja(int in, int fin,int index) {
			super();
			this.in = in;
			this.index=index;
			this.fin = fin;
	...
	public static void main(String[] args) {
		...
		for (int i = 1; i <= casos ; i++) {
			...
			for (int j = 0; j < cant; j++) {
				task[j]=new Pareja(fr.nextInt(), fr.nextInt(),j);
			}
			isImp=true;
			Arrays.sort(task);
			taskT=calcC()+ calcJ();
			...
			for (int j = 0; j < JC.length; j++) {
				if(JC[j].equals("F")) isImp=false;
			}
			if(taskT<cant || !isImp) System.out.println("IMPOSSIBLE");
			else {
	...
	static int calcC() {
		...
		for (int i = 0; i < task.length; i++) {
			if(!JC[task[i].index].equals("F")) continue;
			isAdd=true;
			if(aux.size()==0) {
				JC[task[i].index]="C";
				aux.add(task[i]);

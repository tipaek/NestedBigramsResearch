    		    ...
    		    {    				
    		    	InputReader in = new InputReader(System.in);
    		  	  	...
    		  	  	while(t-->0)
    		  	  	{
    		  	  		...
    		  	  		Arrays.sort(p);
    		  	  		HashMap<String,Queue> h=new HashMap<>();
    		  	  		int end1=0;
    		  	  		int end2=0;
    		  	  		for(int i=0;i<n;i++)
    		  	  		{
    		  	  			if(p[i].first>=end1)
    		  	  			{
    		  	  				if(!h.containsKey(p[i].first+" "+p[i].index))
    		  	  				{
    		  	  					Queue<Character> q=new LinkedList<Character>();
    		  	  					q.add('J');
    		  	  					h.put(p[i].first+" "+p[i].index,q);
    		  	  				}
    		  	  				else
    		  	  				{
    		  	  					Queue q=h.get(p[i].first+" "+p[i].index);
    		  	  					q.add('J');
    		  	  					h.put(p[i].first+" "+p[i].index,q);
    		  	  				}
    		  	  				end1=Integer.max(end1, (int)p[i].index);
    		  	  			}
    		  	  			else if(p[i].first>=end2)
    		  	  			{
    		  	  				if(!h.containsKey(p[i].first+" "+p[i].index))
    		  	  				{
    		  	  					Queue<Character> q=new LinkedList<Character>();
    		  	  				...
    		  	  				//h.put(p[i].first+" "+p[i].index,'C');
    		  	  				end2=Integer.max(end2, (int)p[i].index);
    		  	  			}
    		  	  			else
    		  	  			{
    		  	  				flag=false;
    		  	  		...
    		  	  		if(flag==false)
    		  	  		{
    		  	  			out.println("Case #"+(t1-t)+": "+"IMPOSSIBLE");
    		  	  		}
    		  	  		else
    		  	  		{
    		  	  			out.print("Case #"+(t1-t)+": ");
    		  	  			for(int i=0;i<n;i++)
    		  	  			{
    		  	  				out.print(h.get(p1[i].first+" "+p1[i].index).poll());
    		  	  			}
    		  	  			out.println();
    		  	  		}
    		  	  	}

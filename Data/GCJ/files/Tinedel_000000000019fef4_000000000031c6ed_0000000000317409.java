...
public class Solution {
    ...
    static class State {
        ...
        boolean tooFarAway(int timeLeft) {
            return Math.abs(x) + Math.abs(y) > 2 * timeLeft;
        }
    }
    ...
    private static String solve(int x, int y, String p) {
        ...
        int len = p.length();
        while (steps < len) {
            char nextStep = p.charAt(len);
            Set<State> nextStates = new HashSet<>();
            for(State state: toCheck) {
                nextStates.add(state.E(nextStep));
                nextStates.add(state.N(nextStep));
                nextStates.add(state.S(nextStep));
                nextStates.add(state.W(nextStep));
                nextStates.add(state.P(nextStep));
            }
            steps++;

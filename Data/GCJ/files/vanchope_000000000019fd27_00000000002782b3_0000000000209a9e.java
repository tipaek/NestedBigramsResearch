...
public class Solution implements Runnable {
    ...
    static EventType eventType = EventType.GOOGLE_CODE_JAM_INTERACTIVE; //EventType.GOOGLE_CODE_JAM;
    ...
    public static class Solver extends  SolverAbstract{
        ...
        int readBitInteractively(int pos)  throws IOException {
            String token = sendQueryInteractively("" + (pos+1));
            if (token.charAt(0)=='0' || token.charAt(0)=='1'){
                int bit = Integer.parseInt(""+token);
                return bit;
            }else{
                throw new RuntimeException("expected 0 or 1, but received: "+token);
            }
        }

        String sendQueryInteractively(String query)  throws IOException {
            outGlobal.println(query);
            outGlobal.flush();
            System.err.println("sent query: "+query);
            String token = scanner.nextToken();
            System.err.println("received: "+token);
            if (token.charAt(0) == 'N'){
                System.err.println("Received N from the judge, exiting...");
                System.exit(0);
            }
            return token;
        }
        ...
        protected void solve() throws IOException {
            ...
            int posToConsultFlip = -1;
            int posToConsultReverse = -1;
            int posA = 0;

            while(true) {
                boolean flipped = false;
                boolean reversed = false;
                //read flip + reverse, of possible ===> 2 queries
                if (posToConsultFlip < 0 && posToConsultReverse < 0 ){
                    readBitInteractively(0);
                    readBitInteractively(0);
                }else if (posToConsultFlip >=0 && posToConsultReverse >=0 ){
                    flipped = readBitInteractively(posToConsultFlip) != A1[posToConsultFlip];
                    int consultReversed = readBitInteractively(posToConsultReverse);
                    reversed = flipped && (consultReversed == A1[posToConsultReverse])
                            || !flipped && (consultReversed != A1[posToConsultReverse]);
                }else if (posToConsultFlip >=0){
                    flipped = readBitInteractively(posToConsultFlip) != A1[posToConsultFlip];
                    readBitInteractively(0);
                }else{ // posToConsultReverse >= 0
                    flipped = readBitInteractively(posToConsultReverse) != A1[posToConsultReverse];
                    readBitInteractively(0);
                }
                ...
                for(int iter=0; iter<4; iter++){
                    if (posA >= B/2){
                        ...
                        for (int i = 0; i < A1.length; i++) {
                            sb.append(flipped ? 1-A1[i] : A1[i]);
                        }
                        for (int i = B1.length-1; i >= 0; i--) {
                            sb.append(flipped ? 1-B1[i] : B1[i]);
                        }
                        String answer = sb.toString();
                        String response = sendQueryInteractively(answer);
                        System.err.println("received response "+response+" for the query: "+answer);
                        return;
                    }
                    A1[posA] = readBitInteractively(posA);
                    B1[posA] = readBitInteractively(B - posA - 1);
                    if (posToConsultFlip < 0 && A1[posA] == B1[posA]) {
                        posToConsultFlip = posA;
                    }
                    if (posToConsultReverse < 0 && A1[posA] != B1[posA]) {
                        posToConsultReverse = posA;
                    }
                    if (flipped){
                        A1[posA] = 1 - A1[posA];
                        B1[posA] = 1 - B1[posA];
    ...
    private void solveAll() throws IOException {
        ...
        if (T==0) {
            if (largeFakeTest) {
                ...
                T = scanner.nextInt();
                if (eventType==EventType.GOOGLE_CODE_JAM_INTERACTIVE){
        ...
        for (int test = 0; test < T; test++) {
            ...
            if (eventType == EventType.GOOGLE_CODE_JAM_INTERACTIVE){
                System.err.println("preparing testNo "+solver.testNo+" for solving");
            }
        ...
        if (eventType == EventType.GOOGLE_CODE_JAM_INTERACTIVE){
            ...
            outGlobal.close();
            outGlobal.flush();
            System.err.println("ALL DONE. calling JAVA EXIT 0");
            System.exit(0);
        }
    }

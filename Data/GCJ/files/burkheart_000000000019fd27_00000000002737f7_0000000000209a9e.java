...
public class Solution { 
	...
	public static void invert(char c[]) {
		for(int i = 0; i < B; i++) {
			if (c[i] == '1') c[i] = '0'; else if (c[i] == '0') c[i] = '1'; else c[i] = '.';  
		}
	}
	public static void revert(char c[]) {
		for(int i = 0; i < B/2; i++) {
			char tmp = c[i];
			c[i] = c[B-1-i];
			c[B-1-i] = tmp;
	...
	public static char query(int pos) {
		char res; 

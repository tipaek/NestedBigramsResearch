...
public class A {
	...
	private static int solve(IO io) throws IOException {
		...
		int steps = 0;
		int ans = helper(px, py, x, y, steps, ppath, 0);
	...
	public static int helper(int px, int py, int x, int y, int steps, String path, int idx) {
		if(x == px && y == py) {
			return steps;
		}
		...
		if(path.charAt(idx) == 'N') {
			py++;
		} else {
			py--;
		}

		int n = helper(px, py, x, y + 1, steps + 1, path, idx + 1);
		int e = helper(px, py, x + 1, y, steps + 1, path, idx + 1);
		int s = helper(px, py, x, y - 1, steps + 1, path, idx + 1);
		int w = helper(px, py, x - 1, y, steps + 1, path, idx + 1);
		int not = helper(px, py, x, y, steps + 1, path, idx + 1);
		if(n == -1) n = Integer.MAX_VALUE;
		if(e == -1) e = Integer.MAX_VALUE;
		if(s == -1) s = Integer.MAX_VALUE;
		if(w == -1) w = Integer.MAX_VALUE;
		if(not == -1) not = Integer.MAX_VALUE;

		return Math.min(n, Math.min(e, Math.min(s, Math.min(w, not))));
	}

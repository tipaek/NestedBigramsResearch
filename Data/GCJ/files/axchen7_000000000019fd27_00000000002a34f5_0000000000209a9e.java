...
public class Solution {
    public static void main(String[] args) {
        ...
        for (int c = 1; c <= t; c++) {
            ...
            if (b == 100) {
                boolean[] comp2 = new boolean[2];
                boolean[] rev2 = new boolean[2];
                for (int i = 0; i < 2; i++) {
                    boolean allMatch = true;
                    boolean allDiff = true;
                    int firstMatch = -1;
                    ...
                    if (allMatch || allDiff) {
                        ...
                        comp2[i] = ((scanner.nextInt() == 1) != a[i * 25]);
                        System.out.println(1);
                        scanner.nextInt();
                    } else {
                        System.out.println(i * 25 + firstMatch + 1);
                        comp2[i] = ((scanner.nextInt() == 1) != a[i * 25 + firstMatch]);
                        System.out.println(i * 25 + firstDiff + 1);
                        rev2[i] = ((scanner.nextInt() == 1) == a[i * 25 + firstDiff]) == comp2[i];
                    }
                }
                for (int i = 0; i < 2; i++) {
                    if (comp2[i]) {

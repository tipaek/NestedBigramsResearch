    ...
    static  class ESAb {
        public void solve(int B, Scanner in, PrintWriter out) {
            ...
            for(int i = 0; i < 150; i += 2) {
                if (i % 10 == 0) {
                    if (i == 0) {
                        arr[mid1_idx] = read(mid1_idx, in, out);
                        arr[mid2_idx] = read(mid2_idx, in, out);
                        if (arr[mid1_idx] == arr[mid2_idx]) sameMid = true;
                    } else {
                        char new_mid = read(mid1_idx, in, out);
                        isMiddleChanged = (new_mid != arr[mid1_idx]);
                        if (left_idx != -1) {
                            char new_left = read(left_idx, in, out);
                            isLeftRightChanged = (new_left != arr[left_idx]);
                            fix(arr, isMiddleChanged, isLeftRightChanged, sameMid);
                        } else {
                            read(mid2_idx, in, out);

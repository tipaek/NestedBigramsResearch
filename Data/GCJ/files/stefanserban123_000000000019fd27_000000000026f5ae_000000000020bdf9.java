...
public class Solution {
	...
	public static String solve (String[] input, int index) {
		...
		for (String interval : input) {
			...
			String[] se = interval.split(" ");
			boolean success = timeline.addTask(new Interval (se[0], se[1], index));
...
class Timeline {
	...
	public boolean interference (Interval task, Interval e) {
		// x1 <= y2 && y1 <= x2
		// task.start < e.end && e.start < task.end
		
		
		return (e.end - task.start) * (task.end - e.start) > 0;
	}
	
	public String result (int i) {
		kidA.addAll(kidB);
		kidA.sort((a,b)-> a.start < b.start ? -1 : a.start == b.start ? 0 : 1);

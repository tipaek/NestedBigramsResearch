...
class Planning {
	...
	public boolean addActivityPossible(Activite activity){
		for(int i = 0; i < this.liste.size(); i++){
			...
			/*if((activity.getStart() >= act.getStart() && activity.getStart() < act.getEnd())
					|| (activity.getEnd() > act.getStart() && activity.getEnd() <= act.getEnd())){
				return false;
			} else if(activity.getStart() < act.getStart() && activity.getEnd() > act.getEnd()){
				return false;
			}*/
		}

...
public class Solution {
	...
	void solve() throws IOException {
		...
		for ( int t = 0; t < T; ++t ) {
			...
			for ( TracePos pos : getTrace( N, trace ) ) {
				SudokuPuzzle suds = new SudokuPuzzle( N );

				for ( int n = 0; n < N; ++n ) {
					suds.setPuzzleCell( n, n, pos.diag[n] );
				}
	...
	private List<TracePos> getTrace( int N, int trace ) {
		List<TracePos> pos = new ArrayList<>();
		backtrackTrace( N, trace, 0, new int[ N ], pos, 0 );
		return pos;
	}

	private void backtrackTrace( int N, int trace, int idx, int[] tar, List<TracePos> pos, int sum ) {
		if ( sum == trace && idx == N ) {
			pos.add( new TracePos( tar ) );
		} else if ( idx < N ) {
			for ( int i = 1; i <= N; ++i ) {
				if ( sum + i <= trace ) {
					tar[idx] = i;
					backtrackTrace( N, trace, idx + 1, tar, pos, sum + i );
				}
			}
		}
	}

	protected static class TracePos {
		private int[] diag;

		public TracePos( int[] diag ) {
			this.diag = new int[ diag.length ];
			System.arraycopy( diag, 0, this.diag, 0, diag.length );
		}

...
public class Solution {
	...
	public static String solve(Scanner sc) throws Exception {
		...
		while(!queue.isEmpty()) {
			...
			if(curP.ith > li + 1) break;

			for(int d = 0; d < 4; d++) {
				
				if(curP.move.length() > 0 && curP.move.charAt(curP.move.length() - 1) == 'E' && cDir[d] == 'W') continue;
				if(curP.move.length() > 0 && curP.move.charAt(curP.move.length() - 1) == 'W' && cDir[d] == 'E') continue;
				if(curP.move.length() > 0 && curP.move.charAt(curP.move.length() - 1) == 'N' && cDir[d] == 'S') continue;
				if(curP.move.length() > 0 && curP.move.charAt(curP.move.length() - 1) == 'S' && cDir[d] == 'N') continue;
			...
			//		System.out.println(nX + " " + nY + ", i-th : " + (curP.ith + 1));
				...
				queue.add(new Point(nX, nY, curP.ith + 1,curP.move + cDir[d]));
			}

...
//            IO.printf("Case #%d: ", caseNum);
    ...
    private void solve(int arrSize) {
        Boolean[] ans = new Boolean[arrSize];
        if (arrSize == 10) {
            for (int i = 0; i < arrSize; i++) ans[i] = query(i);
        } else { // arrSize == 20 || arrSize == 100
            for (int i = 0; i < 5; i++) ans[i] = query(i);
            for (int i = 0; i < 5; i++) ans[arrSize - 1 - i] = query(arrSize - 1 - i);
            ...
            while (idx < arrSize / 2) {
                for (int i = 0; i < idx; i++) {
                    ...
                    if (diffIdx == -1 && ans[i] != ans[arrSize - 1 - i]) diffIdx = i;
                    if (sameIdx == -1 && ans[i] == ans[arrSize - 1 - i]) sameIdx = i;
                }
                ...
                while (idx < arrSize / 2) {
                    if (qCount % 10 == 9 || qCount % 10 == 0) break;
                    ans[idx] = query(idx);
                    ans[arrSize - 1 - idx] = query(arrSize - 1 - idx);
                    idx++;
                    qCount += 2;
                }
                if (qCount % 10 != 0) query(0); // blow a query
            }

        }
        for (int i = 0; i < arrSize; i++) IO.print(ans[i] ? 1 : 0);
        IO.println();
        IO.flush();
        IO.readLine();
        IO.flush();

...
public class Solution {
    ...
    private void indicium() {
        ...
        for (int testCase = 0;  testCase < testCases ; testCase++) {
            ...
            debug("trace=" + trace + " size=" + size + " maxTrace=" + maxTrace);
            ...
            debug("builtTrace=" + builtTrace + " ; trace=" + trace);
    ...
    private void cleanupProposals(List<List<Integer>> rowProposals) {
    ...
    private int buildTrace(List<List<Integer>> matrix, int trace) {
        int value = Math.min(matrix.size(), trace - matrix.size() + 1);
        int totalTrace = 0;
        for (int xy = 0 ; xy < matrix.size(); xy++) {
            ...
            boolean canDecrement = totalTrace + (value-1) * remainingRows >= trace;

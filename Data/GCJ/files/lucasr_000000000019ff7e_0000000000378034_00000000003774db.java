		...
		for (int caze = 1; caze <= numberOfCases; caze++) {
			...
			for (i = 0; i <= C.length; i++) {
				if (table[i][other] == need) {
					...
					if (i < C.length && other < J.length && table[i][other] == table[i+1][other+1]) {
						other++;
					} else if (i < C.length && other < J.length && table[i][other] == 1 + table[i+1][other + 1]) {
						other++;
					} else if (other < J.length && table[i][other] == 1 + table[i][other + 1]) {
						i--;
						other++;
					} else if (i < C.length && table[i][other] == 1 + table[i+1][other]) {
						// nothing
					}
				}
			}

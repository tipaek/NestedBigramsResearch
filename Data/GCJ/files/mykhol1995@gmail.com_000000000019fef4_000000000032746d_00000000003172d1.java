...
public class Solution {
    class Solver {
        public void solve(int testNumber, InputReader in, OutputWriter out) {
            ...
            int d = in.readInt();
//            long[] arr = new long[n];
            long max_angle = Long.MIN_VALUE;
            Map<Long, Integer> cakes = new HashMap<>();
            for (int i = 0; i < n; i++) {
                long cakeAngle = in.readLong();
                max_angle = Math.max(cakeAngle, max_angle);
                Integer counter = cakes.get(cakeAngle);
                if (counter == null) {
                    cakes.put(cakeAngle, 1);
                } else {
                    cakes.put(cakeAngle, counter + 1);
                }
            }
            ...
            for (Map.Entry<Long, Integer> entry : cakes.entrySet()) {
                if (entry.getValue().equals(d)) {
                    cuts = Math.min(cuts, 0);
                }
                if (entry.getValue() == d - 1 && !entry.getKey().equals(max_angle)) {
                    cuts = Math.min(cuts, 1);
                }
                long angle = entry.getKey();
                if (d == 3 && angle % 2 == 0) {
                    if (cakes.get(angle / 2) != null) {
                        cuts = Math.min(cuts, 1);
                    }
                }
            }
            out.printLine(String.format("Case #%d: %d", testNumber, cuts));
        }
    }

...
public class Solution {
    ...
    static int evaluate(int[] check, int[] order, boolean same, boolean diff) {
        ...
        if (!same) {
            if (newValues[1] == order[2*(check[1]-1)]) {
                return 4;
            } else {
                return 1;
            }
        } else if (!diff) {
            if (newValues[0] == order[2*(check[0]-1)]) {
                return 4;
            } else {
                return 1;
            }
        } else {
            if (newValues[0] == order[2*(check[0]-1)] && newValues[1] != order[2*(check[1]-1)]) {
                return 2;
            } else if (newValues[0] != order[2*(check[0]-1)] && newValues[1] != order[2*(check[1]-1)]) {
                return 1;
            } else if (newValues[0] != order[2*(check[0]-1)] && newValues[1] == order[2*(check[1]-1)]) {
                return 3;
            } else {

...
public class Solution {
    ...
    private static int canReachIter(Queue<List<Integer>> queue, List<List<Integer>> positions, Map<String, Integer> map) {
        int finalTime = -1;
        while (!queue.isEmpty()) {
            ...
            String positionStr = Stream.of(x, y, stackTime).map(String::valueOf).collect(Collectors.joining("-"));
            if (stackTime >= positions.size()) {
                return finalTime;
            }
            if ((x == positions.get(stackTime).get(0) && y == positions.get(stackTime).get(1) || map.containsKey(positionStr))) {
                finalTime = finalTime == -1 ? stackTime : Math.min(finalTime, stackTime);
                map.put(positionStr, stackTime);
            } else {

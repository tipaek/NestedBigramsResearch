...
public class Solution {
  ...
  static List<Character> getCombos(Map<Integer,Map<List<Integer>,List<Character>>> expToCombos,
      Map<List<Integer>,List<Character>> combos, int expLimit) {
  ...
  static Map<Integer,Map<List<Integer>,List<Character>>> expToCombos = new HashMap<>();
  ...
  static String algo(int X, int Y) {
    ...
    */
    ...
    if (expToCombos.keySet().size() == 0) {
      expToCombos.put(0, combos);
      getCombos(expToCombos, combos, 8);
    }

    // look for Y, X
    List<Integer> XY = new ArrayList<>();
    XY.add(Y); XY.add(X);
    for (int exp : expToCombos.keySet()) {
      combos = expToCombos.get(exp);
      if (combos.containsKey(XY)) {
        List<Character> path = combos.get(XY);
        String ans = "";

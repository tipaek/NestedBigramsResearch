...
public class Solution {
    ...
    private static int solve(Scanner sc){
        ...
        for(int d=1;d<=D;d++){
            for(int num=0;num<A.length;num++){
                globalMin = Math.min(globalMin, minCutChoosing(num, d, A, D));
            }
        }
        return globalMin;
    }
    ...
    private static int minCnt(final long tani, final List<Long> sortedA, int D){
        PriorityQueue<Harvest> pq = new PriorityQueue<>(Comparator.comparingLong(Harvest::getDiff).thenComparing(Harvest::getGetHon));
        for(Long p:sortedA) {
            Harvest toAdd;
            if(p % tani == 0){

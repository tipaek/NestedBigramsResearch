...
public class JumpsSolution {
    ...
    private static String solve(Set<Point> visited, String path, int iter, int currentX, int currentY, int x, int y) {
        ...
        int jump = (int) Math.pow(2, iter);

        String result = null;

        if (Math.abs(x - currentX) >= jump) {
            result = solve(visited, path + EAST, iter + 1, currentX + jump, currentY, x, y);
            String t = solve(visited, path + WEST, iter + 1, currentX - jump, currentY, x, y);
            if (result == null) {
                result = t;
        ...
        if (Math.abs(y - currentY) >= jump) {
            String t;
            t = solve(visited, path + NORTH, iter + 1, currentX, currentY + jump, x, y);
            if (t != null) {
                if (result == null) {
                    result = t;
            ...
            t = solve(visited, path + SOUTH, iter + 1, currentX, currentY - jump, x, y);
            if (t != null) {
                if (result == null) {
                    result = t;
                } else if (t.length() < result.length()) {

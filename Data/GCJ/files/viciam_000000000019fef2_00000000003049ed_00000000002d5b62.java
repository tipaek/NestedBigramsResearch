...
public class Solution {
	String solver (int targetX, int targetY) {
		int x = targetX;
		int y = targetY;
		String result = "";
	...
	void solveCase(int caseNr) {
		...
		ProblemState ps = new ProblemState(x[1], x[0],"");
		ProblemState sol  = (ProblemState) CPSSolver.solve(ps);
		if (sol == null) {
			System.out.println("Case #"+caseNr+": "+"IMPOSSIBLE");
		} else
		System.out.println("Case #"+caseNr+": "+sol.result);
	...
	class ProblemState implements CPSState {
		int x,y;
		String result;
		public ProblemState(int x, int y, String result) {
			this.x=x;
			this.y=y;
			this.result = result;
		}
		...
		public List<CPSState> nextStates() {
			List<CPSState> res = new ArrayList<CPSState>();
			if (x % 2 == 1 && y % 2 == 1) {
				return null;
			} else if (x % 2 == 0 && y % 2 == 0) {
				return null;
			} else if (x % 2 == 0 && y!=0) {
				res.add(new ProblemState(x/2,(y-1)/2,result+"E"));
				res.add(new ProblemState(x/2,(y+1)/2,result+"W"));
			} else if (y % 2 == 0 && x!=0){
				res.add(new ProblemState((x-1)/2,y/2,result+"N"));
				res.add(new ProblemState((x+1)/2,(y)/2,result+"S"));
			}
			return res;
		}

...
public class Solution {
	...
	public static class Carre {
		...
		public static void makeDiags(int n, int k, int[] tab , int i , int cnt, List<List<Integer>> results) {
			if (cnt > k ) { 
				return ;
			}
			if ( i >= n ) {
			...
			for (int j = 1 ; j <= n ; j++ ) {
				...
				tab2[i] = j ;
				int cnt2 = cnt + j ;
				makeDiags(n , k, tab2 , i + 1 , cnt2, results);
			}
	...
	public static void main(String[] args) {
		...
		while (in.hasNextLine() && NB_CASES > 0 ) {			
			...
			Carre.makeDiags(n, k, new int[n], 0 , 0, results);
			int[][] res = null ;

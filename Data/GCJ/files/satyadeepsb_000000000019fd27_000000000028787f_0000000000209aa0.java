...
public class Solution {
	...
	static void shiftMatrixByK(int [][]mat, int k, int N) { 
		if (k > N) { 
	...
	public static long[][] rotLeft(int aCount, long[][] a, int d) {
	     int i, j;
		long[] temp;
	     for(i=0 ;i < d ;i++){
	         temp = a[0];
	...
	public static long[][] generateMatrix(int diagonal, long matrix[][]){
		for(int r = 0; r < matrix.length; r++){
			for(int c = 0; c < matrix[r].length; c++){
				if(r == c) {
					matrix[r][c] = diagonal;
	...
	public static void main(String[] args) {
		...
		for (int t = 1; t <= testCount; t++) {
			...
			//	printData(rotLeft(2, matrix,1));
			//	printData(rotLeft(2, matrix,2));
				//printData(generateMatrix(1, matrix));
				getRes(N,K,matrix);

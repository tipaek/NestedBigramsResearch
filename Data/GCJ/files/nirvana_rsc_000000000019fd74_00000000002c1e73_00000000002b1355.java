...
public class Solution {
    ...
    private static double[] neighbourSum(int[][] grid, int r, int c) {
        ...
        for (int[] dir : DIRS) {
            ...
            while (currR >= 0 && currR < grid.length && currC >= 0 && currC < grid[0].length && grid[currR][currC] == 0) {
                currR += dir[0];
                currC += dir[1];
            }
            if (currR >= 0 && currR < grid.length && currC >= 0 && currC < grid[0].length && grid[currR][currC] != 0) {
                neighbours++;

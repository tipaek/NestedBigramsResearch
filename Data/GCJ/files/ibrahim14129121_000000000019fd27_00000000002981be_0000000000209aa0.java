...
public class Solution {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < testCase; i++) {
			...
			if (trace % dSize == 0) {
				...
				findCombinationsUtil(sArrayList1, dSize - 1, new int[trace - diagonal], 0, trace - diagonal,
						trace - diagonal);

				LinkedList<int[]> linkedList = new LinkedList<int[]>();

				for (int k = 0; k < 2500; k++) {
					linkedList.add(new int[2500]);
				}
						...
						for (int j2 = 0; j2 < dSize; j2++) {
							...
							if (j == j2) {
								...
								if (linkedList.get(j2)[sArrayList1.get(pos)] == 0) {
									...
									int arp1[] = linkedList.get(0);
									arp1[temp] -= 1;
									linkedList.set(0, arp1);
									 ...
									 * arr[j][j2] = sArrayList1.get(pos);
									 * 
									 * int arp[] = linkedList.get(j2); arp[sArrayList1.get(pos)] += 1;
									 * linkedList.set(j2, arp); // arrr.remove(pos); pos++;
									 */
								}
	...
	static void findCombinationsUtil(ArrayList<Integer> arrayList, int dSize, int arr[], int index, int num,
			int reducedNum) {
// Base condition 
		if (reducedNum < 0)
			return;

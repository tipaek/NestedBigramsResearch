...
public class Solution {
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for(int i = 1; i <= T; i++) {
			...
			for (int k = 0; k < N - 1; k++) {
				for (int l = k + 1; l < N; l++) {
					if (checkIfInterlapping(sortedSchedule[k], sortedSchedule[l])) {
						if (checkTriIntersection(sortedSchedule, k, l, N)) {
							impossible = true;
						}
					}
				}
			...
			else if (unsortedAnswer.length() <= 2) {
				pw.println("Case #" + i + ": " + unsortedAnswer);
			}
			else {

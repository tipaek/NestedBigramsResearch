...
public class Solution {
    ...
    static class ParentingPartneringReturns {
        ...
        private String solve(int n, List<Pair<Integer, Integer>> times) {
            Comparator<Pair<Integer, Integer>> comp = Comparator.comparing(Pair::getLeft);
            comp = comp.thenComparing(Pair::getRight);
            times.sort(comp);
            ...
            for (int i = 0; i < n; i++) {
                int start = times.get(i).getLeft();
                if (cBusyTill <= start) {
                    ans.append('C');
                    cBusyTill = times.get(i).getRight();
                } else if (jBusyTill <= start) {
                    ans.append('J');
                    jBusyTill = times.get(i).getRight();
                } else {
                    return "IMPOSSIBLE";
                }
            }

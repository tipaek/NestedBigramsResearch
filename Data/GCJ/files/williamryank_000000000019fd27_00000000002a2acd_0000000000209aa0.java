    ...
    public static boolean recursive(int matrix[][], HashMap<String, HashSet<Integer>> domains) {
        ...
        for (int a = 0; a < matrix.length; a++) {
            for (int b = 0; b < matrix.length; b++) {
                if (matrix[a][b] == 0 && domains.get(a + "," + b).size() < curr_min) {
                    r = a;
                    c = b;
                    curr_min = domains.get(a + "," + b).size();
                }
            }
        }
        ...
        for (Integer curr : value_order) {
            ...
            if (isValid) {
                ...
                boolean is_ok = true;
                HashMap<String, HashSet<Integer>> removed_values = new HashMap<>();
                for (int temp = 0; temp < matrix.length; temp++) {
                    if (temp != c && domains.get(r + "," + temp).contains(curr)) {
                        if (domains.get(r + "," + temp).size() == 1 ){
                            is_ok = false;
                        }
                    }
                    domains.get(r + "," + temp).remove(curr);
                    HashSet<Integer> trala = new HashSet<>();
                    trala.add(curr);
                    removed_values.put(r + "," + temp, trala);
                }
                
                for (int temp = 0; temp < matrix.length; temp++) {
                    if (temp != r && domains.get(temp + "," + c).contains(curr)) {
                        if (domains.get(temp + "," + c).size() == 1) {
                            is_ok = false;
                        }
                    }
                    domains.get(temp + "," + c).remove(curr);
                    HashSet<Integer> trala = new HashSet<>();
                    trala.add(curr);
                    removed_values.put(temp + "," + c, trala);
                }
                ...
                for (String strs : removed_values.keySet()) {

                    domains.put(strs, removed_values.get(strs));
                }
                matrix[r][c] = 0;
            }
        }

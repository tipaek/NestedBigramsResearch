...
public static int tra(int n,int r,int c, int[][] rv, int[][] cv, int[][] mat,int k) {
    int sum = 0, flag = 0;
    for(int i = 0; i < n; i++) {
        ...
        if(c+1 < n) sum = tra(n, r, c+1, rv, cv, mat, k);
        else if(r+1 < n) sum = tra(n, r+1, 0, rv, cv, mat, k);
        else sum = 0;
        rv[r][i]--;
        cv[c][i]--; 
        if(sum < 0) continue;
        if(r == c) sum += i+1;
        if(sum > k) continue;
        if(sum == k && c == 0 && r == 0) {flag = 1; sum = n*n+n; break;}
        flag = 1;
    }
...
public static String cal(String g){
    ...
    int n = a[0], k = a[1];
    int[][] ma = new int[n][n];
    int s = tra(n, 0, 0, new int[n][n], new int[n][n], ma, k);

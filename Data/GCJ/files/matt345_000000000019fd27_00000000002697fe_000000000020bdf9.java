...
public class Parenting {
	...
	public static boolean testj(String cur, int[]starttimes,int[]endtimes, int i, boolean[]ctimes, boolean[]jtimes) {
		boolean done=false;
		
				for(int k=starttimes[i];k<=endtimes[i];k++) {
					if(jtimes[k]==true&&k!=starttimes[i]&&k!=endtimes[i]) return false; //if there is an obstruction in scheduling we break 
					else if(k==endtimes[i]) { //if there were no obstructions and it got to the end, we assign the activity to c
						done=true;
					}
		...
		if(done) {
			for(int j=starttimes[i];j<=endtimes[i];j++) jtimes[j]=true;
			return true;
		}
		else return false;
	}

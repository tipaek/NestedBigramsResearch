...
public class Solution {
    ...
    private static void makeSchedule(int[][] scheduleInfo, Stack<Pair> cameron, Stack<Pair> jamie, Stack<String> result, int count) {
        ...
        String ppreStatus = "J";
        for (int i = 0; i < count; ) {
//            if(ppreStatus != preStatus) {
//                ppreStatus = preStatus;
//                dupTmp = 0;
//            }
            if (preStatus.equals("J")) {
                if (scheduleCheck(scheduleInfo[i], cameron)) {
                    ...
                    while(ppreStatus == preStatus) {
                        dupTmp++;
                        preStatus = result.pop();
                        if (preStatus.equals("C")) cameron.pop();
                        else jamie.pop();
                    }
//                    dupTmp++;
...
//                    }
                ...
                if (scheduleCheck(scheduleInfo[i], jamie)) {
                    jamie.push(new Pair(scheduleInfo[i][0], scheduleInfo[i][1]));
                    result.push("J");
                    i++;
                } else if (scheduleCheck(scheduleInfo[i], cameron)) {
                    cameron.push(new Pair(scheduleInfo[i][0], scheduleInfo[i][1]));
                    result.push("C");
                    i++;
                }else if (!(scheduleCheck(scheduleInfo[i], cameron) && scheduleCheck(scheduleInfo[i], jamie))) {
...
//                    }
    ...
    private static int getDuplicate(int count, int[][] longs, int duplicate) {

...
public class Solution {
	...
	public static String testCase(Scanner sc) throws Exception {
		...
		for(int r = 0 ; r < matrixSize ; ++r) {
			...
			trace += row.get(r);
			rdup += row.stream().distinct().collect(Collectors.toList()).size() - matrixSize;
		}
		for(int c = 0 ; c < matrixSize ; ++c) {
			final int cf = c;
			cdup += rows.stream().map(row -> row.get(cf)).distinct().collect(Collectors.toList()).size() - matrixSize;
		}

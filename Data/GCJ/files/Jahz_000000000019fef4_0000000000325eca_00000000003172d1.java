...
public class Solution {
	...
	public String compute(String[] input) {
		...
		List<Double> sliceAngles = new ArrayList<Double>();

		String[] angles = input[1].split(" ");
		for (String string : angles) {
			sliceAngles.add(Double.valueOf(string));
		}

		Set<Double> sliceAnglesNoDup = new HashSet<Double>(sliceAngles);
	...
	private boolean minDupIsBiggest(List<Double> sliceAngles, Set<Double> sliceAnglesNoDup) {
		boolean result = false;

		Collections.sort(sliceAngles);
		Collections.reverse(sliceAngles);

		if (sliceAngles.get(0).equals(sliceAngles.get(1))) {
			result = true;
		}
	...
	private boolean hasAnyDouble(List<Double> sliceAngles) {
		boolean result = false;

		for (Double long1 : sliceAngles) {
			if (sliceAngles.contains(2.0 * long1)) {
				result = true;
				break;
			}
		}

...
public class Solution {
    ...
    public static void main(String[] args) {
        ...
        for (int num = 1; num <= t; num++) {
            ...
            Map<Character, Integer> soulutionR = new HashMap<>();
            ...
            for (int i = 1; i <= n; i++) {
                ...
                if (r.length() == String.valueOf(q).length() ) {
                    ...
                    int ind1 = 0;
                    while (ind1 < qStr.length()) {
                        
                        if (get(ind1, q) == 1 ||
                                (
                                        soulutionR.get(r.charAt(ind1)) != null && soulutionR.get(r.charAt(ind1)) == get(ind1,q)
                                )) {
                            ind1++;
                        } else {
                            break;
                ...
                map.forEach((k, v) -> {
                    if (v.size() == 1) {
                        ...
                        soulution.put(next, k);
                        soulutionR.put(k, next);
                    }
                });

                map.forEach((k,v) -> soulution.keySet().forEach(s -> v.remove(s)));
                if(soulution.keySet().size() == 10) {
                    break;
                }

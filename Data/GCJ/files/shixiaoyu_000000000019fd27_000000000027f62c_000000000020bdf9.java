...
public class Solution {
    ...
    private String calShift(int[][] schedule) {
        ...
        // could be dup end time, so use a list
        Map<Integer, List<Integer>> reverseLookup = new HashMap<>(); // key is the end, value is the start, this is used later to populate the correct C/J value

        for (int i = 0; i < row; i++) {
            points.add(new Point(schedule[i][0], true, i));
            points.add(new Point(schedule[i][1], false, i));

            if (reverseLookup.containsKey(schedule[i][1])) {
                reverseLookup.get(schedule[i][1]).add(schedule[i][0]);
            } else {
                List<Integer> l = new ArrayList<>();
                l.add(schedule[i][0]);
                reverseLookup.put(schedule[i][1], l);
            }
        }
        ...
        for (int i = 0; i < points.size(); i++) {
            ...
            if (p.isStart) {
                ...
                overlapCount--;
                List<Integer> startIndexArray = reverseLookup.get(p.val);
                for (Integer startIndex : startIndexArray) {

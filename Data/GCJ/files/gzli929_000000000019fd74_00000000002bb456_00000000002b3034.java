	...
	public static void main(String[] args) throws IOException{
		...
		for(int t = 1; t <= tC; t++) {
			...
			for(int i = 1; i < n; i++) {
				...
				if(end.length() > e.length() && end.indexOf(e.toString()) == end.length()-e.length() || e.length() == 0) {
				
					//System.out.println(end + " " + e);
					e = new StringBuilder(end);
				}
				else if(end.length() == 0 || (e.indexOf(end.toString()) == e.length()-end.length() && e.length() >= end.length())) {
				}
				else {

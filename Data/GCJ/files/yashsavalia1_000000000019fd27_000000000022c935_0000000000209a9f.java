			...
			for(int i=input.length()-1;i>=0;i--) {
				if(Character.isDigit(input.charAt(i))){
					...
					if(previouseValue >= element) {
						...
						input.insert(i+1, getOpeningBrackets(element-previouseValue));
						input.insert(i+element-previouseValue+1, String.valueOf(element));
						input.insert(i+element-previouseValue+2, getClosingBrackets(element-previouseValue));
						inserted=true;
					}
					if(inserted) {
						break;

...
/*
    Solution Created: 17:55:47 04/04/2020
...
public class Solution {
	...
	public static void rec(int i, int j, int k, int n) {
		if(found) return;
		if(i==n) {
			if(good(a,k)) {
				found = true;
			...
			for(int m = 1; m<=n; m++) {
				if(found) return;
				boolean cont = false;
				for(int v = 0; v<i; v++) if(a[v][j]==m) cont = true;
				for(int v = 0; v<j; v++) if(a[i][v]==m) cont = true;
				if(cont) continue;
				a[i][j] = m;
				rec(ni, nj, k, n);
			}
	...
	public static boolean good(int[][] ans, int k) {
		int n = ans[0].length;
		int r = 0, c = 0;
		for(int i = 0; i<n; i++) {
			HashSet<Integer> f = new HashSet<Integer>();
			for(int j = 0; j<n; j++) {
				if(f.contains(ans[i][j])) {
					r++;
					break;
				}
				f.add(ans[i][j]);

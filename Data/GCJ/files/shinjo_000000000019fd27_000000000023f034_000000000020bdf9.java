...
public class Solution {
    public static void main(String[] args) {
        ...
        getInput(roofTestCase, tc, schedulCount, scheduleInfo[tc], sc);
        ...
        while (roofTestCase-- > 0) {
            ...
            duplicate = checkDuplicate(schedulCount[tc], scheduleInfo[tc], duplicate);

            if (duplicate <= 1)
                makeSchedule(0, scheduleInfo[tc], cameron, jamie, result, schedulCount[tc]);

            tc = printResult(testCase, tc, duplicate, result);
        }
    }

    private static void getInput(int roofTestCase, int tc, int[] schedulCount, int[][] ints, Scanner sc) {
        while (roofTestCase-- > 0) {
    ...
    private static int checkDuplicate(int i1, int[][] longs, int duplicate) {
    ...
    private static int printResult(int testCase, int tc, int duplicate, Stack<String> result) {

...
public class Solution {
	...
	public static boolean available(boolean[] assigned, int[][] times, int indexToAssign, int numAct)
	{
		for(int i = 0; i < numAct; i++)
		{
			if (assigned[i] == true)
			{
				if(times[indexToAssign][0] < times[i][1]) // the the to assign starts before the assigned ends.  equal is ok
				{
					return false;
				}
				if (times[indexToAssign][1] < times[i][0])// or if it ends after the other starts, equal is ok
				{
					return false;
				}
			}

...
//        }
        ...
        ArrayList<Pair<Pair<Integer,Integer>,Integer>> pp = new ArrayList<>();
        for(int i=0;i<n;i++){
            int[] xx = obj.intArr();
            pp.add(new Pair<Pair<Integer,Integer>,Integer>(new Pair<Integer,Integer>(xx[0],xx[1]),i));
        }
        Collections.sort(pp, new Comparator<Pair<Pair<Integer, Integer>, Integer>>() {
            @Override
            public int compare(Pair<Pair<Integer, Integer>, Integer> o1, Pair<Pair<Integer, Integer>, Integer> o2) {
                if(Integer.compare(o1.first.first,o2.first.first)==0)
        ...
        int p  = pp.get(0).first.second ,q = 0;
//        sb.append("C");

        char[] c = new char[n];
        c[pp.get(0).second] = 'C';
        for(int i=1;i<n;i++){
            if(pp.get(i).first.first>=p){
                p = pp.get(i).first.second;
//                sb.append("C");
                c[pp.get(i).second] = 'C';
            }
            else if(pp.get(i).first.first>=q){
                q = pp.get(i).first.second;
//                sb.append("J");
                c[pp.get(i).second] = 'J';
            }
...
//        for(int i=0;i<n;i++){
//            System.out.println(pp.get(i).first.first+" "+pp.get(i).first.second+"  "+pp.get(i).second);
//        }
        return new String(c)+"";
    }

...
public class Solution {
        ...
        for (int i = 0, time = 0; i <= (result.length - 1) / 2; i++) {
            if (time > 0 && time % 10 == 0) {
                ...
                if (diff == -1 || same == -1) {
                    ...
                    int newBit = makeCall(in, target);
                    time++;
                    if (newBit != result[target]) {
                        complement(result, i);
                    }
                    ...
                    int newSame = makeCall(in, same);
                    time++;
                    boolean sameChange = newSame != result[same];

                    int newDiff = makeCall(in, diff);
                    time++;
                    boolean diffChange = newDiff != result[diff];
            ...
            result[i] = makeCall(in, i);
            time++;
            
            result[b - i - 1] = makeCall(in, b - i - 1);
            time++;
    ...
    private static int makeCall(Scanner in, int i) {

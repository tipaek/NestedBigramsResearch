    ...
    public static String solution(int[] start_time, int[] end_time) {
        ...
        for (int i = 0; i < start_time.length; i++) {
            ...
            for (Map.Entry<Integer, Integer> j_entry : J_activities.entrySet()) {
                ...
                boolean start_overlap = activity_start >= j_entry_activity_start && activity_start < j_entry_activity_end;
                boolean end_overlap = activity_end >= j_entry_activity_start && activity_end < j_entry_activity_end;
            ...
            if (j_possible) {
                ...
                for (Map.Entry<Integer, Integer> c_entry : C_activities.entrySet()) {
                    ...
                    boolean start_overlap = activity_start >= c_entry_activity_start && activity_start < c_entry_activity_end;
                    boolean end_overlap = activity_end >= c_entry_activity_start && activity_end < c_entry_activity_end;
                    if (start_overlap || end_overlap) {
                        c_possible = false;
                        break;
                    }

...
public class Solution {
				...
				for (Activity otherActivity : equivalentActivities) {
					if (!activity.equals(otherActivity)) {
						if (activity.getStartTime() >= otherActivity.getStartTime() && activity.getStartTime() < otherActivity.getEndTime()
							...
							collisions++;
							// System.out.println("Feas: " + activity.toString() + " collides with " + otherActivity.toString());
							if (collisions > 1) {
								feasible = false;
								break;
							}
		...
		while (!done) {
				...
				for (Entry<Integer, Activity> otherActivity : equivalentActivitiesMap.entrySet()) {
					if (!activity.equals(otherActivity)) {
						if (activity.getValue().getStartTime() >= otherActivity.getValue().getStartTime()
							...
							// System.out.println("Equi1: " + activity.toString() + " collides with " + otherActivity.toString());
							...
							break;
								...
								&& activity.getValue().getEndTime() >= otherActivity.getValue().getEndTime()) {
							// System.out.println("Equi2: " + activity.toString() + " collides with " + otherActivity.toString());
							firstCollider = activity.getKey();
							secondCollider = otherActivity.getKey();
			...
			if (firstCollider != null) {
				...
				if (completeOverlapping) {
					...
					// System.out.println("Complete, removing: " + firstToStart);

					addToMap(equivalentActivitiesMap, nextId, new Activity(nextId, firstToStart.getStartTime(), secondToStart.getStartTime()));
					nextId++;
					if (firstToStart.getEndTime() < secondToStart.getEndTime()) {
						addToMap(equivalentActivitiesMap, nextId, new Activity(nextId, firstToStart.getEndTime(), secondToStart.getEndTime()));
					} else {
						addToMap(equivalentActivitiesMap, nextId, new Activity(nextId, secondToStart.getEndTime(), firstToStart.getEndTime()));
					}
					nextId++;
				} else {
					equivalentActivitiesMap.remove(firstToStart.getId());
					...
					// System.out.println("Complete, removing: " + firstToStart);
					// System.out.println("Complete, removing: " + secondToStart);
					...
					if (firstToStart.getEndTime() < secondToStart.getEndTime()) {
						addToMap(equivalentActivitiesMap, nextId, new Activity(nextId, firstToStart.getEndTime(), secondToStart.getEndTime()));
						nextId++;
						addToMap(equivalentActivitiesMap, nextId, new Activity(nextId, secondToStart.getStartTime(), firstToStart.getEndTime()));
					} else {
						addToMap(equivalentActivitiesMap, nextId, new Activity(nextId, secondToStart.getEndTime(), firstToStart.getEndTime()));
						nextId++;
						addToMap(equivalentActivitiesMap, nextId, new Activity(nextId, secondToStart.getStartTime(), secondToStart.getEndTime()));
					}
					nextId++;
				}
	...
	private static void addToMap(Map<Integer, Activity> equivalentActivitiesMap, int nextId, Activity activity) {
		for (Entry<Integer, Activity> otherActivity : equivalentActivitiesMap.entrySet()) {
			if (otherActivity.getValue().getStartTime() == activity.getStartTime() && otherActivity.getValue().getEndTime() == activity.getEndTime()) {
				return;
			}
		}
		// System.out.println("Complete, adding: " + activity.getStartTime() + " to " + activity.getStartTime());
		equivalentActivitiesMap.put(nextId, activity);
	}

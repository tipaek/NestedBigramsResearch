...
public class Solution {
  ...
  private static void printPath(List<int[]> path) {
    for (int i = 0; i < path.size(); i++) {
      if (i == path.size() - 1) {
        System.out.print(String.format("%d %d", path.get(i)[0]+1, path.get(i)[1]+1));
      } else {
        System.out.println(String.format("%d %d", path.get(i)[0]+1, path.get(i)[1]+1));
      }
    }
  }

  private static boolean dfs(List<List<Integer>> pscTriangle, int target, List<int[]> path, int i, int j, int sum) {
    if (sum > target || path.size() > 500) {
      return false;
    }

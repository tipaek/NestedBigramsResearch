...
****/
    ...
    // System.err.println("indexarr="+ Arrays.toString(indexarr));
    // PrintMinutes();
    ...
    public int compute() {
        ...
        for( int i=0; i< num_durations; i++ ) {
            ...
            // System.err.println("start="+ start[indexarr[i]] + " end="  + end[indexarr[i]] + " who= " + who[indexarr[i]]);
            if( Assign( TRY, indexarr[i], CAMERON) == true ) {
                Assign( ACTUAL, indexarr[i], CAMERON);
                who[indexarr[i]] = CAMERON;
            } else if( Assign(TRY, indexarr[i], JAMIE ) == true ) {
                Assign( ACTUAL, indexarr[i], JAMIE);
                who[indexarr[i]] = JAMIE;
            } else {
                    return -1;
            }
            // System.err.println("start="+ start[indexarr[i]] +" end=" + end[indexarr[i]] + " who= " + who[indexarr[i]]);

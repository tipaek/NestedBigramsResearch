...
public class Solution {
	...
	public long count(long[] a, Fraction f) {
		long ans = 0;
		for (int i = 0; i < a.length; i++) {
			ans += (a[i] * f.y + f.x - 1) / f.x;
		}
		return ans;
	}
	...
	public void solve() {
		...
		f: for (int test = 1; test <= nTest; test++) {
			...
			for (int i = 0; i < a.length; i++) {
				for (int j = 1; j <= d; j++) {
					Fraction th = new Fraction(a[i], j, true);
			...
			TreeMap<Fraction, Integer> number = new TreeMap<>();
			int fract = map.size();
			int c = 0;
			Fraction[] fr = new Fraction[fract];
			for (Fraction f : map.keySet()) {
				number.put(f.clone(), c++);
				fr[c - 1] = f.clone();
			}
			TreeMap<Integer, Integer>[] size = new TreeMap[fract];
			...
			for (int i = 0; i < a.length; i++) {
				for (int j = 1; j <= d; j++) {
					Fraction th = new Fraction(a[i], j, true);
					int k = number.get(th);
					if (size[k].containsKey(j)) {
						size[k].put(j, size[k].get(j) + 1);
					} else {
						size[k].put(j, 1);
						if (size[k].size() > d) {
							size[k].pollLastEntry();
						}
					}
				}
			}
			...
			for (int i = 0; i < r; i++) {
				long th = d;
				long num = d;
				f1 : for (int j : size[i].keySet()) {
					for (int k = 0; k < size[i].get(j); k++) {
						if (num >= j) {
							num -= j;
							th--;
						} else {

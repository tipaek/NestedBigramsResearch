	...
	public static void main(String[] args) throws IOException{
		...
		while (t-->0) {
			...
			while (!q.isEmpty()) {
				...
				if(p.a-(1<<(p.p+1))>x-100) q.add(new Pair(p.a-(1<<(p.p+1)),p.b,p.p+1,'W',p));
				if(p.b-(1<<(p.p+1))>y-100) q.add(new Pair(p.a,p.b-(1<<(p.p+1)),p.p+1,'S',p));
				if(p.a+(1<<(p.p+1))<x+100) q.add(new Pair(p.a+(1<<(p.p+1)),p.b,p.p+1,'E',p));
				if(p.b+(1<<(p.p+1))<y+100) q.add(new Pair(p.a,p.b+(1<<(p.p+1)),p.p+1,'N',p));
			}
			if (!ok) {
				bw.write("Case #"+lp+": IMPOSSIBLE\n");
				continue;

...
public void solve() {
	...
	for(int q = 0; q < query; q ++) {
		...
		l -= x * (x + 1) / 2;
		swap &= l != r;
		long ans = bisect2(x, l, r);
		long addR = (ans - x) / 2;
		long addL = ans - x - addR;
		l -= addL * (x * 2 + addL * 2) / 2;
		r -= addR * (x * 2 + addR * 2 + 2) / 2;
		if(swap) {
			long tmp = l;
			l = r;
			r = tmp;
...
long bisect2(long x, long l, long r) {
	long ok = x;
	long ng = (long)2e9 + 100;
	while (abs(ok - ng) > 1) {
		long mid = (ok + ng) / 2;
		
		long addR = (mid - x) / 2;
		long addL = mid - x - addR;
		long ll = l - addL * (x * 2 + addL * 2) / 2;
		long rr = r - addR * (x * 2 + addR * 2 + 2) / 2;
		boolean isOK = ll >= 0 && rr >= 0;
...
long bisect(long diff) {
	long ok = 0;
	long ng = (long)2e9;
	while (abs(ok - ng) > 1) {
		long mid = (ok + ng) / 2;
		
		boolean isOK = mid * (mid + 1) / 2 <= diff;

...
//        Scanner in = new Scanner(new BufferedReader(new FileReader("/Users/vyachs/code/codejam/z3.txt")));
        ...
        for (int caseIndex = 1; caseIndex <= caseNum; caseIndex++) {
            ...
            if (customerNum == 2) {
                ...
                if (numberOfEquals >= 3) {
                    result = "0";
                } else if ((blah(slices) && sliceNum > 2) || oneTwiceHigherThanAnother(slices)) {
                    result = "1";
                } else {
                    result = "2";
                }
    ...
    private static boolean blah(List<Long> slices) {
        Map<Long, Integer> count = new HashMap<>();
        for (Long slice : slices) {
            if (count.containsKey(slice)) {
                count.put(slice, count.get(slice) + 1);
        ...
        Long aLong = count.entrySet().stream()
                .filter(e -> e.getValue() == 2)
                .sorted(new Comparator<Map.Entry<Long, Integer>>() {
                    @Override
                    public int compare(final Map.Entry<Long, Integer> o1, final Map.Entry<Long, Integer> o2) {

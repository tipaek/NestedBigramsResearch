...
public class Solution {
    ...
    private static int solve(int n, int d, Scanner in) {
        ...
        if(d == 2) {
           ...
           if(canSplit1(counts) || canChopOff(counts)) {
               return 1;
           } else {
               return 2;
           }
    ...
    private static boolean canChopOff(Map<Long, Integer> counts) {
        long minSize = Long.MAX_VALUE;
        for (Map.Entry<Long, Integer> pieces: counts.entrySet()) {
            if(pieces.getKey() < minSize && pieces.getValue() == 2) {
                minSize = pieces.getKey();
            }
        }
        
        for (long k: counts.keySet) {
            if(k>minSize) return true;
        }
        return false;
    }

...
public class Solution {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int test = 1; test <= testCase; test++) {
			...
			if (cuts == table) {
				...
				int factor = getFactors(degreeList, table);
				System.out.println("Case #" + test + ": " + (table - factor));
			} else {
	...
	private static int getFactors(List<Long> degreeList, int table) {
		int factor=1;
		for (int i=0; i<degreeList.size()-1;i++)
		{
			for(int j=i+1; j<degreeList.size();j++)
			{
				...
				{
					int div = (int) (degreeList.get(j)/degreeList.get(i));
					if(factor<div)
						factor=div;
				}
			}

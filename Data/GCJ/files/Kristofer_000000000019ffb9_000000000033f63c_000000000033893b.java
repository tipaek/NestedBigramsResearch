...
public class Solution {
  ...
  private String solve() {
    ...
    long total = 0;
    for (int i = 0; i < Q; i++) {
      total += solve(S[i], E[i], visited);
    }
    return "" + total;
  }

  private long solve(int start, int end, Set<Integer> visited) {
    visited.clear();
    PriorityQueue<Entry> queue = new PriorityQueue<>(Comparator.comparingLong(Entry::getCost).thenComparingInt(e -> Math.abs(end - e.position)));

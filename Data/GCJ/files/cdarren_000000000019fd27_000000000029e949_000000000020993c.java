     ...
     private String compute() throws IOException {
        ...
        for (int i = 0; i < result.size(); i++) {
            ...
            for (int j = 0; j < result.size(); j++) {
                ...
                if (i == 0) {
                    columnHash.add(result.get(i).get(j));
                    columnHash.add(result.get(i + 1).get(j));
                    columnHash.add(result.get(i + 2).get(j));
                    columnHash.add(result.get(i + 3).get(j));
                    if (columnHash.size() != result.size()) {
                        column++;
                    }
                }
            }

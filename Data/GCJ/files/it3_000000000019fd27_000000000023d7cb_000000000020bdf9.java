...
//    private static final String IMPOSSIBLE = "IMPOSSIBLE";
    ...
    private void run(int caseNumber) throws Exception {
        ...
        boolean ok = isBipartite(g, color);
//        System.out.println("  color[" + color.length + "]=" + Arrays.toString(color));
        String res;
        if (ok) {
            char[] a = new char[N];
...
//        out.println();
    ...
    private static boolean isBipartite(int[][] g, int[] color) {
        Arrays.fill(color,-1);        
    ...
    private static boolean dfs(int i, int odd, int[] color, int[][] g) {
        color[i]=odd;
        for (int j = 0; j < g[i].length; j++) {
            if(g[i][j]>0) {
                if (color[j]<0) {
                    if (!dfs(j,1-odd,color,g)) {
                        return false;
                    }

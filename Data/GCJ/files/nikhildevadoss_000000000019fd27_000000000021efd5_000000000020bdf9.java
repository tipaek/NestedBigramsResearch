 import java.util .*;
    ...
    public class Solution {
            ...
            for(int i = 1; i <= t; i++){
                ...
                if(!possible(tasks)){
                    ...
                    for(int j = 0; j < tasks.length; j++){
                        if(isAvailable(tasks[j][0], tasks[j][1], cTasks)){
                            schedule[j] = "C";
                            cTasks[cIndex][0] = tasks[j][0];
                            cTasks[cIndex][1] = tasks[j][1];
                        }
                        else{
                            schedule[j] = "J";
                            jTasks[jIndex][0] = tasks[j][0];
                            jTasks[jIndex][1] = tasks[j][1];
                        }
                    }
        ...
        public static boolean possible(int [][] tasks){
            for(int i = 0; i < tasks.length; i++){
                int num = 0;
                for(int j = 0; j < tasks.length; j++){
                    if(i >= tasks[j][0] && i <= tasks[j][1]){
                        num++;
                    }
                }
                if(num > 2){

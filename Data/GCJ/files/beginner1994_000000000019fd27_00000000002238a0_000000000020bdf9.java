...
class Solution {
	...
	public static void main(String[] args) throws Exception {
		...
		outer: for(int _i=1;_i<=_t;_i++)
		{
			...
			for(int i=0;i<n;i++){
				...
				long end = result[i][1];
				if(!isOverlap(cList,start,end))
				{
					cList.add(new Wrapper(start,end));
					sb.append("C");
				}
				else if(!isOverlap(jList,start,end))
				{
	...
	static boolean isOverlap(List<Wrapper> arr, long start,long end) 
	{ 
		...
		max_ele = Math.max(max_ele,end);
		int []aux = new int[(int)max_ele + 1]; 
		for (Wrapper w : arr) 
		{ 
			int x = (int)w.start; 
			int y = (int)w.end; 

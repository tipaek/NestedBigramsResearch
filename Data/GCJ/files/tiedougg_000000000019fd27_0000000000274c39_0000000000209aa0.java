...
public class Solution {
    ...
    private static void solve(int n, int k, int i)
    {
        ...
        if(dfs(n, board, lflag, cflag, 0, 0, 0, k, memo))
        {
    ...
    private static boolean dfs(int n, int[][] board, Set<Integer>[] lflag, Set<Integer>[] cflag, int r, int c, int cnt, int k, Set<String> memo)
    {
        String pb = printBoard(board, n);
        if(memo.contains(pb)) return false;
        for(int i = 1; i<=n; i++)
        {
            if(!lflag[r].contains(i)&&!cflag[c].contains(i))
            {
                ...
                if(cnt+1 == n*n) 
                {
                    ...
                    {
                        memo.add(printBoard(board, n));       
                    }
                }
                //String pb1 = printBoard(board, n);
                //if(memo.contains(pb1)) continue;
                for(int kk = 0; kk<4; kk++)
                {
                    ...
                    {
                        if(dfs(n, board, lflag, cflag, newR, newC, cnt+1, k, memo))
                        {
                            return true;
                        }
                    }

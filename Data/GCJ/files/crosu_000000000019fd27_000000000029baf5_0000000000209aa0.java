...
public class Solution {
	...
	public static ArrayList<Integer> to_trace_sig(Vector<Integer> trace) {
		HashMap<Integer, Integer> values = to_trace_info(trace);
		ArrayList<Integer> counts = new ArrayList<Integer>(values.values());
	...
	public static class LatinSquare {
		...
		public ArrayList<Integer> trace_sig() {
			return to_trace_sig(trace());
		}
	}
	
	public static void advance(LatinSquare l, int next, HashMap<ArrayList<Integer>, LatinSquare> latin_squares) {
		...
		for (int i = 0; i < l.n; i++) {
			if (l.is_valid_set(next, i+1))
			{
				...
				if (last_step) {
					ArrayList<Integer> trace_sig = l.trace_sig();
	...
	public static HashMap<ArrayList<Integer>, LatinSquare> generate_latin_squares(int n) {
		HashMap<ArrayList<Integer>, LatinSquare> latin_squares = new HashMap<ArrayList<Integer>, LatinSquare>();
	...
	public static LatinSquare convertLatinSquare(LatinSquare base, Vector<Integer> trace) {
		...
		for (int i = 0; i < trace.size(); i++) {
			int target_count = this_trace_info.containsKey(i+1) ? this_trace_info.get(i+1) : 0;
			
			for (int unused : unused_base) {
				int base_count = base_trace_info.containsKey(unused) ? base_trace_info.get(unused) : 0;
	...
	public static HashMap<Integer, LatinSquare> solve_for_n(int n) {
		HashMap<ArrayList<Integer>, LatinSquare> latin_squares = generate_latin_squares(n);
		...
		for (int i = 0; i < traces.size(); i++) {
			...
			ArrayList<Integer> trace_sig = to_trace_sig(trace);
			int trace_sum = to_trace_sum(trace);
			if (latin_squares.containsKey(trace_sig)) {
				LatinSquare latin_square =
				  convertLatinSquare(latin_squares.get(trace_sig), trace);
				solutions.put(trace_sum, latin_square);
			}
		}

...
public class Solution {
    ...
    private static List<List<Integer>> getAllPossibleDiag(int dim, int trace) {
        // System.out.println(String.format("dim:%d trace:%d", dim, trace));
        if (dim == 1) {
            if (dim == trace) {
        ...
        for (int i = 1; i <= Math.min(trace - dim + 1, dim); i++) {
            int temp = i;
            List<List<Integer>> subProbSoln = getAllPossibleDiag(dim - 1, trace - i);
    ...
    private static int[][] solve(int dim, int trace) {
        ...
        getAllPossibleDiag(dim, trace).forEach(diag -> {
    ...
    //         noDupDiags.add(diag);
    //     });
    //     System.out.println(getAllPossibleDiag(dim, trace));
    //     System.out.println(noDupDiags);
    // }

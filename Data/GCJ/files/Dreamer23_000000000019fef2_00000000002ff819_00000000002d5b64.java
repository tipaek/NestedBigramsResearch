...
//		test();
	...
	private static String getResult(final Scanner in) {
		...
		state.swaps = new ArrayList<Swap>();
		state.cards = cards;
		Queue<State> q = new ArrayDeque<>();
		q.add(state);
		while(true) {
			State st = q.poll();
			if(findNextIncorrectCard(st.cards, tS, size) == -1) return swapsToOutput(st.swaps);
			for(int i = 0; i < size - 1; i++) for(int j = i+1; j < size; j++) {
				State newState = new State();
				newState.swaps = clone(st.swaps);
				newState.swaps.add(new Swap(i + 1, j - i));
				newState.cards = swapCards(st.cards, i, j, size);
				q.add(newState);
			}
		}
	}
	
	private static List<Swap> clone(List<Swap> swaps) {
		List<Swap> cl = new ArrayList<>();
		for(Swap s : swaps) cl.add(s);
		return cl;
	}

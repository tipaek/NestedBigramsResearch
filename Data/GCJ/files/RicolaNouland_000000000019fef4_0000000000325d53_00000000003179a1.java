...
//
        ...
        input.stream().forEach(l -> potZero.remove(l[1].charAt(0)) );
...
//        return list.stream().map(e -> "" + e.getKey()).collect(Collectors.joining());
        ...
        order.add(0, potZero.iterator().next());
        return order.stream().map(e -> "" + e).collect(Collectors.joining());

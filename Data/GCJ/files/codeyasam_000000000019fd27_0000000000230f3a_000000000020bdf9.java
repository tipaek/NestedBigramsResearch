...
class TaskSchedule {
    ...
    public boolean hasOverlappingHours(Integer[] otherTaskTime) {
        ...
        if (os <= startTime && oe < endTime && oe > startTime) return true;
        else if (os >= startTime && os < endTime && oe > startTime && oe <= endTime) return true;
        else if (os >= startTime && os < endTime && oe >= endTime) return true;
        else if (os <= startTime && oe >= endTime) return true;

...
public class Solution {
                ...
                for (int i = 0; i < n; i++) {
                    for (int j = 0; j < n; j++) {
                        if (arr[i][0] == a[j][0] && arr[i][1] == a[j][1]) {
                            ...
                            if(a[j][0]==a[j+1][0] && a[j][1]==a[j+1][1] && j<n-1){
                                if (a[j][2] == 1)
                                    System.out.print("CJ");
                                else
                                    System.out.print("JC");
    ...
    static int parenting(int a[][],int n){
        ...
        for(int i=1;i<n;i++){
            ...
            else if(a[i][0]==a[i-1][0] && a[i][1]==a[i-1][1]){   // the elements are all equal
                if(a[i-1][2]==1) {  //goto zero
                    for(int k=0;k<pos_0;k++){
                        if(a[i][0]==zero[k][0] && a[i][1]==zero[k][1])   // 1 to 150 and a[k][0]=100 then impossible
                            return -1;
                    }
                    zero[pos_0][0]=a[i][0];zero[pos_0][1]=a[i][1];pos_0++;
                    a[i][2]=0;
                    ...
                    for(int k=0;k<pos_1;k++){
                        if(a[i][0]==one[k][0] && a[i][1]==one[k][1])   // 1 to 150 and a[k][0]=100 then impossible
                            return -1;
                    }
                    one[pos_0][0]=a[i][0];one[pos_0][1]=a[i][1];pos_1++;
                    a[i][2]=1;
                }
            }
        }

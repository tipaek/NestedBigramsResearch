    ...
    public Solution() throws FileNotFoundException {
        ...
        while (T-- > 0) {
            ...
            for (int i = 0; i < values.length; ++i) {
                ...
                //if min value isn't first element, we need to recompute it
                if (firtElement.curVal > 0) --i;
            }
            StringBuilder sb = new StringBuilder();
            for (Element e: values) {
            	int remain = e.curVal;
                while(e.before-- > 0 || remain-- != 0) sb.append('(');
                sb.append(e.oriVal);
                remain = e.curVal;
                while(e.after-- > 0 || remain-- != 0) sb.append(')');
            }

...
public class Solution {
	...
	private static boolean is_resource_occupied(int[][] schedule, int startTime, int endTime, int scheduled_activities) {
		boolean occupied = false;
		for (int i = 0;i < scheduled_activities ;i++) {
			if ((startTime > schedule[i][0] && startTime < schedule[i][1]) || (endTime > schedule[i][0] && endTime < schedule[i][1])) {
				occupied = true;
				break;
			}

...
public class Solution {
    ...
    public static HashMap<String, String> invert = new HashMap<String, String>();
    ...
    public static String get(int x, int y) {
        ...
        x = Math.abs(x);
        int xprimep = 0;
        if(x!=0) xprimep = 1 << (int)(Math.ceil(Math.log(x)/Math.log(2)));
        int xprimen = xprimep-x;

        int y_sgn = (int) Math.signum(y);
        y = Math.abs(y);
        int yprimep = 0;
        if(y!=0) yprimep = 1 << (int)(Math.ceil(Math.log(y)/Math.log(2)));
        ...
        Collections.sort(sols, new LengthComp());
        return sols.get(0);
    }
}

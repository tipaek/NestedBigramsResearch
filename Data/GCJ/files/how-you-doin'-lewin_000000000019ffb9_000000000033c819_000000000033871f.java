...
//            int maxedge = 0;
//            while (ap1 < node1.size() && dist[node1.get(ap1)] == seen) {
//                for (Edge x : graph[node1.get(ap1)]) {
...
//                ap1++;
//            }
//            nweight = Math.max(minedge + 200, Math.min(maxedge + 1, nweight));
...
//            }
//
//            while (ap2 < node2.size() && dist[node2.get(ap2)] == nweight) {
//                seen++;
//                int cur = node2.get(ap2);
//                adist[cur] = nweight;
//                for (Edge x : graph[cur]) {
//                    if (res[x.id] == 0 && adist[x.to] < INF) {
//                        res[x.id] = Math.max(1, adist[cur] - adist[x.to]);

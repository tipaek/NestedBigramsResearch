...
//        System.out.println(solve(-100, 100));
    ...
    private static String solve(int goalX, int goalY) {
        ...
        Move start = new Move(0, 0, "", 1);
        double initialDistance = start.distanceToGoal(goalX, goalY);
        queue.add(start);
        int jumpsCloserToGoal = 0;

        while (!queue.isEmpty() && jumpsCloserToGoal < 1000) {
            Move move = queue.poll();
            double distanceToGoal = move.distanceToGoal(goalX, goalY);

            if (distanceToGoal > initialDistance) {
                jumpsCloserToGoal++;
            } else {
                jumpsCloserToGoal = 0;
            }

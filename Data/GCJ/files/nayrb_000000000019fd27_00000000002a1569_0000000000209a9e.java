...
public class Solution {
	...
	public static int ff (LL list, boolean same, int B) throws IOException{
		if(list.size() == 0) return 0;
		int x = list.removeFirst();
		int rep = (usedLast + 1)%2;
		list.addFirst(x);
		...
		/*
			...
			for(int kei = 0; kei < keis; kei++){
				...
				if(sameIdx != -1 && diffIdx != -1) {
					...
					for(int i = 0; i < B/10; i++){
						int groupNum = i;
						int qCnt = f(g[groupNum].same[0], true, B);
						if(qCnt == 0){
							f(g[groupNum].same[1], true, B);
						}
						else{
							ff(g[groupNum].same[1], true, B);
						}
					}
					/*
					int prevSame = query(sameIdx);
					...
					for(int i = 0; i < B/10; i++){
						int groupNum = i;
						int qCnt = f(g[groupNum].diff[0], false, B);
						if(qCnt == 0){
							f(g[groupNum].diff[1], false, B);
						}
						else{
							ff(g[groupNum].diff[1], false, B);
						}
					}
					//diffIdx != -1
					/*

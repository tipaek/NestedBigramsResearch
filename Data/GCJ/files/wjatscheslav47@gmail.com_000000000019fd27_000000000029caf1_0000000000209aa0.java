...
public class Solution
{
  ...
  private static String getMatrixStringEven(int matrixRange, int trace)
  {
    ...
    {
      int[][] matrix = new int[matrixRange][matrixRange];
      fillTwoColumns(trace, matrix, matrixRange);
      for (int i = 0; i < matrixRange; i++)
      {
        for (int j = 0; j < matrixRange; j++)
        {
          ...
          int el = i + 1 >= matrixRange ? matrix[0][j - 2] : matrix[i + 1][j - 2];
          matrix[i][j] = el;
  ...
  private static void fillTwoColumns(int trace, int[][] matrix, int matrixRange)
  {
    Deque<Integer> traceNumbers = new ArrayDeque<>();
    Deque<Integer> nonTraceNumbers = new ArrayDeque<>();
    int halfTrace = trace / 2;
  ...
  private static Set<Integer> getPossibleTraces(int matrixRange)
  {
    ...
    if (matrixRange % 2 == 0)
    {
      ...
      int minUnusual = (matrixRange / 2) + getSum(2, matrixRange / 2 + 1);
      int maxUnusual = matrixRange / 2 * matrixRange + getSum(matrixRange - 1, matrixRange - matrixRange / 2);
      for (int i = minUnusual; i <= maxUnusual; i++)
      {
        traces.add(i);
      }
    }

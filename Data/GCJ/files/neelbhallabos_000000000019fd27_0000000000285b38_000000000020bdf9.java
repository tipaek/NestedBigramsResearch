...
public class Solution {
	...
	public static void main(String[] args) throws IOException{
		...
		for(int cas = 0;cas < T;cas++){
			...
			int lastC = -1;
			int lastJ = -1;
			PriorityQueue<int[]> k = new PriorityQueue<int[]>(new java.util.Comparator<int[]>() {
			...
			for(int tas = 0; tas < ntasks;tas++) {
				k.add(new int[] {getInt(),getInt(),tas});
			}
			boolean failed = false;
			char[] q = new char[ntasks];
			while(!k.isEmpty()) {
				int[] nex = k.remove();
				if(nex[0] >= lastC) {
					q[nex[2]]= 'C';
			...
			if(!failed) {
				googleSubmit(String.valueOf(q));
			}
			else {
				googleSubmit("IMPOSSIBLE");
			}
		}
		submit();
	}

...
public class Solution {
	...
	private static boolean isOverlapping(int start, int end, List<int[]> list) {
		...
		for(int i = 0; i < list.size(); i++) {
			aStart = list.get(i)[0];
			aEnd = list.get(i)[1];
			if(aStart == start && aEnd == end) return true;
			else if(aStart > start && aStart < end) return true;
			else if(aStart < start && aEnd > start) return true;
			else if(aStart > start && aEnd < end) return true;
			else if(aStart < start && aEnd > end) return true;
		}
		return false;
	}

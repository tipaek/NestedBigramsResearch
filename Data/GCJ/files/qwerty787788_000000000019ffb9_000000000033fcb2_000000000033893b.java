            ...
            for (int i = 0; i < MAX; i++) {
                if (((1 << i) & dh) != 0) {
                    ...
                    for (int start = 0; start < 2; start++) {
                        for (int to = 0; to < 2; to++) {
                            next[to + 1] = Math.min(next[to + 1], mystate[start + 1] + d[start][to][i][mystate[0]]);
                        }
                    }
                    mystate = next;
                }
...
//            System.err.println(Arrays.toString(s1)+" " + Arrays.toString(s2));
            ...
            if (s1[0] == lca) {
                ...
                int around = Math.min(d[0][0][0][s1[0]] + s1[1], d[1][0][0][s1[0]] + s1[2]) +
                             Math.min(d[0][0][0][s2[0]] + s2[1], d[1][0][0][s2[0]] + s2[2]);

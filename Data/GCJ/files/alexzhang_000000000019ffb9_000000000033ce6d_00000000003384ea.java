...
public class Solution {
	...
	public static double quadfinder(long one, long two) {
		...
		double joeseph = -1+Math.sqrt(1+4*(one-two));
	...
	public static void joestar(int kamamam) throws IOException {
		...
		long counter = Math.max((int)Math.ceil(quadfinder(L,R)), (int)Math.ceil(quadfinder(R,L)));
		if (L>=R)
		...
		if (L>=R) {
			kazuma = Math.min((int) supafinder(counter+1, L), (int) supafinder(counter+2, R)); 
			...
			kazuma = Math.min((int) supafinder(counter+1, R), (int) supafinder(counter+2, L)); 

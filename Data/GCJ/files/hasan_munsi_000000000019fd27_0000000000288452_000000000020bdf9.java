...
public class Solution {
    ...
    public static String processOutput(int[][] times){
        ...
        for(int i=0; i<times.length;i++){
            if(cAvailable(timmings,times,i)){
                timmings[i][0] = times[i][0];
                timmings[i][1] = times[i][1];
                timmings[i][2] = 1;

            }else if(cAvailable(timmings,times,i)==false && jAvailable(timmings,times,i)==true){
                timmings[i][0] = times[i][0];
                timmings[i][1] = times[i][1];
                timmings[i][2] = 2;

            }else if(cAvailable(timmings,times,i)==false && jAvailable(timmings,times,i)==false){
                return "IMPOSSIBLE";
            }
        }
    ...
    public static Boolean cAvailable(int[][] timmings, int[][] times, int loop){
        if(loop==0)
            return true;
    ...
    public static Boolean jAvailable(int[][] timmings, int[][] times, int loop){
        for(int i=0;i<loop;i++){
            if(timmings[i][2]==2){
                int min =timmings[i][0],max=timmings[i][1];
                if(times[loop][0]>=min && times[loop][0]<max)

...
public class Solution {
    ...
    static String helper(int n, int sum) {
        ...
        // System.out.println("candidate " + combineList.size());
    ...
    private static void findCandidate(List<char[]>combineList, int n, int sum, char current[], int index) {
        //System.out.println(new String(current) + " sum " + sum + " n " + n);
        if(sum < 0) return;
        if(index == n) {
            if( sum == 0) {
        ...
        int last = index == 0? 1: current[index - 1] - '0';

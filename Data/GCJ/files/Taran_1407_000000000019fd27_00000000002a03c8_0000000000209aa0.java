...
public class Solution{
    ...
    void solve(int TC) throws Exception {
        ...
        k -= n;
        for(int i = 0; i< n; i++){
            int d = Math.min(k, n-1);
            mat[i][i] += d;k -= d;
        }
        if(generateLatinSquare(mat, n, 0, 0)){
            pn("Case #"+TC+": POSSIBLE");
            for(int[] j:mat){
                for(int i:j)p(i+" ");pn("");
            }
    ...
    boolean generateLatinSquare(int[][] mat, int n, int r, int c){
        if(r == n && c == 0)return true;
        if(r == c)return generateLatinSquare(mat, n,r+(c+1)/n, (c+1)%n);
        for(int nxt = 1; nxt <= n; nxt++){
            boolean f = true;
            for(int j = 0; j< n; j++)if(mat[r][j] == nxt || mat[j][c] == nxt)f = false;
            if(f){
                mat[r][c] = nxt;
                if(generateLatinSquare(mat, n,r+(c+1)/n, (c+1)%n))return true;
                mat[r][c] = 0;
            }
        }
        return false;

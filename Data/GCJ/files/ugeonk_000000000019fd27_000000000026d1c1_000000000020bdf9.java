...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 1; i <= T; i++) {
            ...
            //C
            for (int a = 0; a < A; a++) {
                List<Activity> listWithC = activities.stream().filter(item -> item.who != null && item.who.equals("C")).collect(Collectors.toList());
                int finalA = a;
            ...
            //J
            for (int a = 0; a < A; a++) {
                if(activities.get(a).who != "C") {
                    List<Activity> listWithJWithoutC =
                            activities.stream().filter(item -> item.who != null && item.who.equals("J") && !item.who.equals("C")).collect(Collectors.toList());
                    int finalA = a;
            ...
            //activities.forEach(item -> System.out.println(item.toString()));

...
public class Solution {
    ...
    static String solveBrut(long x, long y) {
        List<String> lp = Arrays.asList("W", "N", "S", "E");
        int n = pow2(Math.abs(x) + Math.abs(y));
        List<String> ls = Stream.of(permute(lp, n+1)).flatMap(Collection::stream).collect(Collectors.toList());
        String result  = ls.stream().filter(s -> solvable(s, x, y)).map(s -> minSolvable(s, x, y)).sorted((s,v) -> Integer.compare(s.length(), v.length())).findFirst().orElse(IMPOSSIBLE);
        return result.equals(IMPOSSIBLE) ? IMPOSSIBLE : minSolvable(result, x, y);
    }

    static String minSolvable(String s, long x, long y) {
        int i = 0;
        while (solvable(s.substring(0, s.length() -i), x, y)) {
            i++;
        }
        i--;
    ...
    static boolean solvable(String s, long x, long y) {
        long step = 1;
        long xs = 0;
        long ys = 0;
        int num = 0;
    ...
    static List<String> permute(List<String> prefix, int n) {
        if (n <= 1) {
            return prefix;
        }
        List<String> suffix = permute(prefix, n-1);
        return prefix.stream().map(s -> suffix.stream().map(item -> s + item).collect(Collectors.toList())).flatMap(Collection::stream).collect(Collectors.toList());
    }

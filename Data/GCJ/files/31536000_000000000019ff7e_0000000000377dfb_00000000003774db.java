...
public class Solution {
	...
	public void solve(FastScanner fs, java.io.PrintWriter out) {
		...
		for (int c = 1;c <= T;++ c) {
			...
			TreeMap<Dist, String> ans = new TreeMap<>();
			for (int k = 1;k <= 6;++ k) {
				for (int i = 0;i < pow(3, k);++ i) {
					...
					ans.put(new Dist(dist(tmp, C), dist(tmp, J)), tmp);
				}
			}
	...
	public class Dist implements Comparable<Dist>{
		...
		public int compareTo(Dist o) {
			int comp = Integer.compare(a + b, o.a + o.b);
			if (comp != 0) return comp;
			comp = Integer.compare(Math.abs(a - b), Math.abs(o.a - o.b));
			return comp;
		}

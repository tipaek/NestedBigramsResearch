        ...
        try {
            ...
            for (int iSet = 0; iSet < numberOfSets; iSet++) {
                ...
                for (Querie querie : queries) {
                    Arrays.stream(points).forEach(point -> point.cost = -1);

                    points[querie.start].cost = 0;
                    points[querie.start].recalc();
...
class Point {
    ...
    void recalc() {
        ...
        if (Solution.points[left].cost == -1 || Solution.points[left].cost > potLeft) {
            Solution.points[left].cost = potLeft;
            Solution.points[left].recalc();
        }
        if (Solution.points[right].cost == -1 || Solution.points[right].cost > potRight) {
            Solution.points[right].cost = potRight;
            Solution.points[right].recalc();
        }
        if (Solution.points[jump].cost == -1 || Solution.points[jump].cost > potJump) {
            Solution.points[jump].cost = potJump;
            Solution.points[jump].recalc();
        }
    }
}

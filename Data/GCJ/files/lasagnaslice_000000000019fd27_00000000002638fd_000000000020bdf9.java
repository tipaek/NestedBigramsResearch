	...
	public static void main(String[] args) {
		...
		for (i = 0; i <= 1440; i++) {
			...
			for (j = 0; j < activitycount; j++) {
				if (fixedactivitytimes[j][0] < i && fixedactivitytimes[j][1] > i)
					dcount += 1;
				if (fixedactivitytimes[j][0] == i && fixedactivitytimes[j][1] == i)
					dcount += 0.5;
			}
			if (dcount > 2) {
				System.out.println(dcount + " " + i);
				imp = true;
			}
		}
		...
		for (i = 0; i < activitycount; i++) {
			for (j = i + 1; j < activitycount; j++) {
				if (activitytimes[i][0] > activitytimes[j][0]) {
				...
				if (activitytimes[i][0] == activitytimes[j][0] && activitytimes[i][1] > activitytimes[j][1]) {
					int temp = activitytimes[i][0];
					activitytimes[i][0] = activitytimes[j][0];
					activitytimes[j][0] = temp;
					temp = activitytimes[i][1];

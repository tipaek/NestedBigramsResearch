...
public class Solution {
	...
	public static String answer(int x,int y) {
		...
		if(x<0)	x = 0-x;
		if(y<0)	y = 0-y;
		boolean flag_1 = isPowerOfTwo(y+(~x)),flag_2 = isPowerOfTwo((~y)+x);
		if(!((y&x)==0 || (x<y && flag_1) || (x>y && flag_2)))	return "IMPOSSIBLE";
...
//		if(x<y && flag_1)	ans[ans.length-1] = 
		String str="";
		for(int i=0;i<ans.length;i++) {
			str+=ans[i];
		}
		if((x&y)==0)	return str;
		if(x<y && flag_1)	return (str+(temp_y>0?'N':'S'));

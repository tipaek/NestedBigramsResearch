	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < T; i++) {
			...
			for(int j = 0; j < N; j++) {
				...
				arrTask[j] = new Tache(startTime, endTime);
				
				// check if the task overlap with another one already assigned to C
				boolean overlapC = overlap(arrTask[j], ActivitiesC);
				if(overlapC) {
					// check if the task overlap with another one already assigned to J
					boolean overlapJ = overlap(arrTask[j], ActivitiesJ);
					...
					// can be assigned to J
						...
						ActivitiesJ.add(arrTask[j]);
						solution += 'J';
					}
				} else { // can be assigned to C
					ActivitiesC.add(arrTask[j]);
					solution += 'C';
				}
			}
			if(!impossible)

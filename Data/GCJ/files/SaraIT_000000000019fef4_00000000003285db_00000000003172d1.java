...
public class Solution1 {
	...
	static boolean idDevisible(String[] line) {
		HashSet<Float> set = new HashSet<Float>();

	for (int i = 0; i < line.length; i++) {
		set.add(Float.parseFloat(line[i]));
	}
		for (int i = 0; i < line.length; i++) {
			if (set.contains(Float.parseFloat(line[i]) * 2)) {
				return true;
			}
		}
		return false;
	...
	static int solution(int D, int N, TreeMap<Float, Integer> map, String[] line) {
		Float middle = 0F;
	...
	public static void main(String[] args) {
		...
		for (int i = 1; i <= T; i++) {
			...
			line =in.nextLine().split("\\s+");
			int a = solution(D, N, mapBuild(line), line);

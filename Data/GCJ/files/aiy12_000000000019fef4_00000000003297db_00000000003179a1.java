...
//								System.out.println(c + " " + q.M.charAt(idx));
	...
	//			System.out.println(confirmedMaps);
				...
				HashMap<Character,Long> digitFrequency = new HashMap<Character, Long>();
				
				for (int i = 0; i < queries.size(); i++) {
					Query q = queries.get(i);
//					for (int j = 0; j < q.R.length(); j++) {
						if (!digitFrequency.containsKey(q.R.charAt(q.R.length()-1))) {
							digitFrequency.put(q.R.charAt(q.R.length()-1), 0L);
						}
						digitFrequency.put(q.R.charAt(q.R.length()-1), digitFrequency.get(q.R.charAt(q.R.length()-1)) + 1L);
//					}
				}

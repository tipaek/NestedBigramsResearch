...
public class Solution {
    ...
    private static boolean isValid(String D) {
        ...
        for (int i = 0; i < NUMBER_OF_LINES; i++) {
            int decryptNumber = decrypt(map, R[i]);
            if (decryptNumber < 1 || (Q[i] != -1 && Q[i] < decryptNumber)) {
                return false;
            }
        }
    ...
    private static int decrypt(Map<Character, Integer> dMap, String R) {
        int result = 0;

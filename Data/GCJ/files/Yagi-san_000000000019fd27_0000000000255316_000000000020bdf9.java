...
public class Solution {
	...
	public boolean estLibre(int heureDebut, int heureFin) {
		for(int i=0; i<listBegin.size(); i++) {
			if((heureDebut>=listBegin.get(i) && heureDebut<listEnd.get(i)) || (heureFin>listBegin.get(i) && heureFin<=listEnd.get(i))) {
				return false;
			}
		}
		return true;
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<exercices.size(); i++) {
			...
			for(String activite: activites) {
				...
				if(Cameron.estLibre(heureDebut, heureFin)) {
					Cameron.setBegin(heureDebut);
					Cameron.setEnd(heureFin);
					retourExo+="C";
				} else if(Jamie.estLibre(heureDebut, heureFin)) {

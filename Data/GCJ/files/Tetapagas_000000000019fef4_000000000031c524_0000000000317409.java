    ...
    private static void solveCase(int nth) {
        ...
        while(minute <= minutes) {
            ...
            for(Position myPosition : myPositions) {
                ...
                int xdiff = targetPosition.x - myPosition.x;
                int ydiff = targetPosition.y - myPosition.y;
                if(xdiff == 0) {
                    if(ydiff > 1) nextPositions.add(new Position(myPosition.x, myPosition.y-1));
                    else if(ydiff < -1)nextPositions.add(new Position(myPosition.x, myPosition.y+1));
                    else {
                        nextPositions.add(myPosition);
                    }
                } else if(ydiff == 0) {
                    if(xdiff > 1) nextPositions.add(new Position(myPosition.x-1, myPosition.y));
                    else if(xdiff < -1)nextPositions.add(new Position(myPosition.x+1, myPosition.y));
                    else {
                        nextPositions.add(myPosition);
                    }
                } else {

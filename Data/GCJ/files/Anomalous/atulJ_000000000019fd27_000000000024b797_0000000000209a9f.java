        ...
        for (int x = 0; x < t; x++) {
            ...
            for (int j = 0; j < segments.length; j++) {
                if (segments[j].length() > 0) {
                    ...
                    for (int i = 50; i < 58; i++) {
                        char digit = (char) i;
                        if (segments[j].indexOf(digit) > -1) {
                            int leftIndex = segments[j].indexOf(digit);
                            int rightIndex = leftIndex;
                            int currentIndex = leftIndex;

                            while ((leftIndex > 0 && segments[j].charAt(leftIndex - 1) > digit &&
                                    segments[j].charAt(leftIndex - 1) >= '2' && segments[j].charAt(leftIndex - 1) <= '9') ||
                                   (rightIndex < segments[j].length() - 1 && segments[j].charAt(rightIndex + 1) > digit &&
            ...
            System.out.println("Case #" + (x + 1) + ": " + output.toString());
        }
        scanner.close();
    }
}
import java.util.Arrays;
import java.util.Scanner;

public class Solution {

    public static void main(String[] args) {
        Scanner scanner = new Scanner(System.in);
        String input = scanner.nextLine();

        int numOfCases = Integer.parseInt(input.split(" ")[0]);
        int size = Integer.parseInt(input.split(" ")[1]);
        for (int i = 0; i < numOfCases; i++) {
            TestCase testCase = new TestCase(size, scanner);
            String result = testCase.getResult();
            System.out.println(result);
            input = scanner.nextLine();
            if (!input.equals("Y")) {
                System.exit(1);
            }
        }
    }

    public static class TestCase {
        private final int size;
        private final Integer[] data;
        private final Scanner scanner;

        public TestCase(int size, Scanner scanner) {
            this.size = size;
            this.scanner = scanner;
            this.data = new Integer[size];
        }

        private void complement() {
            for (int i = 0; i < size; i++) {
                if (data[i] != null) {
                    data[i] = data[i] == 0 ? 1 : 0;
                }
            }
        }

        private void reverse() {
            for (int i = 0; i < size / 2; i++) {
                Integer temp = data[i];
                data[i] = data[size - i - 1];
                data[size - i - 1] = temp;
            }
        }

        public String getResult() {
            int guess = 1;
            int idx = 0;
            boolean flag = true;
            boolean fetching = true;
            int sameIdx = -1;
            int diffIdx = -1;

            while (guess <= 150 && Arrays.stream(data).anyMatch(x -> x == null)) {
                if (guess != 1 && guess % 10 == 1) {
                    fetching = false;
                }
                if (fetching) {
                    int ask = flag ? idx : size - idx - 1;
                    if (!flag) {
                        idx++;
                    }

                    System.out.println(ask + 1);
                    String input = scanner.nextLine();
                    if (input.equals("0") || input.equals("1")) {
                        data[ask] = Integer.parseInt(input);
                        if (!flag) {
                            if (data[idx - 1].equals(data[ask])) {
                                sameIdx = idx - 1;
                            } else {
                                diffIdx = idx - 1;
                            }
                        }
                    } else {
                        System.exit(1);
                    }
                    flag = !flag;
                    guess++;
                } else {
                    if (sameIdx != -1) {
                        System.out.println(sameIdx + 1);
                        String input = scanner.nextLine();
                        if (input.equals("0") || input.equals("1")) {
                            if (!data[sameIdx].equals(Integer.parseInt(input))) {
                                complement();
                            }
                        } else {
                            System.exit(1);
                        }
                        guess++;
                    }
                    if (diffIdx != -1) {
                        System.out.println(diffIdx + 1);
                        String input = scanner.nextLine();
                        if (input.equals("0") || input.equals("1")) {
                            if (!data[diffIdx].equals(Integer.parseInt(input))) {
                                reverse();
                            }
                        } else {
                            System.exit(1);
                        }
                        guess++;
                    }
                    fetching = true;
                }
            }

            StringBuilder result = new StringBuilder();
            for (Integer datum : data) {
                result.append(datum);
            }
            return result.toString();
        }
    }
}
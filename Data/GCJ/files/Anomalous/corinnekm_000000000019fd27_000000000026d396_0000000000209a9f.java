...
public class Solution {
    public static void main(String[] args) {
        ...
        for (int t = 0; t < T; t++) {
            ...
            System.out.println("Case #" + (t + 1) + ": " + cleanResult(result));
        }
    }
    ...
    private static String processLine(String line, Map<String, String> digitLetter) {
        ...
        for (int digit = 1; digit <= 9; digit++) {
            ...
            for (int i = length - 1; i >= 0; i--) {
                String current = generateString(i + 1, strDigit);
                result = result.replace(current, generateString(digit, "(") 
                        + generateString(i + 1, digitLetter.get(strDigit)) 

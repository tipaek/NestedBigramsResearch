...
public class Solution {
    ...
    private static void solve(InputUtil input, int t) throws IOException {
        ...
        for (String[] q : question) {
            if (!"-1".equals(q[0]) && q[0].length() == q[1].length()) {
                char key = q[1].charAt(0);
                int value = q[0].charAt(0) - '0';
        ...
        Arrays.fill(result, '\u000B'); // Fill with a non-printable character

        mapping.forEach((key, value) -> result[value] = key);

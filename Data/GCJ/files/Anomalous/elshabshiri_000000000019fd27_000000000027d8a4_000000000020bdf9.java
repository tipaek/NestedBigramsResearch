...
public class ParentingPartnering {
    ...
    private static int solve(Activity[] activities) {
        ...
        for (FreeTime freeTime : freeTimes) {
            if (freeTime.parentingBefore == freeTime.parentingAfter) {
                if (parentingTimes[freeTime.parentingBefore] < freeTime.length) {
                    exchangeCount += 2;
                }
                int timeUsed = Math.min(parentingTimes[freeTime.parentingBefore], freeTime.length);
                parentingTimes[freeTime.parentingBefore] -= timeUsed;
                parentingTimes[freeTime.parentingBefore ^ 1] -= (freeTime.length - timeUsed);
            }
        }

...
public class Solution {
    ...
    public static String solve(int n, int k) {
        ...
        switch (type) {
            case 1:
                fillMatrixType1(matrix, list, n);
                break;
            case 2:
                fillMatrixType2(matrix, list, n);
                break;
            case 3:
                fillMatrixType1(matrix, list, n);
    ...
    private static void fillMatrixType1(int[][] matrix, List<Integer> list, int n) {
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
    ...
    private static void fillMatrixType2(int[][] matrix, List<Integer> list, int n) {
        for (int i = 0; i < n - 2; i++) {
            matrix[i][i] = list.get(1);
            matrix[i][(i + 1) % (n - 2)] = list.get(0);
            int currentCol = i + 2;
            for (int cnt = 0; cnt < n - 2; cnt++) {
                while (matrix[i][currentCol % n] != 0) currentCol++;
                matrix[i][currentCol % n] = list.get(cnt + 2);
            }
        }

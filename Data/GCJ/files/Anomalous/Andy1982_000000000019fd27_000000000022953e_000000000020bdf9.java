...
public class Solution {
    ...
    private static String findSchedule(List<String> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            originalSchedule[i][0] = Integer.parseInt(arr[0]);
            originalSchedule[i][1] = Integer.parseInt(arr[1]);
        }
        ...
        for (int i = 1; i < schedule.length; i++) {
            if (schedule[i][0] >= pq.peek()) {
                int endTime = pq.poll();
                char assignedParent = map.get(endTime);
                pq.add(schedule[i][1]);
                map.put(schedule[i][1], assignedParent);
            } else {
                if (pq.size() >= 2) {
                    return "IMPOSSIBLE";
                }

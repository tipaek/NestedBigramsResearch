...
class Solution {
    ...
    public static void main(String[] args) {
        ...
        for (int id = 0; id < testCases; id++) {
            ...
            if (isTracePossible(current, k, 0, new boolean[current.length], 1, positions)) {
    ...
    static boolean isTracePossible(int[][] matrix, int k, int rowIndex, boolean[] columnVisited, int prevSelected, int[] positions) {
        int remainingSelections = matrix.length - rowIndex;
        if (remainingSelections == 0) {
            return k == 0;
        }
        for (int j = 0; j < matrix.length; j++) {
            int digit = matrix[rowIndex][j];
            if (!columnVisited[j] && digit >= prevSelected && digit <= k / remainingSelections) {
                columnVisited[j] = true;
                positions[rowIndex] = j;
                if (isTracePossible(matrix, k - digit, rowIndex + 1, columnVisited, digit, positions)) {
                    return true;
                }
    ...
    static int[][] getNaturalLatinSquare(int n) {
        if (naturalLatinSquares.size() < n) {
            int[][] prev = getNaturalLatinSquare(n - 1);
            naturalLatinSquares.add(computeCurrent(prev));
        }

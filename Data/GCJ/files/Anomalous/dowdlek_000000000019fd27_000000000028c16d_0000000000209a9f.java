...
public class Solution {
        ...
        for (int cases = 0; cases < n; cases++) {
            ...
            while (linked.size > 1) {
                if (current.prev == null) { // At head
                    processNode(current, linked, true);
                } else if (current.next == null) { // At tail
                    processNode(current, linked, false);
                } else { // Middle node
                    ...
                    if (current.val > current.prev.val && current.val > current.next.val) {
    ...
    private static void processNode(Node current, NDLinked linked, boolean isHead) {
        if (isHead) {
            while (current.next.val < current.val) {
                current.reduce();
            }
    ...
    public static class NDLinked {
        ...
        public void combineAfter(Node current) {
            size--;
        ...
        public void combineBefore(Node current) {
            size--;
            String tempRep = current.prev.rep;
            Node tempNode = current.prev;
            boolean isHead = current.prev.isHead;

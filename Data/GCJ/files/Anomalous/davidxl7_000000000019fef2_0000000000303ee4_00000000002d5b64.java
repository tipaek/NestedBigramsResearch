...
public class Solution {
    ...
    public static void solve(int R, int S) {
        Pair[] initialPairs = new Pair[R * S];
        int index = 0;
        
        for (int i = 0; i < S; i++) {
            for (int j = 0; j < R; j++) {
                initialPairs[index++] = new Pair(j, i);
            }
        }
        
        int size = R * S;
        LinkedList<State> queue = new LinkedList<>();
        queue.add(new State(R, S, new ArrayList<>(), initialPairs));
        ...
        while (!queue.isEmpty()) {
            ...
            for (int a = 1; a < size; a++) {
                for (int b = 1; a + b < size; b++) {
                    Pair[] newSituation = swap(a, b, currentState.situation);
                    ArrayList<Pair> newPath = new ArrayList<>(currentState.path);
                    newPath.add(new Pair(a, b));
                    queue.add(new State(R, S, newPath, newSituation));
                }
            }
        }
    }
    ...
    static class State {
        ...
        public State(int rows, int cols, ArrayList<Pair> path, Pair[] situation) {
            this.rows = rows;
            this.cols = cols;
            this.path = path;

...
public class Solution {
    ...
    private static String getResult(Guess[] guesses) {
        ...
        for (int i = 9; i >= 1; i--) {
            ...
            for (Character c : option) {
                ...
                for (int j = i - 1; j >= 1; j--) options.get(j).remove(c);
            }
        }

        if (guessMode) {
            char lowestChar = Collections.min(distribution.entrySet(), Map.Entry.comparingByValue()).getKey();
            result.append(lowestChar);
            distribution.remove(lowestChar);

            while (!distribution.isEmpty()) {
                char highest = Collections.max(distribution.entrySet(), Map.Entry.comparingByValue()).getKey();
    ...
    private static void evaluateFullLengthGuess(Guess guess, Set<Character> allCharacters, Set<Character> allNonZeroCharacters, Map<Character, Integer> maxNum, List<Set<Character>> options) {
        ...
        for (char c : guess.characters) {
            if (c != guess.characters[0]) allNonZeroCharacters.add(c);
            allCharacters.add(c);
        }
    }
}
...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int t = 0; t < testCases; t++) {
            ...
            out.println("Case #" + (t + 1) + ": " + findPattern());
        }
    ...
    static boolean validatePatterns(String[] patterns, int count, StringBuilder output, boolean isLeft) {
        for (int i = 0; i < count; i++) {
            for (int j = 0; j < patterns[i].length() - 1; j++) {
                if (isLeft) {
                    if (output.charAt(j) != patterns[i].charAt(j)) return false;
                } else {

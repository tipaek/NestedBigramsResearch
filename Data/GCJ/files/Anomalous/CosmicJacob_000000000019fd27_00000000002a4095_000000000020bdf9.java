...
public class Solution {
    ...
    private static String schedule(int eventsCount, int[] schedule1, int[] schedule2, int[][] events, char p1, char p2) {
        ...
        if (isAvailable(events[eventsCount][0], events[eventsCount][1], schedule1)) {
            fillSchedule(events[eventsCount][0], events[eventsCount][1], schedule1);
            return schedule(eventsCount - 1, schedule1, schedule2, events, p1, p2) + p1;
        } else if (isAvailable(events[eventsCount][0], events[eventsCount][1], schedule2)) {
            fillSchedule(events[eventsCount][0], events[eventsCount][1], schedule2);
    ...
    private static void fillSchedule(int start, int end, int[] schedule) {
        for (int i = start; i < end; i++) {
            schedule[i] = 1;
        }
    }

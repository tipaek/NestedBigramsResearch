...
public class Solution {
    ...
    static int solve(int idx, int open) {
        ...
        int ans = INF;
        if (s[idx] == '#') {
            ans = Math.min(ans, 1 + solve(idx + 1, open + 1)); // open
            ans = Math.min(ans, 1 + solve(idx + 1, open - 1)); // close
            ans = Math.min(ans, solve(idx + 1, open)); // leave
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int c = 1; c <= TC; c++) {
            ...
            s = new char[T.length * 10 + 9];

            for (int i = 0; i < s.length; i++) {
                if ((i + 1) % 10 != 0) {
                    s[i] = '#';
                } else if (i / 10 < T.length) {
                    s[i] = T[i / 10];
                }
            }

...
class Solver {
    ...
    private static final int LENGTH = (int) Math.pow(10, 9);
    ...
    public void solve() {
        ...
        int leftBound = findXBound(hit[0], -LENGTH, hit[1]);
        if (hitCenter) return;
        
        int rightBound = findXBound(hit[0], LENGTH, hit[1]);
        if (hitCenter) return;
        
        int middleX = (leftBound + rightBound) / 2;
        
        int topBound = findYBound(hit[1], LENGTH, middleX);
        if (hitCenter) return;
        
        int bottomBound = findYBound(hit[1], -LENGTH, middleX);
        if (hitCenter) return;

...
public class Solution {
    ...
    public static void main(String... args) {
        ...
        for (int testCase = 0; testCase < testCases; ++testCase) {
            ...
            final int BORDER = 1_000_000_000;
            final int MAX_STEPS = 11;
            ...
            while (generateNextSet(currentSet, directions.length - 1, MAX_STEPS)) {
                ...
                for (int i = 0; i < MAX_STEPS; i++) {
                    ...
                    if (sumX == targetX && sumY == targetY) {
                        if ((i + 1) < bestStepCount) {
                            bestStepCount = i + 1;
                            System.arraycopy(currentResult, 0, result, 0, bestStepCount);
                            isImpossible = false;
                        }
                    }
                    step <<= 1;
            ...
            if (isImpossible) {
                ...
                System.out.printf("Case #%d: %s%n", testCase + 1, new String(result, 0, bestStepCount));
            }
        }
    }

    public static boolean generateNextSet(int[] array, int maxValue, int size) {
        int index = size - 1;

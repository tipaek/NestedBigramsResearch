...
public class Solution {
    ...
    public static void solve(Scanner input, int B) {
        ...
        while (read < B) {
            ...
            for (int i = 0; i < limit && read < B && query <= 150; i++) {
                ...
                if (leftMarker == -1 && foundSame == (arr[left + i] != arr[right - i])) {
                    leftMarker = left + i;
                    rightMarker = right - i;
                }
                read += 2;
    ...
    private static void flip(char[] arr, int count, int right) {
        for (int i = 0; i < count; i++) {
            ...
            arr[right - i] = flipChar(arr[right - i]);
        }
    }
    ...
    private static Action getAction(Scanner input, int leftMarker, int rightMarker, int[] prev, boolean foundSame, int B) {
        int[] now = new int[4];
        now[0] = read(input, 0);
        ...
        if (leftMarker != -1) {
            ...
            return compare(prev, now);
        } else {
            if (foundSame) {
                return (prev[0] == now[0] && prev[3] == now[3]) ? Action.REVERSE : Action.FLIP;
            } else {
                return (prev[0] == now[0] && prev[3] == now[3]) ? Action.BOTH : Action.REVERSE;
            }
        }
    }
    ...
    private static Action compare(int[] prev, int[] now) {
        ...
        } else if ((prev[0] + 1) % 2 == now[0] && (prev[1] + 1) % 2 == now[1] && (prev[2] + 1) % 2 == now[2] && (prev[3] + 1) % 2 == now[3]) {
            return Action.FLIP;
        } else {
            return Action.BOTH;
        }

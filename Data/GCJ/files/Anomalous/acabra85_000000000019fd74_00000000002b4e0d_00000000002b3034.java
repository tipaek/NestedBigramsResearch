...
public class Solution {
    ...
    private String findSolution(String[] patterns, int starIndex) {
        if (starIndex == 0) {
            if (patterns.length == 1) {
                ...
                String combinedPattern = patterns[0].substring(1);
                for (int i = 1; i < patterns.length; i++) {
                    combinedPattern = mergePatterns(combinedPattern, patterns[i].substring(1));
    ...
    private String mergePatterns(String pattern1, String pattern2) {
        if (pattern1.contains(pattern2) || pattern2.contains(pattern1)) {
            return pattern1.length() > pattern2.length() ? pattern1 : pattern2;
        }
        return null;
    }
}
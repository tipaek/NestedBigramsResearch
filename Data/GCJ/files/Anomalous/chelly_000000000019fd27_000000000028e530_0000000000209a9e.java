import java.util.*;
import java.io.*;

public class Solution {

    static int query(int n) throws IOException {
        if (n < 1) return -1;
        System.out.println(n);
        System.out.flush();
        return readInt();
    }

    static void solve(int t) throws IOException {
        int N = readInt();
        boolean[] sameArray = new boolean[N + 1];
        boolean[] comparisonArray = new boolean[N + 1];

        int initialQuery = query(1);
        List<Integer> sameIndices = new ArrayList<>();
        List<Integer> diffIndices = new ArrayList<>();

        for (int i = 1; i <= N / 2; i++) {
            int a = query(i);
            int b = query(N + 1 - i);
            if (a == b) {
                sameArray[i] = true;
                sameIndices.add(i);
            } else {
                sameArray[i] = false;
                diffIndices.add(i);
            }
        }

        int sameCmpIdx = -1;
        int diffCmpIdx = -1;

        if (!sameIndices.isEmpty()) {
            sameCmpIdx = sameIndices.get(0);
            comparisonArray[sameCmpIdx] = true;
            sameIndices.remove(0);
        }

        if (!diffIndices.isEmpty()) {
            diffCmpIdx = diffIndices.get(0);
            comparisonArray[diffCmpIdx] = true;
            diffIndices.remove(0);
        }

        int queryCount = 1;
        int sameCmpVal = query(sameCmpIdx);
        queryCount++;

        while (!sameIndices.isEmpty()) {
            if (queryCount % 10 == 1) {
                sameCmpVal = query(sameCmpIdx);
                queryCount++;
                continue;
            }
            comparisonArray[sameIndices.get(0)] = (sameCmpVal == query(sameIndices.get(0)));
            queryCount++;
            sameIndices.remove(0);
        }

        int diffCmpVal = query(diffCmpIdx);
        queryCount++;

        while (!diffIndices.isEmpty()) {
            if (queryCount % 10 == 1) {
                diffCmpVal = query(diffCmpIdx);
                queryCount++;
                continue;
            }
            comparisonArray[diffIndices.get(0)] = (diffCmpVal == query(diffIndices.get(0)));
            queryCount++;
            diffIndices.remove(0);
        }

        if (queryCount % 10 == 0) query(0);

        int[] result = new int[N + 1];
        sameCmpVal = query(sameCmpIdx);
        diffCmpVal = query(diffCmpIdx);

        for (int i = 1; i <= N / 2; i++) {
            if (sameArray[i]) {
                result[i] = comparisonArray[i] ? sameCmpVal : 1 - sameCmpVal;
                result[N + 1 - i] = result[i];
            } else {
                result[i] = comparisonArray[i] ? diffCmpVal : 1 - diffCmpVal;
                result[N + 1 - i] = 1 - result[i];
            }
        }

        for (int i = 1; i <= N; i++) {
            System.out.print(result[i]);
        }
        System.out.println();

        char response = readChar();
        if (response == 'N') System.exit(0);
    }

    public static void main(String[] args) throws IOException {
        int T = readInt();
        for (int t = 1; t <= T; t++) {
            solve(t);
        }
    }

    static BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
    static StringTokenizer st;

    static String next() throws IOException {
        while (st == null || !st.hasMoreTokens()) {
            st = new StringTokenizer(br.readLine().trim());
        }
        return st.nextToken();
    }

    static int readInt() throws IOException {
        return Integer.parseInt(next());
    }

    static char readChar() throws IOException {
        return next().charAt(0);
    }
}
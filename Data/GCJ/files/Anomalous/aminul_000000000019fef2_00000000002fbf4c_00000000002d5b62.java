...
public class Solution {
    ...
    static String findPath(long targetX, long targetY) {
        if ((abs(targetX) + abs(targetY)) % 2 == 0) return "IMPOSSIBLE";
        ...
        while (!queue.isEmpty()) {
            ...
            long step = current.path.length();
            for (int i = 0; i < 4; i++) {
                long newX = current.x + dx[i] * (1L << step);
                long newY = current.y + dy[i] * (1L << step);
                Pair nextPair = new Pair(newX, newY, current.path + directions[i]);
                if (!visited.add(nextPair)) continue;
                queue.add(nextPair);
            }
        }
        return "IMPOSSIBLE";

...
public class Solution {
    ...
    public static boolean isImpossible(long xDiff, long yDiff, long nextNextStepSize, long nextStepSize) {
        ...
        if (nextStepSize != 0) {
            final boolean canFinish = xDiff == nextStepSize || yDiff == nextStepSize;
            if (canFinish) {
                return false;
            }
        }

        final boolean bothMustStep = xDiff % nextNextStepSize > 0 && yDiff % nextNextStepSize > 0;
        final boolean neitherCanStep = xDiff % nextNextStepSize == 0 && yDiff % nextNextStepSize == 0;

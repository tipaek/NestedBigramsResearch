...
//                "8 11");
        ...
        for (int t = 1; t <= T; t++) {
            ...
            long r = in.nextLong();
            long d = Math.abs(l - r);
            int steps = maximize(x -> progrSum(1, x + 1, x), d);

            if (r > l) {
                r -= progrSum(1, steps + 1, steps);
            } else {
                l -= progrSum(1, steps + 1, steps);
            }
            ...
            if (r > l) {
                r -= progrSum(steps2 + 1, steps2 + (remSteps - 1), remSteps);
            } else {
                l -= progrSum(steps2 + 1, steps2 + (remSteps - 1), remSteps);
            }

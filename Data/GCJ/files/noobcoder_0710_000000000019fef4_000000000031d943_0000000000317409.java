    ...
    public static void main(String args[])throws Exception
    {
        ...
        for(int p=0;p<q;p++)
        {
            ...
            while(!queue.isEmpty())
            {
                ...
                if(x1+1<=maxx&&!map2.containsKey((x1+1)+" "+y1))
                {
                queue.add((x1+1)+" "+y1);
                map2.put((x1+1)+" "+y1,map2.get(x1+" "+y1)+1);
                if(map.containsKey((x1+1)+" "+y1)&&map.get((x1+1)+" "+y1)>=map2.get((x1+1)+" "+y1))
                min=Math.min(min,Math.max(map.get((x1+1)+" "+y1),map2.get((x1+1)+" "+y1)));
                }
                if(x1-1>=minx&&!map2.containsKey((x1-1)+" "+y1))
                {
                queue.add((x1-1)+" "+y1);
                map2.put((x1-1)+" "+y1,map2.get(x1+" "+y1)+1);
                if(map.containsKey((x1-1)+" "+y1)&&map.get((x1-1)+" "+y1)>=map2.get((x1-1)+" "+y1))
                ...
                if(y1+1<=maxy&&!map2.containsKey(x1+" "+(y1+1)))
                {
                queue.add(x1+" "+(y1+1));
                map2.put(x1+" "+(y1+1),map2.get(x1+" "+y1)+1);
                if(map.containsKey(x1+" "+(y1+1))&&map.get(x1+" "+(y1+1))>=map2.get(x1+" "+(y1+1)))
                min=Math.min(min,Math.max(map.get(x1+" "+(y1+1)),map2.get(x1+" "+(y1+1))));
                }
                if(y1-1>=miny&&!map2.containsKey(x1+" "+(y1-1)))
                {
                queue.add(x1+" "+(y1-1));
                map2.put(x1+" "+(y1-1),map2.get(x1+" "+y1)+1);
                if(map.containsKey(x1+" "+(y1-1))&&map.get(x1+" "+(y1-1))>=map2.get(x1+" "+(y1-1)))

...
public class Solution {
    ...
    static void solve(BufferedReader reader, int caseNum) throws Exception {
        ...
        for (int i = 0; i < R; i++) {
            for (int j = 0; j < C; j++) {
                ...
                if (i != nodes.length - 1) nodes[i][j].bottom = nodes[i + 1][j];
                if (j != 0) nodes[i][j].left = nodes[i][j - 1];
                if (j != nodes[i].length - 1) nodes[i][j].right = nodes[i][j + 1];
            }
        }

        TreeSet<Node> Q = new TreeSet<>((Node a, Node b) -> {
        ...
        for (int i = 0; i < R; i++) {
            for (int j = 0; j < C; j++) {
                // System.out.print(nodes[i][j].skillDiff() + " ");
                Q.add(nodes[i][j]);
            }
            // System.out.println();
        }
    ...
    static class Node {
        ...
        int skillDiff() {
            int result = 0;
            if (top != null) result += skill - top.skill;
            if (bottom != null) result += skill - bottom.skill;
            if (left != null) result += skill - left.skill;
            if (right != null) result += skill - right.skill;
            return result;
        }
    }

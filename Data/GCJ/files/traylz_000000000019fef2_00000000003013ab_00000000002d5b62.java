...
public class Solution {
    ...
    public static String solve(long x, long y) {
        BigInteger sum = BigInteger.valueOf(x).add(BigInteger.valueOf(y)).abs();
        if (!sum.remainder(TWO).equals(ONE)) {
            return IMPOSSIBURU;
        } else if (Math.abs(x % 2) == 1 && Math.abs(y % 2) == 1) {
            return IMPOSSIBURU;
        }
        for (int numOfMoves = 0; numOfMoves < 1024; numOfMoves++) {
    ...
    private static int[] gatherFrom(int[] signs, long x) {
        BigInteger restToGather = BigInteger.valueOf(x);
        final int[] positiveOutcome = tryGather(signs, restToGather);
        if (positiveOutcome.length == 0) {
            return tryGather(signs, restToGather.negate());
        }

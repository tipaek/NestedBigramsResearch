...
public class Solution {
    ...
    public static void main(String[] args) {
        ...
        for (int testNr = 1; testNr <= t; testNr++) {
            ...
            String solution = magic(x, y);
            if (solution.equals("IMPOSSIBLE")) {
                String reverseSolution = magic(y, x);
                if (!reverseSolution.equals("IMPOSSIBLE")) {
                    char[] arr = reverseSolution.toCharArray();
                    for (int i = 0; i < arr.length; i++) {
                        if (arr[i] == 'N') arr[i] = 'E';
                        else if (arr[i] == 'E') arr[i] = 'N';
                        else if (arr[i] == 'W') arr[i] = 'S';
                        else if (arr[i] == 'S') arr[i] = 'W';
                    }
                    solution = String.valueOf(arr);
                }
            }

            System.out.println("Case #" + testNr + ": " + solution);
        }
    }
    ...
    private static String magic(int x, int y) {
...
//            System.out.println(stepsY);
        ...
        if (stepsX.size() != stepsY.size() && !isImpossible) {
            for (int i = nIter; i < restOfSteps.size(); i++) {
                if (Math.abs(restOfSteps.get(i)) == 0) {
                    sb = new StringBuilder("IMPOSSIBLE");
                    break;
                }

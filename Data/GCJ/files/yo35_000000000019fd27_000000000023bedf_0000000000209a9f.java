	...
	private static String solveTestCase(Scanner sc) {
		...
		for (int i = 0; i < input.length(); ++i) {
			...
			if (currentDepth < targetDepth) {
				Stream.generate(() -> '(').limit(targetDepth - currentDepth).forEach(sb::append);
			}
			else if (currentDepth > targetDepth) {
				Stream.generate(() -> ')').limit(currentDepth - targetDepth).forEach(sb::append);
			}
		...
		if (currentDepth > 0) {
			Stream.generate(() -> ')').limit(currentDepth).forEach(sb::append);
		}

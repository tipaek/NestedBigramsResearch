        ...
        try {
            ...
            for (long i = 0; i < T; i++) {
                ...
                for (int j = 1; j < n; j++) {
                    ...
                    for (int k = 0; k < j; k++) {
                        if ((Cameron.get(k).start < stime && stime < Cameron.get(k).end) || (Cameron.get(k).start < etime && etime < Cameron.get(k).end) || (Cameron.get(k).start == stime && etime == Cameron.get(k).end)) {
                            for (int l = 0; l < j; l++) {
                                if (Jamie.size() == 0) {
                                    Jamie.add(activities.get(j));
                                    answer = answer + "J";
                                    break;
                                } else if ((Jamie.get(l).start < stime && stime < Jamie.get(l).end) || (Jamie.get(l).start < etime && etime < Jamie.get(l).end) || (Jamie.get(l).start == stime && etime == Jamie.get(l).end)) {

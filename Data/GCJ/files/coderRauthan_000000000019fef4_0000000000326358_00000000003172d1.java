			...
			n:for(int i=1; i<= testCases; i++)
			{
				...
				for(int j=0; j<size.length; j++)
				{
					if(map.containsKey(size[j]))
					{
						...
						{
							//System.out.println("biggerPresent got incremented for: "+map.get(size[j]));
							biggerPresent++;
						}
					}
					else
				...
				//System.out.println("biggerPresent is: "+biggerPresent);
				if(biggerPresent > 0)
				{
				...
				while(true)
				{
					for(int j=0; j< size.length; j++)
					{
						...
						{
							fullSize = map.getOrDefault(share, 0) + cut;
						}

...
public class Solution {
	...
	static String solve10(Scanner in) {
		...
		if (sameExists && diffExists) {
			/*
			pairs[indexForSame] = getPair(in, 10, indexForSame);
			pairs[indexForDiff] = getPair(in, 10, indexForDiff);
			// waste 3 times
			getPair(in, 10, indexForSame);
			getPair(in, 10, indexForSame);
			getPair(in, 10, indexForSame);
			
			update(in, 10, pairs, indexForSame, indexForDiff);
			int updateCounter = 0;
			for (int index = 0; index < 5; index++) {
				pairs[index] = getPair(in, 10, index);
	...
	static void update(Scanner in, int size, Pair[] pairs, int indexForSame, int indexForDiff) {

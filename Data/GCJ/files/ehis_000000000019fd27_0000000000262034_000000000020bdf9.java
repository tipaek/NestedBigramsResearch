        ...
        for (int i = 1; i < N; i++) {
            ...
            if (parentActuel.equals("C")) 
            {
                for (int j = 0; j < tachesCDebut.size(); j++) {
                    if ( (debut[i] < tachesCFin.get(j) && debut[i] >= tachesCDebut.get(j)) 
                            || ( fin[i] > tachesCDebut.get(j) && fin[i] <= tachesCFin.get(j))
                            || (debut[i] <= tachesCDebut.get(j) && fin[i] >=  tachesCFin.get(j)) 
                            || (debut[i] >= tachesCDebut.get(j) && fin[i] <=  tachesCFin.get(j) )) {
                        nbrChangement++;
                        parentActuel="J";
                        i--;
            ...
            else
            {
                for (int j = 0; j < tachesJDebut.size(); j++) {
                    if ( (debut[i] < tachesJFin.get(j) && debut[i] >= tachesJDebut.get(j))
                            || ( fin[i] > tachesJDebut.get(j) && fin[i] <= tachesJFin.get(j) )
                            || (debut[i] <= tachesJDebut.get(j) && fin[i] >=  tachesJFin.get(j) )
                            || (debut[i] >= tachesJDebut.get(j) && fin[i] <=  tachesJFin.get(j) )) {

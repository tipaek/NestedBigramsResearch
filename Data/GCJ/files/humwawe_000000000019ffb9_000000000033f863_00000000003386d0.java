        ...
        for (int cas = 1; cas <= n; cas++) {
            ...
            for (int i = 0; i < list.size(); i++) {
                for (int j = i + 1; j < list.size(); j++) {
                    ...
                    BigDecimal fenziBigDecimal = new BigDecimal(list.get(j)[0] - list.get(i)[0]);
                    BigDecimal fenmuBigDecimal = new BigDecimal(list.get(j)[1] - list.get(i)[1]);
                    double v = fenziBigDecimal.divide(fenmuBigDecimal, 20, BigDecimal.ROUND_HALF_DOWN).doubleValue();

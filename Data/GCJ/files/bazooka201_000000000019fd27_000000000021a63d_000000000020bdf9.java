        ...
        for(int run = 1; run <= runs; run++){
            ...
            for(int start = 0; start<starts.length; start++){
                ...
                for(String time: CTimes){
                    if((Integer.parseInt(time.substring(0, time.indexOf(" "))) <= starts[start] && Integer.parseInt(time.substring(time.indexOf(" ")+1)) > starts[start]) || (Integer.parseInt(time.substring(0, time.indexOf(" "))) < ends[start] && Integer.parseInt(time.substring(time.indexOf(" ")+1)) >= ends[start]) || (Integer.parseInt(time.substring(0, time.indexOf(" "))) <= starts[start] && Integer.parseInt(time.substring(time.indexOf(" ")+1)) >= ends[start])){
                        useC = false;
                    }
                }

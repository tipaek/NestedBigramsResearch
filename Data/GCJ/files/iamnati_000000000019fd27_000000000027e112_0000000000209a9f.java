	...
	private static String sPrime(String digits, ProblemInstance currentProblem, Map<ProblemInstance, String> memo) {
		...
		if (currentProblem.nestingDepth[minIndex] == 0) {
			...
			if (minIndex == currentProblem.i) {
				...
				String solution = digits.charAt(minIndex) +  sPrime(digits, currentProblem, memo); // (memo.containsKey(currentProblem) ? memo.get(currentProblem) :
				...
				String solution =  sPrime(digits, currentProblem, memo) + digits.charAt(minIndex); //memo.containsKey(currentProblem) ? memo.get(currentProblem) :
				...
				currentProblem.j = minIndex - 1;
				String firstsolution =  sPrime(digits, currentProblem, memo); //memo.containsKey(currentProblem) ? memo.get(currentProblem) :
				currentProblem.j = oldJ;

				// from min + 1 ... j
				int oldI = currentProblem.i;
				currentProblem.i = minIndex + 1;
				String secondSolution =  sPrime(digits, currentProblem, memo); //memo.containsKey(currentProblem) ? memo.get(currentProblem) :
			...
			String secondSolution =  "(" + secondSolutionPriorToDecoration + ")"; //memo.containsKey(currentProblem) ? memo.get(currentProblem) :

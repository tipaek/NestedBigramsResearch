	...
	public static boolean valid(String in, ArrayList<String> L){
		...
		for (int i =0; i<L.size(); i++ ){
			...
			if( (startnew == startold && endnew==endold)   
				...
				|| (startnew > startold && endnew <endold) 
				|| (startnew < startold && endnew < endold && endnew>startold) 
				|| (startnew > startold && startnew < endold && endnew>endold)
				|| (startnew == startold && (endnew < endold || endnew>endold))
				|| ((startnew < startold || startnew > startold) && endnew==endold )){
				out = false;
				break;
			}else {
				out = true;

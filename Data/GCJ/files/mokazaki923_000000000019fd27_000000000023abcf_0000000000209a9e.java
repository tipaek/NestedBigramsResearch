...
public class Solution {
	...
	static Pair[] solve100(Scanner in) {
		Pair[] pairs = new Pair[50];
		
		boolean isComplemented = true;
		boolean isReversed = false;
		boolean[] originalValues = new boolean[100];
		...
		for (int index = 0; index < 50; index++) {
			pairs[index] = getPair(in, 100, index);
			...
			Pair newSamePair = getPair(in, 100, indexForSame);
			Pair newDiffPair = getPair(in, 100, indexForDiff);
//			boolean isComplemented = isComplemented(samePair, diffPair, newSamePair, newDiffPair);
	//		boolean isReversed = isReversed(samePair, diffPair, newSamePair, newDiffPair);
		...
		for (int index = 0; index < 50; index++) {
			boolean isSame = pairs[index].isSame();
			boolean originalValue = originalValues[index];
			boolean value = originalValue;
			if (isComplemented)
				value = !value;
			if (isReversed && !isSame)
				value = !value;

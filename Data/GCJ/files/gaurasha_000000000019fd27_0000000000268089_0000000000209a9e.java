...
//            System.err.println("start : li : "+li+" ei: "+ei);
...
//                System.err.println("debug : li : "+li+" ei: "+ei);
//                System.err.println("debug : "+bits.stream().map(a->a+"").reduce((a,x)->a+""+x).orElse(""));
//                for(int ii=0;ii<bits.size();++ii){
//                    System.err.print("("+ii+":"+bits.get(ii)+") ");
//                }
//                System.err.println("");
            }
...
//            System.err.println("-ri: "+ri);
    ...
    public static Pair<CHANGE_TYPE,Integer> getChangeType(List<Integer> bits){
        ...
        for(int i=0;i<bits.size();++i){
            if(bits.get(i)!=-1){
                if(sidx==-1 && bits.get(i)==bits.get(bits.size()-1-i)){
                    sidx = i;
                    sflag = 1;
                }
                if(didx==-1 && bits.get(i)!=bits.get(bits.size()-1-i)){
                    didx = i;
                    dflag = 1;
                }
            }
...
//        System.err.println("sidx: "+sidx+" didx: "+didx);
//        System.err.println(bits.stream().map(a->a+"").reduce((a,x)->a+""+x).orElse(""));

        if(sidx ==-1 && sidx==didx){
            ol("Should not be the case");
        }else if(sidx == -1 ){
            int x = check(didx),y= check(didx);
            if(x==bits.get(didx)){

...
public class Solution {
	...
	void addOption(List<List<Task>> allTasks, List<Task> current, int idx)
	{
		...
		for (int i = 0; i < current.size(); i++)
		{
			...
			if (tasks1 != null)
			{
				if (!possible(tasks1, tasks1.get(tasks1.size() - 1), tasks1.get(tasks1.size() - 1).person))
				{
					tasks1 = null;
				}
			}
			if (tasks2 != null)
			{
				if (!possible(tasks2, tasks2.get(tasks2.size() - 1), tasks2.get(tasks2.size() - 1).person))
				{
					tasks2 = null;
				}
			}

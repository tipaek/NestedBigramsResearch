    ...
    for(int e=0;e<count;e++) {
        ...
        for(int i=0;i<activitiesCount;i++) {
            ...
            if(activ.startTime >= cameron.finishingTime) {
                activ.executor = cameron.ID; cameron.finishingTime = activ.endTime;
            } else if(activ.startTime >= jamie.finishingTime) {
                activ.executor = jamie.ID; jamie.finishingTime = activ.endTime;
            } else {
        ...
        Collections.sort(activities, Activity.originalComparator);
        for(int i=0;i<activitiesCount;i++) {
            Activity activ = activities.get(i);
            finalResult += activ.executor;
        }
        System.out.println("Case #" + caseID + ": " + finalResult);
        caseID += 1;
    }

...
public class Solution {
    ...
    private static boolean search(int[][] ltm, boolean[][] colMap, boolean[] rowMap, int n, int i, int j) {
        ...
        for(int m = 1; m <= n; m++) {
            if(!(colMap[j][m] || rowMap[m])) {
                colMap[j][m] = rowMap[m] = true;
                if(search(ltm, colMap, rowMap, n, i, j+1)) { ltm[i][j] = m; break; }
                else colMap[j][m] = rowMap[m] = false;
            }
        }
        return ltm[i][j] > 0;
    }
    ...
    public static void main(String args[]) {
        try {
            ...
            for (int t = 1; t<=testCases; t++) {
                ...
                for(int i =0 ; i < n; i++) {
                    ltm[i][i] = i==n-1 ? div + rem : div;
                    colMap[i][ltm[i][i]] = true;
                }

                for(int i = 0; i < n && isPossible; i++) {
                    boolean[] rowMap = new boolean[n+1];
                    rowMap[ltm[i][i]] = true;
                    if(!search(ltm, colMap, rowMap, n, i, 0)) {

...
public class Solution {
    ...
    private void run() {
        ...
        for (int test = 0; test < tests; test++) {
            String defaultMax = String.valueOf((long) Math.pow(10d, in.nextDouble()) - 1);
            in.nextLine();
            ...
            for (double i = 0; i < Math.pow(10, 4); i++) {
                ...
                if ("-1".equals(localMax)) {
                    localMax = String.valueOf((long) Math.pow(10d, record.length()) - 1);
                }
            ...
            for (; number < 10; number++) {
                for (Guess record : records) {
                    ...
                    int asd = 0;
                    for (Character car : record.max.toCharArray()) {
                        if (record.max.charAt(asd) - '0' != number || key.contains(record.answer.charAt(asd))) {
                            if(record.max.chatAt(asd) != '1'){
                                break;
                            }
                            asd++;
                        } else {
                            number++;
                            key.add(record.answer.charAt(asd));
                            break;
                        }
                    }

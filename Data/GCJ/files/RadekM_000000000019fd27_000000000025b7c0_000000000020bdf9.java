...
public class Solution {
    ...
    private static Output solve(Input input) {
        ...
        Activity firstPersonActivity = null;
        Activity secondPersonActivity = null;
        ...
        for (int i = 0; i < activitiesCount; i++) {
            ...
            if (firstPersonActivity != null && secondPersonActivity != null) {
                return new Output(testCase, IMPOSSIBLE);
            } else if (firstPersonActivity == null) {
                order[activity.getActivityNo()] = FIRST_INITIAL;
                firstPersonActivity = activity;
            } else {
                order[activity.getActivityNo()] = SECOND_INITIAL;
                secondPersonActivity = activity;
            }
        }
    ...
    private static Activity updateIfFinished(Activity assignedActivity, int start) {
        if (assignedActivity == null) {
            return null;
        } else if (assignedActivity.getEnd() <= start) {
            return null;
        } else {

...
class Solution{ //Copy and use this
    ...
    private StringBuilder lastPart, firstPart, midPart;
    ...
    private void takeIntoAccount(String pattern){
        if(flag){
            ...
            //mid part
            if(endIndex >  startIndex && endIndex >= 0 && startIndex < pattern.length()){
                for(index = startIndex; index< endIndex; index++){
                    if(pattern.charAt(index)!='*') midPart.append(pattern.charAt(index));
                }
            }
        }
    }
    ...
    private String solve(){
        ...
        String ans = "";
        ans = firstPart.append(midPart).append(lastPart).toString();

        ...
        for(int run = 1; run <= runs; run++){
            ...
            for(int start = 0; start<starts.length; start++){
                ...
                for(String time: CTimes){
                    ...
                    if(  (starts[start] != ends[start] &&   ((testStart<=starts[start] && testEnd>starts[start])    ||       (testStart<ends[start] && testEnd>=ends[start]) ) )   ||     (testStart<starts[start] && testEnd>ends[start]) ){                     //Write conditionals
                        useC = false;
                    }
                }
                for(String time: JTimes){
                    ...
                    int testEnd = Integer.parseInt(time.substring(time.indexOf(" ") +1));
                    if( (starts[start] != ends[start] &&  ((testStart<=starts[start] && testEnd>starts[start])    ||       (testStart<ends[start] && testEnd>=ends[start]) ))    ||     (testStart<starts[start] && testEnd>ends[start]) ){                       //Write Conditionals
                        System.out.println(starts[start] != ends[start]);
                        useJ = false;
                    }
                }

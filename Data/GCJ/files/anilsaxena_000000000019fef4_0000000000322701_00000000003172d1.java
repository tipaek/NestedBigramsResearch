...
public class Solution {
    ...
    private static Object solveTestCase() {
        ...
        for(Map.Entry<BigDecimal, Integer> entry: map.entrySet()){
            for(Map.Entry<BigDecimal, Integer> entry1: map.entrySet()){
                if(entry1.getKey().compareTo(entry.getKey()) < 0 && entry1.getValue() >= 2 ){
                    return 1;
                }
            }
            if(map.get(entry.getKey().divide(two)) != null){
                return 1;
            }
        }
        return 2;
    ...
    private  static BigDecimal getMaxKey(Map<BigDecimal, Integer> map){

...
public class Solution {
    ...
    public static void checkSquare(int[][] square, int t, int K) {
        ...
        for (int i=0; i < 100000; i++) {
            int r = ThreadLocalRandom.current().nextInt(square.length);
            int i1 = ThreadLocalRandom.current().nextInt(2);
            int i2 = ThreadLocalRandom.current().nextInt(2);;
            while (i2 == i1) {
                i2 = ThreadLocalRandom.current().nextInt(2);
            }
            if (r ==0) {

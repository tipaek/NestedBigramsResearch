...
class Solver {
    ...
    void solve() throws Exception {
        ...
        for (int tce = 1; tce <= tc; tce++) {
            ...
            int N = hp.nextInt();
            int[][] A = new int[N][];
            for (i = 0; i < N; ++i) A[i] = new int[] {hp.nextInt(), hp.nextInt(), i, -7};
            Arrays.sort(A, (a, b) -> a[0] != b[0] ?
                    Integer.compare(a[0], b[0]) : Integer.compare(a[1], b[1]));

            BitSet marked = new BitSet();
            int last = 0;
            for (i = 0; i < N; ++i) if (A[i][0] >= last) {
                marked.set(i);
                last = A[i][1];
            }

            last = 0;
            for (i = marked.nextClearBit(0); i < N; i = marked.nextClearBit(i + 1)) {
                if (A[i][0] < last) break;
                last = A[i][1];
            }

            if (i < N) hp.println("IMPOSSIBLE");
            else {
                for (i = 0; i < N; ++i) A[i][3] = marked.get(i) ? 1 : 0;
                Arrays.sort(A, (a, b) -> Integer.compare(a[2], b[2]));
                for (i = 0; i < N; ++i) hp.print(A[i][3] == 1 ? "C" : "J");
                hp.println();
            }
        }

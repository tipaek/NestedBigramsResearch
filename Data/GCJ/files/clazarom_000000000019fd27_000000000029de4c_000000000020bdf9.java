		...
		public boolean isAvailable(int[] newActivity) {
			for (int[] activity: activities) {
				if (activity[0]!=0 || activity[1]!=0) {
					...
					if((newInitTime >=newActivity[0] && newInitTime<newActivity[1]) ||
					   (newFinalTime > newActivity[0] && newFinalTime<=newActivity[1]) ||
					   (newInitTime < newActivity[0] && newFinalTime>newActivity[0]) ||
					   (newInitTime < newActivity[1] && newFinalTime>newActivity[1])  ){			
						//Not available now

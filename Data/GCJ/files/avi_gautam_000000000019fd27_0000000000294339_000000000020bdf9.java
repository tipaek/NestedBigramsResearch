...
public class Solution {
	...
	public static String indScdle(int [][] a, int N) {
		...
		for (int i = 0; i < N-1; i++) {
			if (!(a[maxFinTimeRowInd][1] <=a[i+1][0] ||  a[maxFinTimeRowInd][0] >=a[i+1][1])) {
		...
		/*
		 * genArr = sb.toString().toCharArray(); sb = null; sb = new StringBuilder();
		 * for (int i = 0; i < N; i++) { sb.append(genArr[a[i][2]]); }
		 */
		return sb.toString();
	}

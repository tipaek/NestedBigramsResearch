        ...
        for (int i = 1; i <= t; ++i) {
            ...
            if (totalDuration <= (end - start) * 2) {
                ...
                while(timeSlotCount > takenCount) {
                    ...
                    boolean cannotReverse = true;
                    for (int k = takenCount-1; k>=1; k--) {
                        int position = timeSlotList.get(k).sequence;
                        char prevAssigned = buffer.charAt(position);
                        buffer.setCharAt(position, 'X');
                        if (prevAssigned == 'C') {
                            modeJ = true;
                            cannotReverse = false;
                            takenCount = k;

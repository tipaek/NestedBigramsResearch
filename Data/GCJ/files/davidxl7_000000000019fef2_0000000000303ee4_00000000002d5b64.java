...
public class Solution {
    ...
    public static void solve(int R, int S){
        ...
        /*pair[]swap1 = new pair[size];
        swap1 = swap(1,2,start);
        for (int i = 0; i < size; i++) {
            System.out.println(swap1[i].a+" "+swap1[i].b);
        }
        System.out.println();
        pair[]swap2 = swap(2,3,swap1);
        for (int i = 0; i < size; i++) {
            System.out.println(swap2[i].a+" "+swap2[i].b);
        }*/
        while(true){
            state temp = Q.poll();
            ...
            for(int a = 1;a<size;a++) {
                for (int b = 1; a + b < size; b++){
                    pair[]tempstack = swap(a,b,temp.situation);
                    ArrayList<pair> temppath = new ArrayList<>(temp.path);
                    temppath.add(new pair(a,b));
                    Q.add(new state(R,S,temppath,tempstack));

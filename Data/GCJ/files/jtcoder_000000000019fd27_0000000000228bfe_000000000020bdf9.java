...
public class Solution {
    ...
    private void run() throws Exception {
        ...
        for (int i = 1; i <= testCases; i++) {
            int intervals = Integer.valueOf(sc.nextLine());
            Interval[] intervalsArr = new Interval[intervals];
            for (int j = 0; j < intervals; j++) {
                String line = sc.nextLine();
                Integer start = Integer.valueOf(line.split(" ")[0]);
                Integer end = Integer.valueOf(line.split(" ")[1]);
                intervalsArr[j] = new Interval(start, end, j);
            }
...
class Interval implements Comparable<Interval> {
    ...
    public int compareTo(Interval o) {
        return Integer.valueOf(this.start).compareTo(o.start);
    }

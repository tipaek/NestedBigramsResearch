...
class Solution{
    ...
    static void generate(int n, int k,int itr){
        ...
        fill(ans,0,0,-1,row,col,k,itr);
    }
    
    static void fill(int ans[][],int i,int j,int val,HashSet<Integer> row,ArrayList<HashSet<Integer>> col,int k,int itr){
        if(j>=ans.length)
        {
            i++;
            j=0;
            ...
            for(int z=1;z<=ans.length;z++){
                    if(!col.get(j).contains(z)){
                        if(!row.contains(z)){
                            ...
                            row.add(z);
                            col.get(j).add(z);
                            fill(ans,i,j+1,-1,row,col,k,itr);
                            if(found)
                                return;

...
public class Solution {
    ...
    public static int interest_finder (int[][] arr, int c_sum)
    {
        ...
        for (int i = 0; i < arr.length; i++) {
            for (int j = 0; j < arr[0].length; j++) {
                if (arr[i][j] != -1) {
                    ...
                    if (part_cnt > 0) {
                        if ((double)arr[i][j] < (double)part_sum / (double)part_cnt) {
                            //arr[i][j] = -1;
                            elim_r.add(i);
                            elim_c.add(j);
                            celim++;
    ...
    public static void main(String[] args) {
        pascalwalk.FastReader keyboard = new pascalwalk.FastReader();
        ...
        for (int i = 0; i < T; i++) {
            ...
            System.out.println("Case #" + (i + 1) + ": " + interest_finder(arr,0));
        }
    }
}

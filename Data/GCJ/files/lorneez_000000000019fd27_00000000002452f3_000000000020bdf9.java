...
public class Solution {
    ...
    private static boolean compareErrands(int nextStart, int nextEnd, int start, int end) {
        boolean overlap = (nextStart>start)&&(nextStart<end) || (nextEnd>start)&&(nextEnd<start);
        boolean outeredge = (nextStart == start)&&(nextStart<=end) || (nextEnd==end)&&(nextEnd>=start);
        boolean inneredge = (nextStart>start)&&(nextEnd<end) || (nextStart<start)&&(nextEnd>end);
        boolean equal = (nextEnd==end)&&(nextStart==start);
        return  (overlap || outeredge || inneredge || equal);
    }
}

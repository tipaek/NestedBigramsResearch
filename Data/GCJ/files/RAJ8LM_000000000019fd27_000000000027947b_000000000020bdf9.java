        ...
        for (WorkQueueEntry task : taskList) {
            ...
            for (int i = 0; i < jamieTasks.length; i++) {
                ...
                if ((i == jamieTasks.length - 1) && (task.startTime >= jamieTasks[i].endTime)) {
                    ...
                    if(task.startTime < jamieTasks[i+1].startTime && task.endTime <= jamieTasks[i+1].startTime){
                        jamieQueue.add(task);
                        resultString += 'J';
                        taskOwnerFound = true;
                        break;
                    }
                    else if(task.startTime < jamieTasks[i+1].startTime && task.endTime > jamieTasks[i+1].startTime){
            ...
            if (taskOwnerFound)
                ...
                for (int i = 0; i < cameronTasks.length; i++) {
                    ...
                    if ((i == cameronTasks.length - 1) && (task.startTime >= cameronTasks[i].endTime)) {
                        cameronQueue.add(task);
                        resultString += 'C';
                        taskOwnerFound = true;
                        break;
                    }
                    else if ((i < cameronTasks.length) && (task.startTime >= cameronTasks[i].endTime)){
                        if(task.endTime <= cameronTasks[i+1].startTime && task.endTime <= cameronTasks[i+1].startTime){
                            cameronQueue.add(task);
                            resultString += 'c';
                            taskOwnerFound = true;
                            break;
                        }else if(task.startTime < cameronTasks[i+1].startTime && task.endTime > cameronTasks[i+1].startTime){
                            //  This is already an overlap
                            //  Let it go for Cameron
                            break;
                        }

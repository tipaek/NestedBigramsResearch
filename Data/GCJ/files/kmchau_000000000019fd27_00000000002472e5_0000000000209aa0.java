...
//            long n = in.nextLong();
            ...
            sum_up(new ArrayList<Integer>(numbers),target, set, n);
...
//                    printMatrix(matrix);
    ...
    static void sum_up_recursive(ArrayList<Integer> numbers, int target, ArrayList<Integer> partial, Set<String> set, int size) {
        int s = 0;
        for (int x: partial)
            s += x;
        if (s == target) {
            if (partial.toArray().length == size) {
        ...
        for(int i=0;i<numbers.size();i++) {
            ...
            partial_rec.add(n);
            sum_up_recursive(remaining,target,partial_rec, set, size);
        }
    }

    static void sum_up(ArrayList<Integer> numbers, int target, Set<String> set, int size) {
        sum_up_recursive(numbers,target,new ArrayList<Integer>(), set, size);
    }

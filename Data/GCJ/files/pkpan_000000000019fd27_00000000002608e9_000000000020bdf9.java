...
public class Solution {
    ...
    private static void doAssignments(List<Task> taskList) {
        NavigableMap<Integer, String> availability = new TreeMap<>();
        availability.put(0, "CJ");

        taskList.stream()
                .sorted(Comparator.comparing(Task::getStartingTime))
                .forEach(task -> updateAvailability(task, availability));
    }

    private static void updateAvailability(Task task, NavigableMap<Integer, String> availability) {
    ...
    private static class Task {
        ...
        private Task(int startingTime, int endingTime) {

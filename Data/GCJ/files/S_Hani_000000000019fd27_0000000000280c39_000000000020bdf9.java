...
public class Solution {
	public static void main(String[] args) {
		...
		for (int testcase = 1; testcase <= testcases; testcase++) {
			...
			activity: for (int i = 0; i < no_of_activities; i++) {
				...
				C_busy = is_resource_occupied(schedule_C, startTime, endTime);
				if (!C_busy) {
					...
					J_busy = is_resource_occupied(schedule_J, startTime, endTime);
					if (!J_busy) {
	...
	private static boolean is_resource_occupied(int[][] schedule, int startTime, int endTime) {

 ...
 */
	...
	static final int INF = 1_500_000_000;
	...
	public static void main(String[] args) {
		...
		for(int runs = 1; runs <= T; runs++) {
			...
			Arrays.sort(ord, (a,b) -> (qs[a]-qs[b]));
			
			for(int i = 0; i < N; i++) {
				if(qs[ord[i]] > 0) firstUpdate(ord[i]);
			}
			// Now do second kind
			secondUpdate();
	...
	static void firstUpdate(int id) {
		int targetDist = qs[id];
		
		int dFromRoot[] = distsFromRoot();
		for(Edge e : adj[id]) {
			int to = e.to(id);
			
			int cur = dFromRoot[to] + e.d;
			if(cur < targetDist) {
				e.d += (targetDist-cur);
			}
		}
	...
	static void secondUpdate() {
		...
		while(qs[ord[l]] >= 0) { l++; r++;}
		int prevD = 0;
		while(l < N) {
			while(r+1 < N && qs[ord[r+1]] == qs[ord[l]]) r++;
			// handle [l, r]
			if(debug) System.out.println(l+" "+r+"  "+qs[ord[l]]);
			
			int dFromRoot[] = distsFromRoot();
			int nextD = prevD+1;
			for(int i = l; i <= r; i++) nextD = Math.max(nextD, dFromRoot[ord[i]]);
			
			if(debug) System.out.println("D = "+Arrays.toString(dFromRoot));

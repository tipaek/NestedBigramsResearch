...
class Solver {
    ...
    void solve() throws Exception {
        ...
        for (int tce = 1; tce <= tc; tce++) {
            ...
            while (true) {
                if (Q > 0 && Q % 10 == 0) {
                    boolean sameBitFlipped = false, diffBitFlipped = false;
                    for (j = 0; j < B; ++j) if (BITS[j] >= 0) {
                        if (BITS[j] == BITS[B - 1 - j]) {
                            sameBitFlipped = BITS[j] != query(j);
                            break;
                        }
                    }
                    for (j = 0; j < B; ++j) if (BITS[j] >= 0) {
                        if (BITS[j] != BITS[B - 1 - j]) {
                            diffBitFlipped = BITS[j] != query(j);
                            break;
                        }
                    }

                    if (sameBitFlipped && diffBitFlipped) {
                        flipAll();
                    } else if (sameBitFlipped) {
                        flipAll();
                        reverse();
                    } else if (diffBitFlipped) {
                        reverse();
                    }
                } else {
                    for (j = 0; j < B; ++j) if (BITS[j] < 0) {
                        BITS[j] = query(j);
                        BITS[B - 1 - j] = query(B - 1 - j);
                        break;
                    }
                    if (j >= B) break;
                }
            ...
            for (int bit : BITS) hp.print(bit);
            hp.println();

            if (!hp.next().equals("Y")) break;
        }
    ...
    int query(int idx) throws Exception {
        ++Q;
    ...
    void flipAll() {
        for (int i = 0; i < B; ++i) if (BITS[i] >= 0) BITS[i] ^= 1;
    }

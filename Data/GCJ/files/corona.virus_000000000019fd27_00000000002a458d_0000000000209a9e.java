...
public class Solution{
	...
	private static boolean solve(Scanner input, int b) {
		...
		while(true) {
			//System.out.println("another loop, count is "+cnt+" same_alone: "+same_alone+" not_same: "+not_same_alone+" mixed:"+mixed);
			int x1=-1;
			int w=-1;
			if(same_alone) {
				...
				//System.out.println("simulate mix");
				if(one_one[0]!=-1) {
					x1=one_one[0];
					w=0;
				}
			...
			z_z=new int[] {-1,-1};
			one_z=new int[] {-1,-1};
			//System.out.print("string is: ");for(int i=0;i<B.length;i++) System.out.print(B[i]+" ");System.out.println();
			int lim=10-cnt%10+1;
	...
	private static void complement(int[] B, int p) {
		int n = B.length;
		//System.out.println("complement 0: to "+p);
		for(int i=0;i<=p;i++) {

...
class Solution {
    ...
    static class MyActivity{
        public MyActivity(int startTime, int endTime){
        ...
        public boolean equals(MyActivity activity){
            if(this.startTime == activity.startTime && this.endTime == activity.endTime) return true;
            return false;
        }
    }
    ...
    private static void setOutputList(){
        for(int i = 0; i< testCases.size(); i++){
            List<MyActivity> sortedActivities = sortActivities(new ArrayList<MyActivity>(testCases.get(i).activities));
    ...
    private static void setDataFromUserInput(){
        ...
        for(int i=0; i<numberOfTestCases; i++){
            ...
            for(int j = 0; j< numberOfActivities; j++){
                ...
                activityList.add(new MyActivity(Integer.parseInt(tokens[0]), Integer.parseInt(tokens[1])));
            }

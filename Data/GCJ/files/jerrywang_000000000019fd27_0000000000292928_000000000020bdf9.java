    ...
    public static void main(String[] args) {
        ...
        for(int i = 0; i < NC; i ++){
            ...
            for(int j = 0; j < activities.size(); j ++){
                for(int k = j+1; k < activities.size(); k ++){
                    ...
                    if(activities.get(j)[0] < activities.get(k)[1] && activities.get(k)[0] < activities.get(j)[1]){
                        if(activities.get(j)[0] < activities.get(k)[0]) gstart = activities.get(k)[0]; else gstart = activities.get(j)[0];
                        if(activities.get(j)[1] > activities.get(k)[1]) gend = activities.get(k)[1]; else gend = activities.get(j)[1];
                        for(int m = k+1; m < activities.size(); m ++){
                            if(activities.get(m)[0] < gend && gstart < activities.get(m)[0]) impossible = true;
                        }
                    }
                }
            }
            if(impossible) System.out.println("Case #" + (i+1) + ": IMPOSSIBLE");
                ...
                if(!editable.isEmpty()) jam.addAll(editable);

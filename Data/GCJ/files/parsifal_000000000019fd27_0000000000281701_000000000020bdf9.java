            ...
            for (int d = 0; d < n; d++) {
                if (jamieSchedule.size() == 0) {
                    ...
                    for (Map.Entry<LocalTime, LocalTime> entry : jamieSchedule) {
                        // for start time
                        if (activitySchedule.getKey().equals(entry.getKey()) || activitySchedule.getKey().isAfter(entry.getKey()) && activitySchedule.getKey().isBefore(entry.getValue())) {
                            canSchedule = false;
                            break;
                        }

                        // for end time
                        if (activitySchedule.getValue().equals(entry.getValue()) || activitySchedule.getValue().isAfter(entry.getKey()) && activitySchedule.getValue().isBefore(entry.getValue())) {
                            canSchedule = false;
                            break;
                        }

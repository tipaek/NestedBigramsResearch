    ...
    void solve() throws IOException {
        ...
        for (int t = 1; t <= tc; ++t) {
            ...
            while (!queue.isEmpty() && res.length()==0){
                ...
                //N possible +y
                if (Math.abs(point.y+newpow) <= Math.abs(y)){// && !visited.contains(point.x+"."+(point.y+newpow))) {
                ...
                //E possible +x
                if (Math.abs(point.x+newpow) <= Math.abs(x)){// && !visited.contains((point.x+newpow)+"."+point.y)) {

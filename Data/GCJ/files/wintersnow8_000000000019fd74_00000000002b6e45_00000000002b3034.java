	...
	public void run() throws Exception {
		...
		for (int q = 0; q<test; q++) {
			...
			for (int i = 0; i<n; i++ ) {
				...
				if (sf.length()>fro.length() && sf.substring(0,fro.length()).equals(fro)) {
					fro = sf;
				}
				else if (fro.length()>sf.length() && fro.substring(0,sf.length()).equals(sf)) {
					;
				}
				else if (fro.equals(sf)) ;
				else {
		//			System.out.println(fro.length() + " ||| " + sf.length() + "------------");
					ok = false;
				}
		...
		//		System.out.println(fro + " " + back + " " + ok);
			}

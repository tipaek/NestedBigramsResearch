...
class act implements Comparable<act>{
	...
	public int id;
	//public String ans;
	public act(int st,int end,int id){
		this.st=st;
		this.end=end;
		this.id=id;
		//this.ans="";
	...
	public int compareTo(act a){
		if(this.st!=a.st)return this.st-a.st;
		else{
...
public class Solution{
	...
	public static void solve(act[] acts,int n,int t) throws Exception{
		...
		//for(int i=0;i<n;i++){
			//print.println(acts[i].st+" "+acts[i].end);
		//}
		Stack<act> st=new Stack<>();
		for(int i=n-1;i>=0;i--){
			st.push(acts[i]);
		}
		boolean imp=false;
		...
		while(!st.isEmpty()){
			...
			if(Ccur==null){
				...
				if(overlap(Ccur.st,Ccur.end,temp.st,temp.end)){
					if(overlap(Jcur.st,Jcur.end,temp.st,temp.end)){
						imp=true;
						break;
					}
					else{

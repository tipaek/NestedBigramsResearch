...
public class Solution {
    public static void main(String[] args) {
        ...
        for (int test = 1; test <= t; test++) {
            ...
            if (possible) {
                String ans;
                if(out[out.length-1]!='S' && out[out.length-1]!='N' && out[out.length-1]!='E' && out[out.length-1]!='W'){
                    ans=(new String(out)).substring(0,out.length-1);
                }
                else{
                    ans = new String(out);
                }
                System.out.println(String.format("Case #%d: %s", test, ans));

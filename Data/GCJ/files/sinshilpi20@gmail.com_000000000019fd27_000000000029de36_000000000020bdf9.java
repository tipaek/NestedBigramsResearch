...
class Solution {
    ...
    private static boolean isFirstPath(int start, boolean[][] adj, boolean[] visited, int ct) {
        ...
        if (isEnd(start, adj)) {
            ...
            for (int j = 0; j < adj.length; j++) {
                if (!visited[j] && canStartSecondPath(j, adj, visited) && isSecondPath(j, adj, visited)) {
                    printSolution(visited, ct);
                    return true;
                }
            }
    ...
    private static boolean canStartSecondPath(int start, boolean[][] adj, boolean[] visited) {
        for (int i = 0; i < adj.length; i++) {
            if (adj[i][start] && !visited[i]) return false;
        }
        return true;
    }

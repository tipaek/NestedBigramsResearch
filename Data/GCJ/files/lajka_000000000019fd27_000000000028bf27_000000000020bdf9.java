    ...
    public static void main(String[] args) {
        ...
        for (int testCase = 1; testCase <= numberOfTestCases; testCase++) {
            ...
            for (int periodNum = 1; periodNum <= numberOfPeriods; periodNum++) {
                ...
                Set<Integer> range = IntStream.range(leftBorder, rightBorder).mapToObj(num -> Integer.valueOf(num)).collect(Collectors.toSet());

                Set cameronIntersection = cameronMinutes.stream().filter(integer -> range.contains(integer)).collect(Collectors.toSet());

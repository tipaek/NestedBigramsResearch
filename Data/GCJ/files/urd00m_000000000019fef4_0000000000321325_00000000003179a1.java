		...
		for (int cn = 1; cn < t + 1; cn++) {
			...
			for (int i = 0; i < 10000; i++) {
				...
				if (("" + m).length() != r.length() || m == -1)
					...
					if (convert.containsKey(r.substring(0, 1)) == false)
						convert.put(r.substring(0, 1), id++);
					lowerBound[convert.get(r.substring(0, 1))] = Math.min(lowerBound[convert.get(r.substring(0, 1))],

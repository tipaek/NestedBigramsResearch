...
public class Solution {
    ...
    private static String taskAssignment(List<Integer[]> activitiesTime) {
        ...
        TaskSchedule cameronSchedule = new TaskSchedule(firstActivityTime[0], firstActivityTime[1]);
        TaskSchedule jamieSchedule = new TaskSchedule(0, 0);
        for (int i = 1; i < activitiesTime.size(); i++) {
            ...
            } else if (cameronSchedule.hasOverlappingHours(currentActivityTime) &&
                       jamieSchedule.hasOverlappingHours(currentActivityTime)) {
                return "IMPOSSIBLE";
            } else if (cameronSchedule.hasOverlappingHours(currentActivityTime)) {
                toBeAdded = "J";
            } else if (jamieSchedule.hasOverlappingHours(currentActivityTime)) {
                toBeAdded = "C";
            }

            if (Objects.equals("J", toBeAdded)) {
                jamieSchedule = new TaskSchedule(currentActivityTime[0], currentActivityTime[1]);
            } else {
                cameronSchedule = new TaskSchedule(currentActivityTime[0], currentActivityTime[1]);
            }
            sb.append(toBeAdded);
        }

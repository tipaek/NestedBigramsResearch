	...
	public static String Pattern(int[][] schedule) {
		...
		for (int i = 1; i < schedule.length; i++) {
			...
			for (int j = 0; j < CS.size(); j++) {
				...
				if (START>=CS.get(j)&&END<=CE.get(j)) validC=false;
				if (START<CS.get(j)&&END>CE.get(j)) validC=false;
				if (END>CS.get(j)&&END<=CE.get(j)) validC=false;
				//if(!validC) break;
			}
			for (int j = 0; j < JS.size(); j++) {
				...
				if (START>=JS.get(j)&&END<=JE.get(j)) validJ=false;
			    if (START<JS.get(j)&&END>JE.get(j)) validJ=false;
				if (END>JS.get(j)&&END<=JE.get(j)) validJ=false;
				//if(!validJ) break;
			}
			if (validC) {
				answer+="C";

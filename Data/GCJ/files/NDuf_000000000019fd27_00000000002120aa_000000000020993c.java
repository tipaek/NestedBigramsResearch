...
public class Solution {
	...
	public static String testCase(Scanner sc) throws Exception {
		...
		for(int r = 0 ; r < matrixSize ; ++r) {
			...
			trace += row.get(r);
			rdup += matrixSize - row.stream().distinct().collect(Collectors.toList()).size();
		}
		for(int c = 0 ; c < matrixSize ; ++c) {
			final int cf = c;
			cdup += matrixSize - rows.stream().map(row -> row.get(cf)).distinct().collect(Collectors.toList()).size();
		}

...
public class Solution {
	...
	public static void solve(int i, BufferedReader reader,
		...
		BigInteger lastLevel = BigInteger.ZERO;
		BigInteger newLevel = BigInteger.ZERO;
		do {
			long[][] newFloor = new long[r][c];
			ret.add(newLevel);
			lastLevel = newLevel;
			newLevel = BigInteger.ZERO;
			for (int x = 0; x < r; x++) {
				for (int y = 0; y < c; y++) {
					if (floor[x][y] > 0) {
						BigInteger bi = BigInteger.valueOf(floor[x][y]);
						newLevel.add(bi);
	...
	public static double getAvgSkill(long[][] floor, int row, int col) {
		int no = 1;
		long sum = floor[row][col];
		if (row > 0) {

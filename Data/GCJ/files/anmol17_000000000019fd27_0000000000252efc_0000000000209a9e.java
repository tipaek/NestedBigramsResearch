...
public class Solution {
    ...
    private void processCase(int b, Scanner in) throws Exception {
        ...
        for (int i = 0; i < 150; i++) {
            ...
            if (i != 0 && i % 10 == 0) {
                ...
                if(totalReceived % 2 == 0 && (i + 1) % 10 == 0) {
                    getValueForIndex(1, in);
                    continue;
                }
                getAndProcessNextValue(totalReceived, bits, b, in, combinationToIndicesMap);
                totalReceived++;
            }
        }
    ...
    private FluctuationType getFluctuationType(Map<CombinationType, List<Integer>> combinationToIndicesMap, Scanner in, List<Integer> bits, int[] loopVar) {
        ...
        if(sameValueIndices == null) {
            ...
            int newVal1 = getValueForIndex(differentValueIndices.get(0) + 1, in);
            if(newVal1 == val1) {
            ...
            int newVal1 = getValueForIndex(sameValueIndices.get(0) + 1, in);
            if(newVal1 == val1) {
                return FluctuationType.NOTHING;
            } else {
                return FluctuationType.COMPLIMENT;
            ...
            int newVal1 = getValueForIndex(sameValueIndices.get(0) + 1, in);
            int newVal2 = getValueForIndex(differentValueIndices.get(0) + 1, in);
            loopVar[0]++;
    ...
    private void getAndProcessNextValue(int totalReceived, List<Integer> bits, int b, Scanner in, Map<CombinationType, List<Integer>> combinationToIndicesMap) {
        int indexToRequest = getNextIndex(bits, b, totalReceived);
        int newVal = getValueForIndex(indexToRequest + 1, in);
    ...
    private int getValueForIndex(int index, Scanner in) {

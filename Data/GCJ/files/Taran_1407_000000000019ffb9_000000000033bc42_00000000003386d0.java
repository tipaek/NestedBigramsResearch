...
public class Solution {
    ...
    void solve(int TC) throws Exception{
        ...
        for(int i = 0; i< N; i++)
            for(int j = i+1; j< N; j++){
                long a = P[j][1]-P[j][0], b = P[j][0]-P[i][0], c = P[i][0]*P[j][1]-P[i][1]*P[j][0];
                ...
                for(int x = 0; x+1< even.size(); x += 2){
                    curAns += even.get(x)+even.get(x+1);
                }
                int lo = (odd.size()%2 == 1?odd.get(odd.size()-1):-1), le = (even.size()%2 == 1?even.get(even.size()-1):-1);
                if(lo == -1 && le == -1)
                    ans = Math.max(ans, curAns+Math.min(2, one));
                else if(lo == -1){
                    ans = Math.max(ans, curAns+Math.min(2, one)+le);
                }else if(le == -1){
                    ans = Math.max(ans, Math.max(curAns+lo-1+Math.min(2, one), curAns+lo+Math.min(1, one)));
                }else{
                    ans = Math.max(ans, curAns+le+Math.max(curAns+lo-1+Math.min(2, one), curAns+le+lo+Math.min(1, one)));
                }
            }

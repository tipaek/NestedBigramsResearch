...
//                    }
                ...
                if (i + step <= bound & !paths.containsKey(eKey)) {
                    paths.put(eKey, paths.get(i + "," + j) + "E");
                    nextQueue.add(eKey);
                }
                String wKey = "" + (i - step) + "," + j;
                if (i - step >= -bound & !paths.containsKey(wKey)) {
                    paths.put(wKey, paths.get(i + "," + j) + "W");
                    nextQueue.add(wKey);
                }
                String nKey = i + "," + (j + step);
                if (j + step <= bound & !paths.containsKey(nKey)) {
                    paths.put(nKey, paths.get(i + "," + j) + "N");
                    nextQueue.add(nKey);
                }
                String sKey = i + "," + (j - step);
                if (j - step >= -bound & !paths.containsKey(sKey)) {
        ...
        for (int test = 1; test <= tests; test++) {
            int x = scan.nextInt();
            int y = scan.nextInt();
            System.out.println("Case #" + test + ": " + (paths.getOrDefault(x + "," + y, "IMPOSSIBLE")));
        }
    }
}

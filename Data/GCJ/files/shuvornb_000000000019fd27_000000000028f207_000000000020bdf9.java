...
public class Solution {
    ...
    private static String getCandidate(Task currentTask, List<Task> taskJ, List<Task> taskC) {
        ...
        for(Task task: taskC) {
            ...
            if(currentTask.endTime > task.startTime && currentTask.endTime <= task.endTime) flag = false;
            if(currentTask.startTime <= task.startTime && currentTask.endTime >= task.endTime) flag = false;
        }
        if(flag) return "C";
        else {
            flag = true;

...
public class Solution {
    ...
    static String solve(String str[] , int n) {
        for(int i = 0; i < n; ++i) {
            for(int j = 0; j < n; ++j) {
                if(i != j) {
                    String a = str[i]; a = a.substring(1 , a.length());
                    String b = str[j]; b = b.substring(1 , b.length());
                    if(a.length() > b.length()) {if(!check(b , a)) return "*";}
                    else if(b.length() > a.length()) {if(!check(a , b)) return "*";}
                    else {if(!a.equals(b)) return "*";}
                }
            }
        }
        String ans = "";

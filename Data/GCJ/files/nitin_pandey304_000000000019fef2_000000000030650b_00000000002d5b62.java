...
public class Solution {
	...
	private static long targetX, targetY, maxVal;
	private static HashMap<String, String> map ;
	private static String recur(String curr, long x, long y, long val) {
		if(x == targetX && y == targetY) {
			return curr;
		}
		if(Math.abs(x) > Math.abs(maxVal) || Math.abs(y) > Math.abs(maxVal)) {
			return "IMPOSSIBLE";
		}
		// if(val > maxVal) {
		// 	return "IMPOSSIBLE";
		// }
		String key = curr + "_" + x + "_" + y + "_" + val;
    ...
    private static String solve(long x, long y) {
	   ...
	   long val = 1;
	   map = new HashMap<>();
	   maxVal = nextPowerOf2(Math.abs(x) + Math.abs(y));

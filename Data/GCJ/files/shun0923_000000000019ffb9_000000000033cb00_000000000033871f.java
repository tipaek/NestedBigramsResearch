...
public void solve() {
	...
	for(int q = 0; q < query; q ++) {
		...
		Comparator<long[]> c = (sort1, sort2) -> Long.compare(sort1[0], sort2[0]);
		Queue<long[]> time = new PriorityQueue<>(c);
		Queue<long[]> pos = new PriorityQueue<>(c);
		long times[][] = new long[numNode][2];
		for(int i = 0; i < numNode; i ++) {
			times[i][1] = i;
		}
		times[0][0] = 0;
		for(int i = 0; i < numNode - 1; i ++) {
			if(array[i] < 0) {
				pos.add(new long[]{- array[i], i + 1});
			}else {
				time.add(new long[]{array[i], i + 1});
			}
		}
		long prvTime = 0;
		for(int i = 1; i < numNode; i ++) {
			if(!pos.isEmpty() && pos.peek()[0] <= i) {
				long crt[] = pos.poll();
				if(crt[0] == i) prvTime ++;
				times[(int)crt[1]][0] = prvTime;
			}else {
				long crt[] = time.poll();
				times[(int)crt[1]][0] = crt[0];
				prvTime = crt[0];
			}
		}

		Queue<long[]> pq = new PriorityQueue<>(c);
		for(int i = 1; i < numNode; i ++) {
			pq.add(times[i]);
		}
		while(!pq.isEmpty()) {
			long crt[] = pq.poll();
			for(int i = 0; i < numNode; i ++) {
				if(cost[i][(int)crt[1]] > 0 && crt[0] > times[i][0]) {
					cost[i][(int)crt[1]] = crt[0] - times[i][0];
					cost[(int)crt[1]][i] = crt[0] - times[i][0];
					break;
				}
			}
		}
		...
		for(int i = 0; i < numEdge; i ++) {
			ans[i] = cost[edges[i].source][edges[i].target];
		}
		prtln(ans);
	}
}

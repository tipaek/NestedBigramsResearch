...
public class Solution {
	...
	public static String activity_sel(int act, Scanner sc) {
		...
		Collections.sort(act_time, ((a1,a2) -> a1[0] - a2[0]));
		for(Integer[] entry : act_time) {
		...
		for(int i =0; i<act; i++) {
			...
			if(act_time.get(i)[0] < 0 || act_time.get(i)[1] <0 || act_time.get(i)[0] >= act_time.get(i)[1])
				return "IMPOSSIBLE";
			...
			if(act_time.get(i)[0] >= C_flag[1] || act_time.get(i)[1] <= C_flag[0]) {
				C_flag = act_time.get(i);
				index = C_flag[2];
				result[index] = 'C';
			}
			else if(act_time.get(i)[0] >= J_flag[1] || act_time.get(i)[1] <= C_flag[0]) {

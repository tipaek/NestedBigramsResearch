...
public class Solution {
	...
	void solve(int TC) {
		...
		while(n-->0) {
			long x=nl();
			f.put(x, f.containsKey(x)?f.get(x)+1:1);
		}
		int ans=3;
		for(long v:f.keySet()) {
			if(d==1) {
				...
				if(f.get(v)>=2)ans=Math.min(ans, 0);
				else ans=Math.min(ans, 1);
			} else {
				if(f.get(v)>=3)ans=Math.min(ans, 0);
				else if(f.get(v)==2&&containsGreater(v, f))ans=Math.min(ans, 1);
				else if(f.get(v)==1&&f.containsKey(1L*2*v))ans=Math.min(ans, 1);
	...
	boolean containsGreater(long x, HashMap<Long, Integer>f) {
		for(long v:f.keySet()) {
			if(v>x)return true;
		}
		return false;

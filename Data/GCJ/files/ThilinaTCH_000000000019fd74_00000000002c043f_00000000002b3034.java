...
public class Solution {
    ...
    private static String solve(String[] patterns, String longestPattern) {
        ...
        if(longestPattern.charAt(longestPattern.length() -1) == '*'){
            String longestStringWithprefix = "";
            for (int i = 0; i < patterns.length; i++) {
                if(patterns[i].charAt(0) == '*' && patterns[i].length() > longestStringWithprefix.length()){
                    longestStringWithprefix = patterns[i];
                }
            }
            shortestMatch = shortestMatch.concat(longestStringWithprefix.replace("*", ""));
        }
        for (int i = 0; i < patterns.length; i++) {

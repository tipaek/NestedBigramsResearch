...
class Solution {
	...
	void dfs(int r, int c, int done, int n, int k, int[][] a) {
		if (done == (n * n)) {
			if (ok(k, a) && ans == null) {
				ans = new int[n][n];
				for (int i = 0; i < n; i++)
		...
		for (int i = 1; i <= n; i++) {
			if (iv(r, c, i, a)) {
				a[r][c] = i;
				if(c+1<n)dfs(r, c+1, done+1, n, k, a);
				else dfs(r+1, 0, done+1, n, k, a);
				a[r][c]=0;
			}
		}
		return;
	...
	boolean iv(int r, int c, int val, int[][] a) {

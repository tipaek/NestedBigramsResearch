    ...
    static class SquareDance {
        ...
        public void solve(int testNumber, LightScanner in, LightWriter out) {
            ...
            for (int testCase = 1; testCase <= testCases; testCase++) {
                ...
                int[][] a = in.ints(h, w), na = new int[h][w];
                long ans = 0, sum = 0;
                ...
                while (cont) {
                    ...
                    for (int i = 0; i < h; i++) if (w >= 0) System.arraycopy(a[i], 0, na[i], 0, w);
                    ans += sum;
                    for (int i = 0; i < h; i++) {
                        for (int j = 0; j < w; j++) {
                            if (a[i][j] == 0) continue;
                            ...
                            for (int k = 0; k < 4; k++) {
                                ...
                                while (0 <= x && x < w && 0 <= y && y < h && a[y][x] == 0) {
                                    y += DY[k];
                                    x += DX[k];
                                }
                                if (0 <= x && x < w && 0 <= y && y < h && a[y][x] != 0) {
                                    count++;
                                    enemy += a[y][x];
                                }
                            }
                            if (a[i][j] * count < enemy) {
                                //System.out.println(i + " , " + j + " eliminated because " + enemy + "/" + count + " > " + a[i][j]);
                                cont = true;
                                sum -= a[i][j];
                                na[i][j] = 0;
                            }
                    ...
                    for (int i = 0; i < h; i++) if (w >= 0) System.arraycopy(na[i], 0, a[i], 0, w);
                }
                out.ans(ans).ln();
            }
        }

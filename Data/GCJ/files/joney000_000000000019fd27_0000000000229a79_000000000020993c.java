 ...
 // int [][]mat = new int[MAXN][MAXN];
  ...
  void run()throws Exception{
    ...
    for(int t = 1; t <= tests; t++){
      int n = i();
      HashSet<Integer>[] rowSet = new HashSet[MAXN];
      HashSet<Integer>[] colSet = new HashSet[MAXN];
      for(int pos = 1; pos <= MAXN - 1; pos++){
        rowSet[pos] = new HashSet<Integer>();
        colSet[pos] = new HashSet<Integer>();
      }
      long sum = 0;
      for(int r = 1; r <= n; r++){
        for(int c = 1; c <= n; c++){
      ...
      for(int pos = 1; pos <= n; pos++){
        if(rowSet[pos].size() < n)invaidRows++;
        if(colSet[pos].size() < n)invaidCols++;
      }

      out.write("Case #"+t+": ");
      out.write(""+sum+" "+invaidRows+" "+invaidCols);
      out.write("\n");
    }
  }

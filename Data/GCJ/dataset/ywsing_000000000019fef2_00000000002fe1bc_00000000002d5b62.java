...
//            System.out.println("Case #" + case_ + ":" + sb);
    ...
    public static String t(long x, long y, int n) {
        if (n < 0)
            return "";
        long pwr = 1 << n;
        long pwrs = n == 0 ? 0 : (1l << (n - 1));
        long xx, yy;
        String s;
        if (Math.abs(xx = x - pwr) <= pwrs && Math.abs(yy = y) <= pwrs) {
            if ((s = t(xx, yy, n - 1)) != null && !s.endsWith(" ")) return s + "E";
        }
        if (Math.abs(xx = x + pwr) <= pwrs && Math.abs(yy = y) <= pwrs) {
            if ((s = t(xx, yy, n - 1)) != null && !s.endsWith(" ")) return s + "W";
        }
        if (Math.abs(xx = x) <= pwrs && Math.abs(yy = y - pwr) <= pwrs) {
            if ((s = t(xx, yy, n - 1)) != null && !s.endsWith(" ")) return s + "N";
        }
        if (Math.abs(xx = x) <= pwrs && Math.abs(yy = y + pwr) <= pwrs) {
            if ((s = t(xx, yy, n - 1)) != null && !s.endsWith(" ")) return s + "S";
        }
        if (Math.abs(xx = x) <= pwrs && Math.abs(yy = y) <= pwrs) {
            if ((s = t(xx, yy, n - 1)) != null) return s + " ";
        }
        return null;
    }

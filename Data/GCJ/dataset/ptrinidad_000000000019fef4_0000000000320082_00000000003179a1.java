	...
	public static void main(String[] args) {
		...
		for (int test = 1; test <= t; ++test) {
			...
			for (int i = 0; i < 10000; i++) {
				...
				if (letters.size() == 10) {
					LinkedList<Character> toBeRemoved = new LinkedList<Character>();
					for (Map.Entry<Integer,LinkedList<Character>> pair: map.entrySet())  {
						if (pair.getValue().size() == 1) {
							toBeRemoved.add(pair.getValue().getFirst());
						}
					}
					for (Character c: toBeRemoved) {
						for (Map.Entry<Integer,LinkedList<Character>> pair: map.entrySet())  {
							if (pair.getValue().contains(c) && pair.getValue().size() != 1) {
								pair.getValue().remove(c);
							}
						}
					}

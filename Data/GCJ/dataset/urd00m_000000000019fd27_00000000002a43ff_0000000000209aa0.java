...
public class Solution {
	...
	public static void main(String args[]) throws IOException {
		...
		for (int cn = 1; cn < t + 1; cn++) {
			...
			if(n > 2 && trace[0] != 1 && trace[1] == 1) {
				row[trace[0]][0] = false; //if there is only one 
				column[trace[0]][0] = false;  
				row[trace[1]][1] = false; 
				column[trace[1]][1] = false;  
				trace[0]--; trace[1]++; //mix it up 
				row[trace[0]][0] = true; 
				column[trace[0]][0] = true; 
				row[trace[1]][1] = true; 
				column[trace[1]][1] = true; 
			}
			/*
			for(int i = 0; i < n; i++) {
				//n-i-1

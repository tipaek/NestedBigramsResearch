...
public class Solution {
    class Solver {
        public void solve(int testNumber, InputReader in, OutputWriter out) {
            ...
            Map<Character, Char> freqMap = new HashMap<>();
            for (int i = 0; i < queries.length; i++) {
                in.readInt();
                queries[i] = in.readString();
            }
            for (int i = 0; i < queries.length; i++) {
                if (queries[i].length() == u) {
                    char c = queries[i].charAt(0);
                    Char v = freqMap.get(c);
                    if (v == null) {
                        freqMap.put(c, new Char(c, 1));
                    } else {
                        v.frequency++;
                    }
                    for (int j = 1; j < queries[i].length(); j++) {
                        char c0 = queries[i].charAt(j);
                        Char v0 = freqMap.get(c0);
                        if (v0 == null) {
                            freqMap.put(c0, new Char(c0, 0));
                        }
                    }
                }
            }
            Char[] chars = new Char[freqMap.size() - 1];
            int i = 0;
            char zero_c = 'A';
            for (Map.Entry<Character, Char> entry : freqMap.entrySet()) {
            ...
            Arrays.sort(chars, Comparator.comparingInt(c -> c.frequency));
            StringBuilder digitsString = new StringBuilder();
            digitsString.append(zero_c);
            for (int j = chars.length - 1; j >= 0; j--) {
                digitsString.append(chars[j].c);
            }
            out.printLine(String.format("Case #%d: %s", testNumber, digitsString.toString()));
        }
    }

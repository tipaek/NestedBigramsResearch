    ...
    boolean dfs(int x, int y, String ans) {
        ...
        if (Math.abs(x)%2==1) {
            ...
            if (x!=-1 && dfs((x - 1) / 2, y / 2, new String(ans + "E")))
                return true;
            if (x==1 && y%4==0 && dfs(0, y / 4, new String(ans + "WE")))
                return true;
            if (x==-1 && y%4==0 && dfs(0, y / 4, new String(ans + "EW")))
                return true;
        }
        else {
            ...
            if (y!=-1 && dfs(x / 2, (y - 1) / 2, new String(ans + "N")))
                return true;
            if (y==1 && x%4==0 && dfs(x / 4, 0, new String(ans + "SN")))
                return true;
            if (y==-1 && x%4==0 && dfs(x / 4, 0, new String(ans + "NS")))
                return true;
        }
        return false;
    }

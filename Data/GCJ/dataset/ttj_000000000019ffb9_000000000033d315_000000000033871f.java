        ...
        for (int i = 0; i < nodeList.size(); i++) {
            ...
            for (Node node : group) {
                ...
                if (linkSet != null) {
                    ...
                    if (!isDone) {
                        for (Link link : linkSet) {
                            if ((link.first != node.position && latencyMap.containsKey(link.first))
                                    || (link.second != node.position && latencyMap.containsKey(link.second))) {
                                int baseLatency = (link.first != node.position && latencyMap.containsKey(link.first))
                                        ? latencyMap.get(link.first)
                                        : latencyMap.get(link.second);
                                link.latency = latency - baseLatency;
                                isDone = true;
                                break;
                            }
                        }
            ...
            for (Integer position : currentSet) {
                latencyMap.put(position, latency);
            }

...
public class Solution {
	void solveCase(int caseNr) {
				...
				if (cut > cutsize) {
					...
					if (possibleCuts+freeCuts >= neededCuts) {
						if (prevSliceCount + currentCuts >= neededCuts) {
							long suggestedValue = (oldPossibleCutsDone + (neededCuts-prevSliceCount));
							System.out.println("Case #"+caseNr+": "+ (suggestedValue > 0 ? suggestedValue :0));
						} else {						
							long suggestedValue = possibleCuts;
							System.out.println("Case #"+caseNr+": "+ (suggestedValue > 0 ? suggestedValue :0));
						}
						return;
					}
		...
		long suggestedValue = (diners-arr[0][1]);
		System.out.println("Case #"+caseNr+": "+ (suggestedValue > 0 ? suggestedValue :0));
	}

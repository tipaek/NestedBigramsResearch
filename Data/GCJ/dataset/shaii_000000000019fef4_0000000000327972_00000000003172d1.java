...
public class Solution {
	...
	private static int calculateResult(int D, long[] slicesAngles)
	{
		...
		for (int i = 0; i < slicesAngles.length; i++) {
			...
			int k = i;
			while (dTemp > 0 && k < slicesAngles.length && currentSliceSize == slicesAngles[k] && totalSlicesNeeded < totalSlicesNeededMin){
				dTemp--;
				k++;
			}
			if (k == slicesAngles.length && dTemp > 0)
			{
				...
				j = k;
				while (dTemp > 0 && j < slicesAngles.length && totalSlicesNeeded < totalSlicesNeededMin){
					if (slicesAngles[j] % currentSliceSize != 0)
					{

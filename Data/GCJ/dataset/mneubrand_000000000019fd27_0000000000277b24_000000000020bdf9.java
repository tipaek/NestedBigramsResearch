...
public class Solution {
	...
	private static String handleCase(BufferedReader br) throws Exception {
		...
		if(overlappingActivities.size() > 0) {
			...
			cameron.add(overlappingActivities.get(0));
			List[] valid = genVariants(overlappingActivities, cameron, new ArrayList<int[]>(), 1);
			if(valid != null) {
				return printVariant(valid[0], valid[1], solution);
			} else {
				return "IMPOSSIBLE";
			}
		} else {
	...
	private static String printVariant(List<int[]> cameron, List<int[]> jamie, char[] solution) {

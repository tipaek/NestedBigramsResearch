			...
			for(int i=1 ; i<numOfActivity ; i++){
				...
				for(int j=0 ; j<numOfActivity ; j++){
					...
					if(i != j){
						...
						/*System.out.println("numArr[i][0] :: "+numArr[i][0]);
						System.out.println("numArr[i][1] :: "+numArr[i][1]);
						System.out.println("numArr[j][0] :: "+numArr[j][0]);
						System.out.println("numArr[j][1] :: "+numArr[j][1]);
						
						System.out.println("numArr[i][0] > numArr[j][0] && numArr[i][0] < numArr[j][1] :: "+(numArr[i][0] >  numArr[j][0] && numArr[i][0] < numArr[j][1]));
						System.out.println("numArr[i][1] > numArr[j][0] && numArr[i][1] < numArr[j][1] :: "+(numArr[i][1] > numArr[j][0] && numArr[i][1] < numArr[j][1]));
						*/
						if( ( numArr[i][0] > numArr[j][0] && numArr[i][0] < numArr[j][1] ) || ( numArr[i][1] > numArr[j][0] && numArr[i][1] < numArr[j][1] ) ){
							...
							//System.out.println("No Conflict in activity :: "+ i + " iterate for other row");
							takenActivity[i] = 'B'; // Assigning activity to Blank
				...
				if(imPossible){
					//System.out.println("In IMPOSSIBLE");
					break;
				}
				
				if(takenActivity[i] == 'B'){

          ...
          for(int j = 0; j<N; j++) {
        	  ...
        	  if(j == 0) {
        		  ...
        		  for(int g = 0; g<j_schedule.size(); g++) {
        			  if((start < j_schedule.get(g).get(0) && end < j_schedule.get(g).get(0)) || (start >= j_schedule.get(g).get(1)) ) {
        				  output += "J";
        				  List<Integer> list = new ArrayList<Integer>(); 
        			        list.add(start); 
        			        list.add(end); 
        			  ...
        			  if (c_schedule.isEmpty()) {
      				  ...
      				  flag = 1;
        				  ...
        				  for(int g = 0; g<c_schedule.size(); g++) {
                			  if((start < c_schedule.get(g).get(0) && end < c_schedule.get(g).get(0)) || (start >= c_schedule.get(g).get(1)) ) {
                				  output += "C";
                				  List<Integer> list = new ArrayList<Integer>(); 
                			        list.add(start); 
                			        list.add(end); 

...
public class Solution {
    ...
    private static String solveProblem(List<Activity> activities) {
        ...
        // Sort by activity
        List sortedActivities = activities.stream().sorted(Comparator.comparingInt(Activity::getDuration).reversed()).collect(Collectors.toList());

        Schedule jSchedule = new Schedule();
        Schedule cSchedule = new Schedule();

        for(Activity activity:activities) {
            if(jSchedule.bookActivity(activity));
            else if(cSchedule.bookActivity(activity));
    ...
    static class Schedule{
        ...
        boolean bookActivity(Activity newActivity) {
            for(Activity activity: activities) {
                if(activity.endTime> newActivity.startTime && activity.startTime < newActivity.endTime)
                    return  false;
            }

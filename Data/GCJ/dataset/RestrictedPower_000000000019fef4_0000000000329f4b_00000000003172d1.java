...
/*
    Solution Created: 14:29:51 02/05/2020
...
public class Solution {
	...
	static HashMap<Long, ArrayList<Pair>> costs;
	public static void solveCase(Reader in, Writer out) {
		int n = in.nextInt(), d = in.nextInt();
		long[] a = in.nl(n);
		int ans = d-1;
		costs = new HashMap<Long, ArrayList<Pair>>();
		for(Long i : a) {
			long cur = i;
			addOption(cur, 0, 1);
			int created = 1;
			...
			while(cur>0 && cur%2==0) {
				...
				splits+=created;
				created*=2;
				addOption(cur, splits, created);
			}
		}
		for(Entry<Long, ArrayList<Pair>> i : costs.entrySet()) {
			long cur = d;
			ArrayList<Pair> pairs = i.getValue();
			Collections.sort(pairs);
			int cost = 0;
			int complete = 0;
			HashSet<Integer> usd = new HashSet<Integer>();
			
			for(int j = 0; j<pairs.size(); j++) {
				if(complete>=cur) break;
				Pair p = pairs.get(j);
				usd.add(j);
				cost+=p.cost;
				complete+=p.amount;
			...
			for(int j = pairs.size()-1; j>=0; j--) {
				if(!usd.contains(j)) continue;
				Pair p = pairs.get(j);
				if(complete-p.amount>=cur) {
					complete-=p.amount;
					cost-=p.cost;
				}
			}
			Util.dbg(complete);
			if(complete>=cur) ans = Math.min(cost, ans);
		}
		out.print(ans);
	}
	
	public static void addOption(long val, int cost, int amount) {
		if(!costs.containsKey(val)) costs.put(val, new ArrayList<Pair>());
		costs.get(val).add(new Pair(cost,amount));
	}
	static class Pair implements Comparable<Pair>{
		int cost,amount;
		public Pair(int cost, int amount) {
			this.cost = cost;
			this.amount = amount;
		}

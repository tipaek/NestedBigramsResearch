        ...
        for (int q = 1; q <= t; q++) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (strings.get(i).split("\\*").length == 2) {
                    left.add(strings.get(i).split("\\*")[0]);
                    right.add(strings.get(i).split("\\*")[1]);
                } else {
                    if (strings.get(i).startsWith("*")) {
                        right.add(strings.get(i).split("\\*")[0]);
                    } else {
                        left.add(strings.get(i).split("\\*")[0]);
                    }
                }
            }
            Collections.sort(left, Comparator.comparingInt(String::length));
            Collections.sort(right, Comparator.comparingInt(String::length));
            Collections.reverse(left);
            Collections.reverse(right);
            String max = left.get(0);
            boolean found = true;
            StringBuilder res = new StringBuilder();
            for (int i = 1; i < left.size(); i++) {
                if (max.startsWith(left.get(i))) {
                    continue;
                } else {
                    found = false;
                    break;

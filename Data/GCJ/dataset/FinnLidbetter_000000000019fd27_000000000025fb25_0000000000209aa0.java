...
public class Solution {
  public static void main(String[] args) throws IOException {
    ...
    for (int test=0; test<nTests; test++) {
      ...
      if (trace==n+1 || trace==n*n-1 || (n==3 && (trace==5 || trace==7))) {
      ...
      /*
      boolean valid = isValid(grid, trace);
      if (!valid) {
        System.err.printf(gridToString(grid));
      }
      */
  ...
  static boolean isValid(Integer[][] grid, int trace) {
    int n = grid.length;
    for (int val=1; val<=n; val++) {
      for (int i=0; i<n; i++) {
        if (!rowHasValue(grid, i, val)) {
          System.err.printf("ERROR on case %d %d. Missing value %d in row %d\n", n, trace, val, i);
          return false;
        }
        if (!colHasValue(grid, i, val)) {
          System.err.printf("ERROR on case %d %d. Missing value %d in col %d\n", n, trace, val, i);
          return false;
        }
      }
    }
    ...
    if (sum!=trace) {
      System.err.printf("ERROR on case %d %d. The grid does not have the correct trace\n", n, trace);
      return false;
    }
    return true;
  }

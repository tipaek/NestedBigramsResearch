...
public class Solution {
    ...
    public static String parentingPartnering(BufferedReader f, PrintWriter out, int caseNumber) throws IOException{
        ...
        String[] assignmentArray = new String[N];
        boolean cAssigned = false, jAssigned = false;
        int cAssignment = -1, jAssignment = -1;
            ...
            if(time.isStart){
                ...
                if(activeCounter > 2){
                    ...
                    if(!cAssigned){
                        cAssigned = true;
                        //assignmentString += "C";
                        assignmentArray[time.identifier] = "C";
                        cAssignment = time.identifier;
                    }else if(!jAssigned){
                        jAssigned = true;
                        //assignmentString += "J";
                        assignmentArray[time.identifier] = "J";
        ...
        for(int i = 0; i < assignmentArray.length; i++){
            assignmentString += assignmentArray[i];       
        }

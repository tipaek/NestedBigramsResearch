...
public class Solution {
    ...
    public static String losOp() {
        ...
        while (gevonden) {
            gevonden = false;
            for (int j = 0; j <= 9; j++) {
                if(index.contains(j)) continue; // Al opgelost.
                String s2 = D.get(gs[j]);
                if (s2.length() == 1) { // Haal getal uit anderen.
                    gevonden = true;
                    index.add(j);
                    for (int k = 0; k <= 9; k++) {
                        if (k == j) continue;
                    ...
                    for (int i = 0; i <= 9; i++) {
                        if (i == j) continue;
                        for (char c : D.get(gs[i]).toCharArray()) {
                            s2 = s2.replace(c + "", "");
                        }
                    }
                    if (s2.length() == 1) {
        ...
        String resultaat = "";
        ArrayList<Character> gehad = new ArrayList<>(10);
        for (int i = 0; i <= 9; i++) {
            for (Entry<Character, String> g : D.entrySet()) {
                if (!gehad.contains(g.getKey()) && g.getValue().contains(Integer.toString(i))) {

...
public class Solution {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= t; ++i) {
            ...
            int jobs = in.nextInt();
            cTimeStr = new String(new char[3600+1]).replace("\0", "0");
            jTimeStr = new String(new char[3600+1]).replace("\0", "0");
            String ans  = "";
    ...
    public static String assignSlot(int start, int end) {
        ...
        if ( !cTimeStr.substring(start, actualEnd).contains("1") ) {
            StringBuffer buf = new StringBuffer(cTimeStr);
            String replaceString = new String(new char[actualEnd-start]).replace("\0", "1");
            buf.replace(start, actualEnd, replaceString);
            cTimeStr = buf.toString();
            return "C";
        }
        if (!jTimeStr.substring(start, actualEnd).contains("1")) {

				...
				for (int i = 0; i <b; i++) {
					...
					if(cnt==0) {
						...
						for (int index = 0; index < list_size; index++) {
							...
							if(ta.toString()==tb.toString()) tb_contain=true;
							if(ta.toString()==tc.toString()) tc_contain=true;
							if(tb.toString()==tc.toString()) tc_contain=true;
							for(StringBuffer yy:list) {
								if(!ta_contain && yy.toString()==ta.toString()) ta_contain=true;
								if(!tb_contain && yy.toString()==tb.toString()) tb_contain=true;
								if(!tc_contain && yy.toString()==tc.toString()) tc_contain=true;
							}
							if(!ta_contain){
								list.add(ta);	
							}

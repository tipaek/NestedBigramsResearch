...
public class Solution {
	private static HashMap<String, Boolean> isPossibleCombo = new HashMap<String, Boolean>();
	...
	public static Boolean isPossible(Integer total, Integer size) {
		Boolean isAlreadyPossible = isPossibleCombo.get(total + ":" + size);
		if (isAlreadyPossible != null) {
			return isAlreadyPossible;
		}
		
		Boolean isCurrentlyPossible = false;
		for (int i = 1; i <= size; i++) {
			Boolean isPossible = total % (i * size) == 0;
			isCurrentlyPossible = isCurrentlyPossible || isPossible;
		}
		isPossibleCombo.put(total + ":" + size, isCurrentlyPossible);
		return isCurrentlyPossible;
	}
...
//			sb.append("POSSIBLE");
	...
	public static void testIsPossible() {
		...
		for (int i = MIN; i <= MAX; i++) {
			int numPossible = 0;
			for (int j = i; j <= (i*i); j++) {
				Boolean isPossible = isPossible(j, i);
				System.out.println("total: " + j + " size: " + i + " " + isPossible);

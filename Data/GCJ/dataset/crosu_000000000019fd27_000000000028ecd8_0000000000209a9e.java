...
public class Solution {
	...
	public static void solve_for_100() {
		...
		int[] intermediate_values = new int[100];
		int[] final_values = new int[100];
		
		int[][] small_chunk_indices = new int[10][10];		
		for (int i = 0; i < 10; i++) {
			for (int j = 0; j < 10; j++) {
				if (j < 5) {
					small_chunk_indices[i][j] = i*5 + j;
				} else {
					small_chunk_indices[i][j] = (20-i-2)*5 + j;
				}
			}
		}
		
		for (int i = 0; i < 10; i++) {
			for (int j : small_chunk_indices[i]) {
				values[j] = getNth(j);
			}
		}		
		
		for (int i = 0; i < 10; i++) {
			int[] chunk_indices = small_chunk_indices[i];
			int[] chunk = extractByIndices(values, chunk_indices);
			int[] final_chunck = updateForFlipsAndComplements(chunk, chunk_indices);
			insertByIndices(intermediate_values, chunk_indices, final_chunck);
		}
		
		int[][] large_chunk_indices = new int[2][50];		
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 50; j++) {
				if (j < 25) {
					large_chunk_indices[i][j] = i*25 + j;
				} else {
					large_chunk_indices[i][j] = (4-i-2)*25 + j;
				}
			}
		}
		
		for (int i = 0; i < 2; i++) {
			int[] chunk_indices = large_chunk_indices[i];
			int[] chunk = extractByIndices(intermediate_values, chunk_indices);
			int[] final_chunck = updateForFlipsAndComplements(chunk, chunk_indices);
			insertByIndices(final_values, chunk_indices, final_chunck);
		}

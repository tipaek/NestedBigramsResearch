...
public class Solution {
    ...
    private static boolean fun500 (long n , ArrayList<Pair> lis) {
        for (long i=0;i<n;i++) {
            Pair p = new Pair((int)i+1,(int)i+1);
            lis.add(p);
        }
        return true;
    }
    private static boolean fun1000 (long n , ArrayList<Pair> list) {
        long x = n - 500;
        for (int i=0;i<500;i++) {
            if (i == x-1) {

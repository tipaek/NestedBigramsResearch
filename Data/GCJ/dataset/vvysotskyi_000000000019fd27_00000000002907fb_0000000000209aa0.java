...
public class Solution {
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < testCasesCount; i++) {
      matrices.add(getMatrix(s));
    }

    for (int i = 1; i <= testCasesCount; i++) {
      int[][] matrix = matrices.get(i - 1);
      String possibility = matrix == null ? "IMPOSSIBLE" : "POSSIBLE";
      System.out.println(String.format("Case #%s: %s ", i, possibility));
      if (matrix != null) {
        String result = Arrays.stream(matrix)

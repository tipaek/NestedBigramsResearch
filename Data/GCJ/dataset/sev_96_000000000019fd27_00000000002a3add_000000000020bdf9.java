        ...
        for (int k = 0; k < caseCount; k++) {
            ...
            for (Map<Integer, Integer> map1 : list) {
                ...
                for (Map<Integer, Integer> map2 : list) {
                    ...
                    if ((j.getKey() > i.getKey() && j.getKey() < i.getValue()) || (j.getValue() > i.getKey() && j.getValue() < i.getValue()) || (count != count1 && i.getKey().equals(j.getKey()) && i.getValue().equals(j.getValue()))) {
                        if (count == 0) {
                            value[count1] = 'J';
                        ...
                        output = "IMPOSSIBLE";
                        break;
                    } else if (i.getKey().equals(j.getKey()) && i.getValue().equals(j.getValue()) && common > 1) {
                        value[count1] = value[commonPlace] == 'C' ? 'J' : 'C';

                    }else if (i.getKey().equals(j.getKey()) && i.getValue().equals(j.getValue())) {
                        common++;
                        commonPlace = count1;

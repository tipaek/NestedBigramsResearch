    ...
    for(int t =1;t<=T;t++){
        ...
        for(Map.Entry<Integer,Integer> entry : map.entrySet()){
            ...
            if(jobMap.size() < 2){
                ...
                resultMap.put((entry.getKey()+"_"+entry.getValue()),whoDoJob);continue;
            }
            if(key >= jobMap.get('J')){
                jobMap.put('J',entry.getValue());
                resultMap.put((entry.getKey()+"_"+entry.getValue()),'J');
            }
            else if(key >= jobMap.get('C')){
                jobMap.put('C',entry.getValue());  
                resultMap.put((entry.getKey()+"_"+entry.getValue()),'C'); 
            }
            else{ break;
            }
        }
        for(Map.Entry<Integer,Integer> entry : request.entrySet()){
            if(resultMap.get(entry.getKey()+"_"+entry.getValue()) == null){
                result = new StringBuilder("Case #"+t+": ").append("IMPOSSIBLE");break;
            }
            else result.append(resultMap.get(entry.getKey()+"_"+entry.getValue()));
        }

...
public class Solution implements Runnable {
    ...
    static String inputFilename = null;//"B_example.txt"; // if 'null', it reads data from std-in.
    ...
    public static class Solver extends  SolverAbstract{
        ...
        protected void solve() throws IOException {
            ...
            for (char ch : S.toCharArray()) {
                int need = Integer.parseInt(""+ch);
                if (need>=current){
                    for (int i = 0; i < need - current; i++) {
                        sb.append('(');
                    }
                }else{
                    for (int i = 0; i < current - need; i++) {
                        sb.append(')');
                    }
                }
                sb.append(ch);

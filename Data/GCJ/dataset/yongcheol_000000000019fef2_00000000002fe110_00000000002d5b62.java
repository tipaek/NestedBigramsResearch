	...
	public static String solve(Scanner sc) throws Exception {
		...
		while(!queue.isEmpty()) {
			...
			for(int d = 0; d < 4; d++) {
				...
				if(curP.move.length() > 0 && curP.move.charAt(0) == 'E' && cDir[d] == 'W') continue;
				if(curP.move.length() > 0 && curP.move.charAt(0) == 'W' && cDir[d] == 'E') continue;
				if(curP.move.length() > 0 && curP.move.charAt(0) == 'N' && cDir[d] == 'S') continue;
				if(curP.move.length() > 0 && curP.move.charAt(0) == 'S' && cDir[d] == 'N') continue;

				long nX = curP.x + (iDir[d][1] << curP.ith);
				long nY = curP.y + (iDir[d][0] << curP.ith);

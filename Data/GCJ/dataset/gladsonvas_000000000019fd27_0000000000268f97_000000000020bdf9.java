	...
	public static void main(String[] args) throws IOException {
		...
		for(int i=1;i<=T;i++){
			...
			for(int x=0;x<N;x++){
					...
					if(y.length()-2>=0){
						if(y.charAt(y.length()-1)==y.charAt(y.length()-2) && y.charAt(y.length()-1)=='C'){
							...
							if(indexOfLatestAssigned>-1){
								if((S[x][0]>=S[indexOfLatestAssigned][0] && S[x][1]<=S[indexOfLatestAssigned][1]) || 
										(S[x][0]>=S[indexOfLatestAssigned][0] && S[x][1]>=S[indexOfLatestAssigned][1] && S[x][0]<S[indexOfLatestAssigned][1]) || 
										(S[x][0]<=S[indexOfLatestAssigned][0] && S[x][1]>=S[indexOfLatestAssigned][1]) || 
										(S[x][0]<=S[indexOfLatestAssigned][0] && S[x][1]<=S[indexOfLatestAssigned][1] && S[x][1]>S[indexOfLatestAssigned][0] )){
									y="IMPOSSIBLE";
									break;
								}
							}
				...
				if((S[x][0]>=S[x-1][0] && S[x][1]<=S[x-1][1]) || 
						(S[x][0]>=S[x-1][0] && S[x][1]>=S[x-1][1] && S[x][0]<S[x-1][1]) || 
						(S[x][0]<=S[x-1][0] && S[x][1]>=S[x-1][1]) || 
						(S[x][0]<=S[x-1][0] && S[x][1]<=S[x-1][1] && S[x][1]>S[x-1][0] )){

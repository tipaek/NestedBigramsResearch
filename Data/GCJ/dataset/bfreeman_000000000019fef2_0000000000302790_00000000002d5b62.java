...
public class Solution {
    ...
    public static void runCase(int caseNum, int x, int y) {
        ...
        getBits(xbs, x);
        getBits(ybs, y);
        boolean xzeroed = false;
        boolean yzeroed = false;
        ...
        while (!solved) {
            if (xbs.get(bit) == 0 && ybs.get(bit) == 0) {
                ...
                if (shiftIfAble(xbs, bit)) { // TODO: may need to shift one or the other more intelligently

                } else if (shiftIfAble(ybs, bit)){
    ...
    private static String transcribeSolution(ArrayList<Byte> xbs, ArrayList<Byte> ybs) {
        ...
        int bit = 0;
        boolean solutionTranscribed = false;
    ...
    private static boolean shiftIfAble(ArrayList<Byte> xbs, int bit) {
        if (bit == 0) {
            return false;
        } else if (xbs.get(bit - 1) == 1) {
                if (bit == xbs.size() - 1){
                    xbs.set(bit - 1, (byte) -1);
                    xbs.set(bit, (byte) 0);
                    xbs.add((byte) 1);
                    return true;
                } else if (xbs.get(bit + 1) == 0) {
                    xbs.set(bit - 1, (byte) -1);
                    xbs.set(bit, (byte) 0);
                    xbs.set(bit + 1, (byte) 1);
                    return true;

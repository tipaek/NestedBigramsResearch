            ...
            else
            {
               ...
               jj[indexer.get(0)] = 'C';
                ...
                for (int i = 1; i< ender.size();i+=1)
                {
                    ...
                    {
                        if (trueMap.containsKey(ender.get(index)))
                        {
                            char ch = mapper.get(ender.get(index));
                            if (ch =='C')
                                jj[indexer.get(i)] = 'J';
                            else
                                jj[indexer.get(i)] = 'C';                        
                       
                            mapper.put (ender.get(i), jj[indexer.get(i)]);
                        }
                        else
                        {
                            jj[indexer.get(i)] = mapper.get(ender.get(index));
                            mapper.put (ender.get(i), jj[indexer.get(i)]);
                            trueMap.put (ender.get(index), true);        
                        }                
                    }
                }
               for (int i =0; i< n;i+=1)

...
public class Solution {
  ...
  static class Overrandomized {
    ...
    boolean allKnown(int qAt) {
      for (char c : queries[qAt].queryAnswer.toCharArray()) if (charToInt[c] == -1) return false;
      return true;
    }
    ...
    boolean dfs2(int qAt, int at) {
      Query q = queries[qAt];
      String queryAnswer = q.queryAnswer;
      while (at < queryAnswer.length() && charToInt[queryAnswer.charAt(at)] != -1) at++;
      if (queryAnswer.length() == at) return checkQuery(qAt) && dfs1(qAt + 1);
      char c = queryAnswer.charAt(at);
      for (int d = 0; d < 10; d++) {
    ...
    class Query {
      ...
      long parseAnswer() {
        long x = 0;
        for (char c : queryAnswer.toCharArray()) {
          x = x * 10 + charToInt[c];
        }
        return x;
      }

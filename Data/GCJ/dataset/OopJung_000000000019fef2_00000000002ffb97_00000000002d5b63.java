...
public class R1B2020_P2 {
	...
	public static void main(String[] args) throws IOException{
		...
		for(int t = 1; t<=T; t++) {
			...
			reply = br.readLine();
			if(reply.equals("HIT")) {
				leftx = findminy(-temp, -temp/2);
				rightx = findmaxy(-temp/2, temp);
			}
			else if(reply.equals("MISS")){
				bw.write(0 + " " + temp/2 +  "\n");
				bw.flush();
				...
				if(reply.equals("HIT")) {
					...
					leftx = findminy(-temp/2, temp/2);
					rightx = findmaxy(-temp/2, temp/2);
				}
				else {
					continue;
				}

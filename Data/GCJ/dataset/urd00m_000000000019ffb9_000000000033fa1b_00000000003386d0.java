		...
		for(int cn = 1; cn < t+1; cn++) {
			...
			for(int i = 0; i < n; i++) {
				for(int j = i+1; j < n; j++) {
					...
					Double slope = -1.0; 
					if(points[i][1] == points[j][1]) slope = (double) Integer.MAX_VALUE; 
					else if (points[i][0] == points[j][0]) slope = 0.0;  
					else slope = (Math.abs(points[i][1]-points[j][1])+0.0)/(Math.abs(points[i][0]-points[j][0])+0.0); 

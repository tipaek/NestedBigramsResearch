	...
	public void run() throws Exception {
		...
		for (int q = 0; q<test; q++) {
			...
			for (int i = 0; i<n; i++ ) {
				...
				if (sf.length()>=fro.length() && sf.contains(fro)) {
					fro = sf;
				}
				else if (fro.length()>=sf.length() && fro.contains(sf)) {
					;
				}
				else ok = false;
				if (sb.length()>=back.length() && sb.contains(back)) {
					back = sb;
				}
				else if (back.length()>=sb.length() && back.contains(sb)) {
					;
				}
				else ok = false;
			}

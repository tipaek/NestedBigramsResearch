...
//				graph.get(i) = new ArrayList<Integer>();
			...
			for (int i = 0;i<n;i++) {
				for (int j = 0;j<n;j++) {
					if (i != j) {
						if (st[i].start <= st[j].start && st[i].end >= st[j].end) {
							graph.get(i).add(j);
						}
						else if(st[i].start > st[j].start && st[i].end < st[j].end) {
							graph.get(i).add(j);
						}
						else if (st[i].end < st[j].end && st[j].start < st[i].end) {
							graph.get(i).add(j);
						}
						else if (st[i].end > st[j].end && st[i].start < st[j].end) {
							graph.get(i).add(j);
						}
					}
...
//									System.out.println("IMPOSSIBLE");
								...
								parent[graph.get(temp).get(j)] = assign[temp];
							}
						}
					}
				}

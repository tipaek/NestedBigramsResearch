...
public class Solution{
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= t; i++) {
            List<Integer> numbers = Arrays.stream(in.nextLine().split(" ")).map(val -> Integer.parseInt(val))
            ...
            if (trace == k) {
                ...
                matrix.stream().map(val -> val.stream().map(st -> st.toString()).collect(Collectors.joining(" ")))

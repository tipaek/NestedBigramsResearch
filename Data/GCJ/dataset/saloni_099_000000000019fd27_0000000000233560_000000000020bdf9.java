			...
			if(array.length>2) {
				...
				for(int k=2;k<n;k++) {
					...
					for(int h=0;h<j_start.size();h++) {
						if(min>j_start.get(h) && min<j_end.get(h) || (max<j_end.get(h) && max>j_start.get(h))) {
							check=1;
							
							break;
						}
						/*else if(min<=j_start.get(h) && max<=j_start.get(h) ) {
						...
						else if(min>=j_end.get(h) && temp==0) {
					...
					if(check==0) {
						...
						for(int h=0;h<c_start.size();h++) {
							...
							else if(min<=c_start.get(h) && max<=c_start.get(h) && temp==0 ) {
							...
							else if(min>=c_end.get(h) && temp==0) {

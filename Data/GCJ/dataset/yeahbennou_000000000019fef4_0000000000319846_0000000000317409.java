...
class Project extends Solution
{
    ...
    void solve()
    {
        int N = ri(), M = ri();
        String moves = rs();
        int[][] onPath = new int[N * 2 + 2][M * 2 + 2];
        for(int[] i: onPath) Arrays.fill(i, -1);
        // N -> j + 1
        // E -> i + 1
        int ans = Integer.MAX_VALUE;
        int cN = N, cM = M, time = 0;
        for(int i = 0; i < moves.length(); i++)
        {
            if(Math.abs(cN) + Math.abs(cM) <= time) ans = min(ans, max(Math.abs(cN) + Math.abs(cM), time));
//            onPath[cN + N][cM + M] =
            time++;
        ...
        if(Math.abs(cN) + Math.abs(cM) <= time) ans = min(ans, max(Math.abs(cN) + Math.abs(cM), time));
        pln(ans == Integer.MAX_VALUE? "IMPOSSIBLE": ans);
    }
    void setup()
    {
        int T = ri();

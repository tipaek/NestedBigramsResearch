    ...
    public static void main(String[] args) throws IOException {
		...
		for(int i = 1; i <= T; i++){
			...
			while((x!= 0 || y!= 0) && !imp){
				...
				if(x==step && Math.abs(y)!=step && y%(2*step)==0){
					x-=step;
					sb.append('W');
				}else if(x==-step && Math.abs(y)!=step && x%(2*step)==0){
					x+=step;
					sb.append('E');
				}else if(y==-step && Math.abs(x)!=step && y%(2*step)==0){
					y+=step;
					sb.append('N');
				}else if(y==step && Math.abs(x)!=step && x%(2*step)==0){
					y-=step;
					sb.append('S');
				}

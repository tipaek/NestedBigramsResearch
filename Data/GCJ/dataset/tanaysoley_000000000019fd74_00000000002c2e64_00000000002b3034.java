	...
	public static void main(String[] args) throws IOException {
		...
		for (int tc = 1; tc <= testCases; tc++) {
			...
			for (int i = 1; i < patternList.size(); i++) {
				String longest = patternList.get(0).start;
				if (!(matchStart(longest, patternList.get(i).start))) {
					startMatches = false;
					break;
				}else{
					longest = longest.length() > patternList.get(i).start.length() ? longest : patternList.get(i).start;
				}
			}
			...
			if (startMatches) {
				String longest = patternList.get(0).end;
				for (int i = 1; i < patternList.size(); i++) {
					if (!(matchEnd(longest, patternList.get(i).end))) {
						endMatches = false;
						break;
					} else {
						longest = longest.length() > patternList.get(i).end.length() ? longest : patternList.get(i).end;
					}
				}
			} else {
				endMatches = false;

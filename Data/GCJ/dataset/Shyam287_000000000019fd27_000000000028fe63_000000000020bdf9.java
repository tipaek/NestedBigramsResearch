...
class Solution {
	...
	private static void scheduleActivities(int[] startTime, int[] endTime, int activities, int testCase) {
		...
		int[] unsortedStartTime = new int[activities];
		int[] unsortedEndTime = new int[activities];

		LinkedHashMap<Integer, Integer> map = new LinkedHashMap<Integer, Integer>();
		
		for(int i = 0; i<activities; i++) {
			unsortedStartTime[i] = startTime[i];
			unsortedEndTime[i] = endTime[i];
		...
		char[] jamieOrCam = new char[activities];
		jamieOrCam[0] = 'J';
		...
		if(!"IMPOSSIBLE".equals(s)) {
			for(int i = 0 ; i<activities ;i++) {
				...
				if( unsortedStartTime[i] == startTime[j] && unsortedEndTime[i] == endTime[j]) {
					s.append(jamieOrCam[j]);
				}
			}
		}

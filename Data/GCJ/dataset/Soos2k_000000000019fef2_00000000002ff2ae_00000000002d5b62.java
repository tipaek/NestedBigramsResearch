...
public class Solution {
    ...
    //     int len = (int) Math.log(Math.max(Math.abs(x),Math.abs(y))) + 5;
        // for (int i = 0; i < len; i++){
        //     for (int j = 0; j < len; j++){
        //         String first = convertToString(i);
        //         String second = convertToString(j);
    ...
    public static String direction(int x, int y){
        String result = "";
        int len = (int) Math.log(Math.max(Math.abs(x),Math.abs(y))) + 2;
        int val = 2*Math.max(Math.abs(x),Math.abs(y));
        boolean[][] mark = new boolean[64*val + 1][64*val + 1];
        ...
        while (!findNode.isEmpty()){
            ...
            mark[node.x + 8*val][node.y+8*val] = true;

...
public class Solution {
    final static String filePrefix = "E";
...
//    private static final String IMPOSSIBLE = "IMPOSSIBLE";
    ...
    private void run(int caseNumber) throws Exception {
        ...
        if (N >= traceMaps.length || !traceMaps[N].containsKey(K)) {
            out.printf("Case #%d: %s\n", caseNumber, NO);
        } else {
            int[][] a = (int[][]) traceMaps[N].get(K);
            out.printf("Case #%d: %s\n", caseNumber, "POSSIBLE");
    ...
    static Map<Integer, int[][]> genLatinSquare(int n) {
//        Map<Integer, int[][]> map = new HashMap<>();
        Map<Integer, int[][]> map = new TreeMap<>();
        List<int[][]> base = genReducedLatinSquare(n);
        int[][] pns = permutations(0,n-1);
        int[][] pn1s = permutations(1,n-1);
        int cnt = 0;
        for (int[][] a : base) {
            for (int[] pn : pns) {
                for (int[] pn1 : pn1s) {
                    int[][] na = new int[n][n];
                    int s = 0;
                    for (int r = 0; r < na.length; r++) {
                        for (int c = 0; c < na[r].length; c++) {
                            na[r][c] = a[pn[r]][c==0?0:pn1[c-1]];
                        }
                        s += na[r][r];
                    }
                    map.put(s, na);
...
//        System.out.println("cnt="+cnt+"\n");
    ...
    public static int[][] permutations(int from, int to) {
    ...
    static int cnt = 0;
    static List<int[][]> genReducedLatinSquare(int n) {
        cnt = 0;
        int[][] a = new int[n][n];
        for (int i = 0; i < a.length; i++) {
            a[0][i] = a[i][0] = i+1;
...
//        System.out.println("  ls=" + ls);
    ...
    private static void go(int[][] a, int r, int c, List<int[][]> ls) {
        if (c == a.length) {
            go(a, r+1, 1, ls);
            return;
        }
...
//    static int[] debugTestCases = {};
    ...
    static Map[] traceMaps = new Map[6];
...
//        if (testFilenames[0] != null) myTest();        
        ...
        for (int i = 2; i < traceMaps.length; i++) {
//        for (int i = 2; i <= 5; i++) {
//        for (int i = 2; i <= 4; i++) {
//            genReducedLatinSquare(i);
            traceMaps[i] = genLatinSquare(i);
        }

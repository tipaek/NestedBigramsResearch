...
public class Solution {
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= t; ++i) {
            ...
            for (ArrayList<Integer> interval : intervals) {
                if (interval.get(0) >= c) {
                    ans.setCharAt(getIndexOf(interval.get(0), originalIntervals), 'C');
                    c = interval.get(1);
                } else if (interval.get(0) >= j) {
                    ans.setCharAt(getIndexOf(interval.get(0), originalIntervals), 'J');
                    j = interval.get(1);
                } else {
    ...
    static int getIndexOf(int start, List<ArrayList<Integer>> intervals) {
        for (int i = 0; i < intervals.size(); i++) {
            if(intervals.get(i).get(0) == start) {
                return i;
            }
        }
        return -1;

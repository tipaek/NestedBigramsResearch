...
public class Solution {
    public static String schedule(int[][] intervals,int caseNumber){
        ...
        if(intervals.length > 0){
            ...
            for(int index = 1; index < intervals.length; index++){
                ...
                }else if(result.charAt(result.length()-1) == 'J'){
                    if(validate(Jlist, intervals[index])){
                        result += 'J';
                        Jlist.add(intervals[index]);
                    }else if(validate(Clist, intervals[index])){
                        result += 'C';
                        Clist.add(intervals[index]);
                    }else{
                        return "Case #" + (caseNumber+1) + ": " + "IMPOSSIBLE";
    ...
    public static boolean validate(ArrayList<int[]> checkList,int[] list){
        for(int index = 0; index < checkList.size(); index++){
            if(list[0] > checkList.get(index)[0] && list[0] < checkList.get(index)[1] || list[1] > checkList.get(index)[0] && list[1] < checkList.get(index)[1] || list[0] == checkList.get(index)[0] && list[1] <= checkList.get(index)[1]){
                return false;
            }
        }
        return true;

...
public class Solution {
  private static String solve(Scanner input, int B) {
    ...
    for(int i = 1;i < B/2+1;++i) {
        ...
        ans = input.next();
        time++;
        res[B-i] = Integer.valueOf(ans);
        if(same==-1||diff==-1) {
            if(same==-1&&res[i]==res[B-i]) same = i;
            else if(diff==-1&&res[i]!=res[B-i]) diff = i;
        }
    }
    String ss = "";
  ...
  private static int test(Scanner input,int[] arr,int same,int diff,int time) {
      ...
      if(sa==arr[same]&&da==arr[diff]) {
          ...
          for(int i = 1;i < B;++i) {
              if(arr[i]!=-1) narr[i] = arr[B-i];
          }
      }else {
          for(int i = 1;i < B;++i) {
              if(arr[i]!=-1) narr[i] = arr[B-i]==1?1:0;
          }
      }
      for(int i = 1;i < B;++i) {
          if(arr[i]!=-1) arr[i] = narr[i];
      }
      return time;
  }

...
class Solution {
	...
	private String getCaseResult(Scanner scanner) {
		...
		while(stillHasUnassignedActivities(allActivities)) {
			assignOne(allActivities);
			...
			for (Activity activity : allActivities) {
				for (Activity other : allActivities) {
					...
					if (activity.overlaps(other)) {
						if (activity.hasPerson() && other.hasPerson()) {
							if (activity.getPerson() == other.getPerson()) {
								return "IMPOSSIBLE";
							}
						} else if (activity.hasPerson()) {
							other.setPerson(inverse(activity.getPerson()));
						} else if (other.hasPerson()) {
							activity.setPerson(inverse(other.getPerson()));
						}
					}
				}
			}

...
public class Solution {
	...
	public static void main(String[] args) {
		...
		try {
			...
			for (int i = 1; i <= testCases; i++) {
				...
				System.out.println(Solution.calculate(input,i));
			}
	...
	public static StringBuilder calculate(String input,int testCase){
		...
		return getBalancedString(input,openingBraces,closingBraces,testCase);
	}
	...
	public static StringBuilder getBalancedString(String input, int[] opening,
			int[] closing,int testCase) {
		StringBuilder outputString = new StringBuilder("Case #"+testCase+": ");
		for (int i = 0; i < input.length(); i++) {
			if (opening[i] != 0) {

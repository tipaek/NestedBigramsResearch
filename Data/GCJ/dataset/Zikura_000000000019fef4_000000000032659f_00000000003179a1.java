...
public class Solution {
        ...
        for(int caseNum=1;caseNum<=numCases;caseNum++) {
            ...
            for(int i=0;i<10000;i++) {
                ...
                for(int exponent = 1; exponent < maxExponent; exponent++) {
                    ...
                    if (num < maxValue+10 && result.length() == exponent+1) {
                        System.err.format("%d %s: Restricting %s to 1\n", num, result, result.charAt(0));
                        removeFromList(facts, result.charAt(0), 0, 0);
                        removeFromList(facts, result.charAt(0), 2, 9);

                        System.err.format("%d %s: Restricting %s to <= %d\n", num, result, result.charAt(result.length()-1), num%10);
                        removeFromList(facts, result.charAt(1), (int)(num%10)+1, 9);
                    }
                }

                if (num < 10 && result.length() == 1) {
                    System.err.format("%d %s: Restricting %s to <= %d\n", num, result, result.charAt(0), num);
    ...
    private static void removeFromList(Map<Character, List<Integer>> facts, char key, int lower, int upper) {
        ...
        if(list.size()==1) {
            System.err.printf("Determined %s to be %d\n", key, list.get(0));
            for(Character otherKey : facts.keySet()) {
                if(!otherKey.equals(key)) {
                    if(facts.get(otherKey).contains(list.get(0))) {
                        removeFromList(facts, otherKey, list.get(0), list.get(0));
                    }
                }
            }
        }

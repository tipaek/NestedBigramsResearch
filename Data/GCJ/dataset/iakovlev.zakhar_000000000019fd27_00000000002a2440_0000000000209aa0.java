...
public class Solution {
	...
	public void solve() {
		...
		// }
		// }
		// System.out.println(m + " " + set.size() + " " + (m * (m - 1) + 1));
		// for (int i : set) {
		// System.out.print(i + " ");
		// }
		// System.out.println();
		...
		number = new TreeMap[max];
		for (m = 2; m < max; m++) {
			trace[m] = new TreeMap<>();
			number[m] = new TreeMap<>();
			a = new int[m][m];
			rec(0, 0);
			// for (int k = 0; k <= m * m - m; k++) {
			// if (number[m].containsKey(k)) {
			// System.out.println(m + " " + k + " " + number[m].get(k));
			// int[][] ans = trace[m].get(k);
...
//		for (int m = 6; m <= 100; m++) {
//			for (int k = m; k <= m * m; k++) {
//				if (!check(find(m, k), m, k)) {
//					System.err.println(m + " " + k);
//				}
//			}
//			System.out.println(m + " " + (System.currentTimeMillis() - time0));
//		}
	}

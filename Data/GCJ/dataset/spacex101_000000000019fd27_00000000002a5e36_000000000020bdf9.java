    ...
    public static void main(String[] args) {
        ...
        for(int i = 0; i <tests; i++){
            ...
            for(int j = 1; j < times.length; j++){
                ...
                // System.out.println(times[j].beginning);
                for(int k = 0;k < C.personSchedule.length; k++){
                    ...
                    else if((times[j].beginning<C.personSchedule[k].beginning) && (times[j].end>C.personSchedule[k].end)){
                        canScheduleC = false;
                    }
                    else if((times[j].beginning<C.personSchedule[k].beginning) && (times[j].end<C.personSchedule[k].end)){
                        canScheduleC = false;
                ...
                if (canScheduleC == false){
                    for(int k = 0;k < J.personSchedule.length; k++){
                        ...
                        else if((times[j].beginning<J.personSchedule[k].beginning) && (times[j].end<J.personSchedule[k].end)){
                            canScheduleJ = false;
                        }
                        else if((times[j].beginning<J.personSchedule[k].beginning )&&( times[j].end>J.personSchedule[k].end)){
                            canScheduleJ = false;
                ...
                else if (canScheduleJ == true && canScheduleC ==false){
                    J.personSchedule[jIndex].beginning = times[j].beginning;
                    J.personSchedule[jIndex].end = times[j].end;
                    schedule +="J";
                }
                else if(canScheduleC == false && canScheduleJ == false){
                    schedule = "IMPOSSIBLE";
                }
                System.out.println(schedule+"jwomama");
            }

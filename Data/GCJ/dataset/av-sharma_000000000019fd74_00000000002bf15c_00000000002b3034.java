			...
			if (testCase == 1){
				...
				for (int i = 0; i < n && flag; i++) {
					if (!longest.contains(strs[i].substring(1))) flag = false;
						...
						int startIndex = longest.indexOf(strs[i].substring(1));
						int len = strs[i].substring(1).length() + startIndex - 1;
						if (len != longest.length() - 1) flag = false;
					}
				}
				...
				for (int i = 0; i < n && flag; i++) {
					...
					if (split.length == 1) {
						...
						if (indexStar == 0 && longestHalfR.contains(split[0].trim())) {
							int startIndex = longestHalfR.indexOf(split[0].trim());
							int len = split[0].trim().length() + startIndex - 1;
							if (len != longestHalfR.length() - 1) flag = false;
						}
						if (indexStar == strs[i].length()-1 && !longestHalfL.contains(split[0].trim())) flag = false;
						continue;
					}
					if (!longestHalfL.contains(split[0].trim())) flag = false;
					if (!longestHalfR.contains(split[1].trim())) flag = false;
					else {
						int startIndex = longestHalfR.indexOf(split[1].trim());
						int len = split[1].trim().length() + startIndex - 1;
						if (len != longestHalfR.length() - 1) flag = false;
					}
				}
				System.out.print("Case #" + t + ": ");

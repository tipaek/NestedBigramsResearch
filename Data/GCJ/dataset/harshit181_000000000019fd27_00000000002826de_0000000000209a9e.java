...
public class Solution {
	...
	public static boolean isBitSame(ArrayList<Object> possibleList,int bit)
	{
		int tempArr[]=(int [])possibleList.get(0);
		int last=tempArr[bit];
		if(last==-1)
			return true;
		for(int i=1;i<possibleList.size();i++)
		{
			int [] data=(int [])possibleList.get(i);
			if(data[bit]!=last)
				return false;

...
public class Solution {
    public static void main(String[] args) {
        ...
        while (testCases-- > 0) {
            ...
            int total = 10000;
            List<Record> fullRecords = new ArrayList<>();
            while(total-->0){
                String digit = scanner.next();
                if(!digit.equals("-1"))
                    fullRecords.add(new Record(digit, scanner.next()));
            }
            String dDigit = fullRecords.stream().map(record -> record.randomString).reduce(Solution::reduceString).get();
            List<Record> records = fullRecords.stream().filter(record -> record.i.length() == 1).filter(record -> Integer.valueOf(record.i) <= 9).collect(Collectors.toList());
            ...
            randomDigits.stream().filter(randomDigit -> !nonZeroDigits.contains(randomDigit.digit)).forEach(randomDigit -> {
                ...
                randomDigit.possible = new ArrayList<>();
                randomDigit.possible.add(0);
            });
            randomDigits.forEach(randomDigit1 -> randomDigit1.removePossibility(0));

            resultList.add(solveProblem(randomDigits,fullRecords));
        }
        int count = 0;
    ...
    private static String solveProblem(List<RandomDigit> randomDigits, List<Record> records) {
        records.stream().filter(record -> record.i.length() == 1).forEach(record -> randomDigits.stream().filter(randomDigit -> randomDigit.digit.equals(record.randomString.charAt(0))).forEach(randomDigit -> {

...
public class Solution {
  ...
  static class Tree {
    ...
    public boolean equals(Object o) {
      ...
      Tree tree = (Tree) o;
      return Objects.equals(points, tree.points)
          && Objects.equals(t1, tree.t1)
          && Objects.equals(t2, tree.t2)
          && Objects.equals(t3, tree.t3)
          && Objects.equals(t4, tree.t4)
          && Objects.equals(direction, tree.direction)
          && Objects.equals(parent, tree.parent);
    }

    @Override
    public int hashCode() {
      return Objects.hash(points, t1, t2, t3, t4, direction, parent);
    }
...
//        if (!cross(poll, x, y)) {
          ...
          if (t != null) {
            ...
            boolean b = crossedThePoint(n1, s1, e1, w1, x, y);
            isImpossible = isImpossible && b;
            if (!pointsAlreadyVisited.contains(n1)&&!b) {
              p.add(t1);
              pointsAlreadyVisited.add(n1);
            }
            if (!pointsAlreadyVisited.contains(s1)&&!b) {
              p.add(t2);
              pointsAlreadyVisited.add(s1);
            }
            if (!pointsAlreadyVisited.contains(e1)&&!b) {
              p.add(t3);
              pointsAlreadyVisited.add(e1);
            }
            if (!pointsAlreadyVisited.contains(w1)&&!b) {
              p.add(t4);
...
//        }
  ...
  private static boolean cross(Points poll, int x, int y) {
    int i = Math.abs(x) + Math.abs(y);
    if (i < Math.abs(poll.x) + Math.abs(poll.y)) {
      return true;
    }
    return false;
  }
  ...
  private static boolean crossedThePoint(Points n1, Points s1, Points e1, Points w1, int x, int y) {
    int i = Math.abs(x) + Math.abs(y);
    if (i < Math.abs(n1.x) + Math.abs(n1.y)
        && i < Math.abs(s1.x) + Math.abs(s1.y)
        && i < Math.abs(e1.x) + Math.abs(e1.y)
        && i < Math.abs(w1.x) + Math.abs(w1.y)) {
      return true;
    }
    return false;
  }

...
public class Solution {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= t; ++i) {
            ...
            message = new Solution().findMyString(inputs, false);
            if(message.indexOf("IMPOSSIBLE")!=-1) {
                message = new Solution().findMyString(inputs, true);
            }
            if(message.indexOf("IMPOSSIBLE")!=-1) {
                message = "IMPOSSIBLE";
            }
    ...
    private String findMyString(String[] inputs , boolean optimise) {
        ...
        for(String str: inputs) {
            ...
            if(cList.isEmpty()) {
                cList.add(arr);
                outputList.append("C");
            } else if(jList.isEmpty() && optimise){
                jList.add(arr);
                outputList.append("J");
            } else {
                for(int i=0; i<cList.size(); i++) {
                    int[] temp = cList.get(i);
                    if((arr[0] >= temp[0] && arr[0] < temp[1])  || (arr[1] > temp[0] && arr[1] <= temp[1]) || (arr[0] < temp[0] && arr[1] > temp[1])) {

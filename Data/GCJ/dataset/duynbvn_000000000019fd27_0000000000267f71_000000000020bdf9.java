...
public class Solution {
   ...
   public static void main(String[] args) {
     ...
     public static boolean canPut(int begin, int end, List<TimeDuration> list) {
        ...
        TimeDuration now = new TimeDuration(begin, end);
        for (TimeDuration timeDuration : list) {
            if (timeDuration.contain(begin)) return false;
            if (timeDuration.contain(end)) return false;
            if (now.contain(timeDuration.begin)) return false;
            if (now.contain(timeDuration.end)) return false;
        }
        return true;
    }

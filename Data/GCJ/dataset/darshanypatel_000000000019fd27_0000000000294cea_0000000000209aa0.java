...
public class Solution {
                ...
                if (matrix[i][j] == 0) {
                    for (int k = 1; k <= N; k++) {
                        if (!rows.get(i).contains(k) && !cols.get(j).contains(k)) {
                            matrix[i][j] = k;
                            List<Set<Integer>> rowsCopy = deepCopy(rows);
                            List<Set<Integer>> colsCopy = deepCopy(cols);
                            rowsCopy.get(i).add(k);
                            colsCopy.get(j).add(k);

                            if (tryFilling(matrix, rowsCopy, colsCopy)) {
                                return true;
                            }
                        }
                    }
    ...
    private static void getTracesRecursively(int n, int k, Set<List<Integer>> traces,
        ...
        if ((n - currentTrace.size()) * n < k) {
            return;
        }

        if ((n - currentTrace.size()) > k) {
            return;
        }

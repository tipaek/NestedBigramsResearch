...
public class Solution {
	...
	static String findSolutionDirection(char dir, int x, int y, int fromX, int fromY, int tiro) {
		...
		if(!verifyPower2(Math.abs(fromY-y),tiro)||!verifyPower2(Math.abs(fromX-x),tiro)) {
			return "IMPOSSIBLE";
		}
		/*else if(Math.abs(fromX-x)<Math.pow(2, tiro+1)&&Math.abs(fromY-y)<Math.pow(2, tiro+1)) {
	...
	static boolean verifyPower2(int value, int tiro) {
		while(tiro!=0) {
			if(value%Math.pow(2, tiro)!=0) {
				return false;
			}
			tiro--;
		}

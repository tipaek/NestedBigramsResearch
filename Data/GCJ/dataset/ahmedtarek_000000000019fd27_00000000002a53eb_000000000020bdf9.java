...
public class Solution {
	public static void main (String[] args) {
		...
		for (int i = 1; i <= t; ++i) {
			...
			int x = in.nextInt();
			int[][] answers = new int[x][2];
			int activityCount = 0;

			int[][] activitiesList = new int[x][3];
			for (int j = 0; j < x; j++) {
			...
			for (int j = 0; j < x; j++) {
				...
				if (cameronAvailable) {
					...
					answers[j] = new int[] {activityReference, 1};
				} else {
					for (int k = startTime; k < endTime; k++) {
					...
					if (jamieAvailable) {
						...
						answers[j] = new int[] {activityReference, 2};
					} else {
						impossible = true;
					}
				}
			...
			if (impossible) {
				...
				Arrays.sort(answers, new java.util.Comparator<int[]>() {
					public int compare(int[] a, int[] b) {
						return Integer.compare(a[0], b[0]);
					}
				});

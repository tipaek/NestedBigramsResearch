...
//      }
    ...
    for (int counter = 0, bit = 0; counter < THIRTY; counter++, bit *= 2) {
      ...
      if (xneedsbit) {
        ...
        int amountOther = weKnowNthIsSingleFilledButHowManyNoneAfter(counter, dirX, dirY);
        while (amountOther > 0) {
  ...
  private static int weKnowNthIsSingleFilledButHowManyNoneAfter(int pos, dir[] x, dir[] y) {
    pos++;
    int jeej = 0;
    while (pos < x.length) {
      if (x[pos] != dir.STILL || y[pos] != dir.STILL) {
        return jeej;
      }
      jeej++;
      pos++;

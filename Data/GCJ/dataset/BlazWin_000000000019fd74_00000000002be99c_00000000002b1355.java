            ...
            while (true) {
                ...
                for (SquareDance.Cell cand : cands) {
                    ...
                    if (cand.top != null) {
                        ...
                        if (!cand.top.removed && cand.top.isCand()) {
                            newCands.add(cand.top);
                        }
                    }
                    if (cand.bot != null) {
                        cand.bot.top = cand.top;
                        if (!cand.bot.removed && cand.bot.isCand()) {
                            newCands.add(cand.bot);
                        }
                    }
                    if (cand.left != null) {
                        cand.left.right = cand.right;
                        if (!cand.left.removed && cand.left.isCand()) {
                            newCands.add(cand.left);
                        }
                    }
                    if (cand.right != null) {
                        cand.right.left = cand.left;
                        if (!cand.right.removed && cand.right.isCand()) {
                            newCands.add(cand.right);
                        }
                    }
                }
                cands.clear();

...
public class Solution {
    ...
    //    long beginTime = System.nanoTime();
        ...
        for (int c = 0; c < testCases; c++) {
            ...
            sort(pattern);
String name = buildName(pattern);
            if(match(pattern,name)) solution[c] = name;
            else solution[c] = "*";
        }
    ...
    private static String buildName(String[] x) {
        ...
        Arrays.sort(x);
        String nameStartA=x[ultimul].substring(0, x[ultimul].indexOf('*'));
        String nameEndA=x[0].substring(x[0].indexOf('*')+1);

        String nameStart = nameStartL.startsWith(nameStartA) ? nameStartL : nameStartA;
        String nameEnd = nameEndL.endsWith(nameEndA) ? nameEndL : nameEndA;

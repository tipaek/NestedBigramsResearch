...
public class Solution {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= t; ++i) {
            ...
            for (int j = 0; j < N; j++) {
                ...
                events.add(endEvent);
                Activity activity = new Activity(j, startEvent, endEvent);
    ...
    private static String getOutput(List<Event> events, List<Activity> activities) {
        ...
        for (int i = 1; i < activities.size(); i++) {
            ...
            if (first.endEvent.time <= curr.startEvent.time) {
                curr.owner = first.owner;
                first = curr;
            } else {
                curr.owner = anotherOwner(first);
    ...
    private static char anotherOwner(Activity activity) {
        return activity.owner == 'C' ? 'J' : 'C';
    }

...
public class Solution {
		...
		String res = "";
		 for (int i = 1; i <= t; i++) {
			 ...
			 for(int time=0;time<1440;time++) {
				 ...
				 List<Job> jobsNow = jobsStarttingAtTimeT(jobs, time);
				 
				 for(Job currentJob : jobsNow) {
	...
	private static List<Job> jobsStarttingAtTimeT(List<Job> jobs,int t) {
		List<Job> jobAtCurrentTime = new ArrayList<>();
		for(Job job :jobs) {
			if(job.assigendTo == null && job.st == t) {
				jobAtCurrentTime.add(job);
			}
			if(job.st > t) {
				break;
			}
		}
		return jobAtCurrentTime;
	}

...
public class Solution {
	...
	public static void main (String [] arg) {
		...
		for (int ii = 1; ii<=TT; ++ii) {
			...
			int BIT = 32;
			while ( ((1L << BIT) > X) && ((1L << BIT) > Y)) BIT--;
			BIT++;
			String ans = recurse(X,Y,BIT);
	...
	public static String recurse(long X, long Y, int BIT) {
		if (BIT == 0) {
		...
		if (!H.containsKey(key)) {
			...
			if (Xt && Yt) {
				...
				String ans = recurse(X ^ T, Y, BIT-1);
				H.put(key, (ans== null) ? null : ans + "E");
			} else if (Yt) {
				String ans = recurse(X, Y ^ T, BIT-1);
				H.put(key, (ans == null) ? null : ans + "N");
			} else {
				String ans0 = recurse(X, Y, BIT-1);
				if (ans0 != null) {
					H.put(key, ans0);
				} else {
				 String ans1 = (X == 0) ? null : flip( recurse(T - X, Y, BIT-1), 'E', 'W');
				 String ans2 = null;
				 if (ans1 == null) {
					ans2 = flip( (Y == 0) ? null : recurse(X, T - Y, BIT-1), 'N', 'S');
					if (ans2 != null) {
						H.put(key, ans2 + "N");
					} else {
						H.put(key, null);

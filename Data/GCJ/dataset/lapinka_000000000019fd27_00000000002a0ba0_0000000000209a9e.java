...
public class Solution {//Code Jam quals 2020, 
	...
	public boolean playGame(int B) {
		...
		if (B==20) {
			...
			/*System.out.println("------------------------");
			for (int i=0; i<10; i++) System.out.print(exter[i]+" ");
			System.out.println("-> exterior");
			for (int i=0; i<10; i++) System.out.print(inter[i]+" ");
			System.out.println("-> interior");
			System.out.println("palind_e="+palind_e);
			System.out.println("antind_e="+antind_e);
			System.out.println("palind_i="+palind_i);
			System.out.println("antind_i="+antind_i);
			System.out.println("------------------------");*/
			...
			// Check what happens to interior
						if ((palind_i==-1)||(antind_i==-1)) {
							...
							int tmp=sc.nextInt();
							sc.nextLine();
							if (tmp!=inter[palind_i]) needC=true;
							//if (needC) System.out.println("Need C for interior");
							System.out.println(antind_i+6);
							tmp=sc.nextInt();
							sc.nextLine();
							if (((needC)&&(tmp==inter[antind_i])) || ((!needC)&&(tmp!=inter[antind_i]))) needR=true;
							//if (needR) System.out.println("Need R for interior");
							if (needC) applyC(inter);
							if (needR) applyR(inter);
							//for (int i=0; i<10; i++) System.out.print(inter[i]+" ");
							//System.out.println("-> new interior");
						}
						//produce result
						StringBuilder answer = new StringBuilder();

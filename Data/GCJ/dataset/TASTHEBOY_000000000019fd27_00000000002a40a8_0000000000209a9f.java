      ...
      // Scanner in= new Scanner(new BufferedReader(new InputStreamReader(System.in)));
	   Scanner in= new Scanner(System.in);
		...
		for(int j=0; j<S.length(); j++) {
			curr=S.charAt(j);
			iteration=Character.getNumericValue(curr) -1;
			modified_curr=(char)iteration;
			  ...
			  //prev=curr;
				...
				if(S.length()>1) {
					if(j==0 && S.length()>1) {
						...
						ClosingBrackets(modified_curr, parentheses);
						String K=getRemaining(parentheses);
						tmp=tmp+C+OpenningBrackets(modified_curr)+curr.toString()+K;
					}
					else if(curr !=prev&& prev !='0' && prev !=' ' && j!=S.length()-1&& parentheses.size()>=1) {
						int p=Character.getNumericValue(prev);
						String C=CloseThis(p-1,parentheses);
						ClosingBrackets(modified_curr, parentheses);
						tmp=tmp+C+OpenningBrackets(modified_curr)+curr.toString();

	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		while(testCase<=t) {
			...
			for(int i=0;i<n;i++) {
				...
				pair1.add( new Pair(Integer.parseInt(s[0]),Integer.parseInt(s[1]),i));
			}
			...
			//after assigining task to c and j rearrange according to original order.
				...
				/*
				 * char[] ch = new char[n]; for(int i=0;i<n;i++) { Pair p = pair.get(i); for(int
				 * j=0;j<n;j++) { Pair p1 = pair1.get(j); if((ch[j]=='C'||ch[j]=='J')) continue;
				 * if(p1.s==p.s && p.e==p1.e) { ch[j]=s.charAt(i); break; } } }
				 * 
				 * for(int i=0;i<n;i++) { sb1.append(ch[i]); }
				 */

...
public class Solution {
    ...
    public static void main(String[] args) {
        ...
        for(int c = 1; c <= cases; c++) {
            ranges.clear();
            String ans = "Case #" + c + ": ";
            in.nextLine(); //Get rid of the bounds line
            ArrayList<String> nums = new ArrayList<>();
            String type = "";
            HashMap<String,Integer> occurrences = new HashMap<>();
            ArrayList<String> cantbezero = new ArrayList<>();
            for(int i=0; i<10000; i++) {
                ...
                if(!query.equals("-1")) {
                    ...
                    String[] result = info[1].split("");
                    for (String s : result) {
                        if (!nums.contains(s)) nums.add(s);

                        if (occurrences.containsKey(s)) {
                            int occur = occurrences.get(s);
                            occur += 1;
                    ...
                    if(!cantbezero.contains(result[0])) cantbezero.add(result[0]);
                }
            }
            ...
            if(type.equals("known")) {
                ...
                for(int k=0; k<nums.size(); k++) {
                    if(!cantbezero.contains(nums.get(k))) {
                        ans += nums.get(k);
                        nums.remove(k);
                        break;
                    }
                }
                nums.sort(Comparator.comparingInt(occurrences::get));

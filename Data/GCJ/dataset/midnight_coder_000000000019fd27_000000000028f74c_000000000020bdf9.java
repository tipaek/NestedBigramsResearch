...
public class Main {
    ...
    private static void solve()
    {
        ...
        Stack<Integer> Jamie= new Stack<>();
        Stack<Integer> Cameron= new Stack<>();
        ...
        for(int i=0;i<matSorted.length;i++)
        {
            ...
            if(i<matSorted.length-1 && doesOverlap(matSorted[i],matSorted[i+1]))
            {
                if(person=='J')
                {
                    ...
                    if(!Cameron.isEmpty() && doesOverlap(Cameron.peek(),mapSorted[i+1])){
                        impossible=true;
                        break;
                    }
                }
                else
                {
                    ...
                    person= getPerson(person);
                    if(!Jamie.isEmpty() && doesOverlap(Jamie.peek(),mapSorted[i+1])){
                        impossible=true;
                        break;
                    }
                }

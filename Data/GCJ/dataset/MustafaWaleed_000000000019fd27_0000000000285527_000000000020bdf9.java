...
class ParentingPartneringReturns {
    ...
    void setSchedule() {
        ...
        for (int i = 0; i < numberOfActivities; i++) {
            ...
            if(cameronSchedule[startTime] == false ) {
                setCameronScheduleFull(activities[i].start,activities[i].end);
                schedule.append(CAMERON);
            }else if(jamieSchedule[startTime] == false) {
                setJamieScheduleFull(activities[i].start,activities[i].end);

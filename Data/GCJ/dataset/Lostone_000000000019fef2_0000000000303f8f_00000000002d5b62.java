...
public class Solution {
    ...
    while (!p.isEmpty()) {
      ...
      while (!q.isEmpty()) {
        ...
        if (t != null) {
          ...
          if (crossedThePoint(count)) {
            System.out.println("IMPOSSIBLE");
            return;
          }
          if (!pointsAlreadyVisited.contains(n1) && withinRange(n1)) {
            p.add(t1);
            pointsAlreadyVisited.add(n1);
          }
          if (!pointsAlreadyVisited.contains(s1)&&withinRange(s1)) {
            p.add(t2);
            pointsAlreadyVisited.add(s1);
          }
          if (!pointsAlreadyVisited.contains(e1)&&withinRange(e1)) {
            p.add(t3);
            pointsAlreadyVisited.add(e1);
          }
          if (!pointsAlreadyVisited.contains(w1)&&withinRange(w1)) {
            p.add(t4);
  ...
  private static boolean withinRange(Points n1) {
    if(n1.x<=MAX && n1.y<=MAX){
      return true;
    }
    return false;
  }

  private static boolean crossedThePoint(int x) {
    if (x >= 30) {
      return true;
    }
    return false;

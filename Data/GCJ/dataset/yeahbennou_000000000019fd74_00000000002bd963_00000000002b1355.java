...
class Project extends Solution
{
    ...
    {
        int a, b;
        public Point(int c, int d) { a = c; b = d; }
    }
    class Node
    {
        Point[] adj;
        public Node(Point... adjj)
        {
            adj = adjj;
        }
    }
    ...
    void solve()
    {
        ...
        for(int i = 0; i < R; i++)
            ...
            {
                score += grid[i][j] = ri();
                if(i != 0) adj[i][j][UP] = (new Point(i - 1, j));
                if(j != 0) adj[i][j][LEFT] = (new Point(i, j - 1));
                if(i != R - 1) adj[i][j][DOWN] = (new Point(i + 1, j));
                if(j != C - 1) adj[i][j][RIGHT] = (new Point(i, j + 1));
            }
        Queue<Point> dead = new LinkedList<>();
        for(int i = 0; i < R; i++)
            for(int j = 0; j < C; j++)
            {
                if(willDie(i, j))
                    dead.add(new Point(i, j));
            }
        boolean[][] pred = new boolean[R][C];
        tot = score;
        if(dead.isEmpty())
        {
            pln(tot);
        ...
        Queue<Node> old = new LinkedList<>();
        for(Point p: dead)
        {
            score -= grid[p.a][p.b];
            grid[p.a][p.b] = -INF;
            pred[p.a][p.b] = true;
            old.add(new Node(adj[p.a][p.b][0], adj[p.a][p.b][1], adj[p.a][p.b][2], adj[p.a][p.b][3]));
        }
        for(Point p: dead)
            update(p);
        tot += score;
        for(int efasdf = 0; efasdf < R * C; efasdf++)
        {
            int sz = dead.size();
            for(int elm = 0; elm < sz; elm++)
            {
                Point px = dead.poll();
                int a = px.a, b = px.b;
                for(Point p: old.poll().adj)
                    if(p != null && !pred[p.a][p.b])
                        if(willDie(p.a, p.b))
                        {
                            pred[p.a][p.b] = true;
                            dead.add(p);
            ...
            for(Point p: dead)
            {
                score -= grid[p.a][p.b];
                grid[p.a][p.b] = -INF;
                old.add(new Node(p, adj[p.a][p.b][0], adj[p.a][p.b][1], adj[p.a][p.b][2], adj[p.a][p.b][3]));
            }
            for(Point p: dead) update(p);
            tot += score;
        }
        pln(tot);
    }
    void update(Point p)
    {
        Point le = adj[p.a][p.b][LEFT], ri = adj[p.a][p.b][RIGHT], up = adj[p.a][p.b][UP], dn = adj[p.a][p.b][DOWN];
        if(le != null) adj[le.a][le.b][RIGHT] = ri;
        if(ri != null) adj[ri.a][ri.b][LEFT] = le;
        if(up != null) adj[up.a][up.b][DOWN] = dn;
        if(dn != null) adj[dn.a][dn.b][UP] = up;
    }
    boolean willDie(int a, int b)
    {
        int tot = 0, cnt = 0;
        for(Point p: adj[a][b])
        {

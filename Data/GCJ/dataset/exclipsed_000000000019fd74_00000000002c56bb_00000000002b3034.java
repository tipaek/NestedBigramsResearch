...
public class Solution {
	...
	public static void main(String[] args) throws Exception {
		...
		loop: for(int t = 1; t <= T; ++t) {
			...
			for(int i = 0; i != N; ++i) {
				int ind = 0;
				while(ind < strings[i].length() && strings[i].charAt(ind++) != '*') {
					if(ind > pref.length()) {
			...
			s = suff + s;
			for(int i = 0; i != N; ++i) {
				int begin = 0; int end = strings[i].length() - 1;
				while(strings[i].charAt(begin) != '*') begin++;
				while(strings[i].charAt(end) != '*') end--;
				if(begin >= end) {
					strings[i] = "";
					amntEmpty++;
				}
				else strings[i] = strings[i].substring(begin + 1, end);
			}
			//first round end
			while(amntEmpty < N) {
				for(int i = 0; i != N; ++i) {
					...
					if(countAsterisks == 0) {
						...
						int idx = 0;
						while(strings[i].charAt(idx) != '*') {
							pre += strings[i].charAt(idx++);
						}
						idx = strings[i].length() - 1;
						while(strings[i].charAt(idx) != '*') {
							suf = strings[i].charAt(idx--) + suf;
						}
						p.append(pre); s = suf + s;
						int begin = 0; int end = strings[i].length() - 1;
						while(strings[i].charAt(begin) != '*') begin++;
						while(strings[i].charAt(end) != '*') end--;
						if(begin >= end) {

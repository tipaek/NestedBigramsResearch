...
//			System.out.println(inputText);
			...
			extracted(inputText, masterBuffer);
			StringBuffer bruteReverseBuffer = new StringBuffer();
			extracted(new StringBuffer(inputText).reverse().toString(), bruteReverseBuffer);
	...
	private static String printReverse(StringBuffer bruteReverseBuffer) {
		for(int i=0;i<bruteReverseBuffer.length();i++) {
			if(bruteReverseBuffer.charAt(i) == OPEN_CH)
				bruteReverseBuffer.setCharAt(i, CLOSE_CH);
			else if(bruteReverseBuffer.charAt(i) == CLOSE_CH)
				bruteReverseBuffer.setCharAt(i, OPEN_CH);
		}
		return bruteReverseBuffer.reverse().toString();
	}

	private static void extracted(String inputText, StringBuffer masterBuffer) {
		try {

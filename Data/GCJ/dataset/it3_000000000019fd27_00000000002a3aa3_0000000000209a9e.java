...
public class Solution {
    final static String filePrefix = "D";
...
//        xx();
    ...
    void solve() throws IOException {
        ...
        for (int i = 1; i <= 150; i+=2) {
            if (known >= B/2) {
                ...
                String s = LINE();
                if (s.charAt(0)=='N') {
                    System.err.println("ERROR: wrong answer");
                    System.exit(1);
                }
                return;
            }
            if (i > 1 && i % 10 == 1) {
                ...
                boolean okSame = query(same) == a[same];
                boolean okDiff = query(diff) == a[diff];
                if (!okSame) {
                    if (okDiff) {
                        a = rev(comp(a));
                    } else {                        
                ...
                for (int idx : new int[] {known, B-1-known}) {
                    a[idx] = query(idx);
                }
                known++;
            }
    ...
    char query(int idx) throws IOException {
        out.println(idx+1);
        String s = LINE();
        if (s.charAt(0) == 'N') {
            System.err.println("ERROR at query "+idx);
            System.exit(1);
        }
        return s.charAt(0);
    }
    ...
    void xx() throws IOException {
        ...
        for (int i = 1; i <= 150; i++) {
            ...
            if (unsure) {
                char[] aComp = comp(a);
                char[] aRev = new StringBuffer(new String(a)).reverse().toString().toCharArray();
                char[] aCompRev = comp(aRev);
    ...
    private static char[] rev(char[] a) {
        return new StringBuffer(new String(a)).reverse().toString().toCharArray();
    }
    private static char[] comp(char[] a) {

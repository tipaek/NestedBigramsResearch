...
class Resolver{
	...
	public String computeAnswear() {
		...
		recursiveSolver.checkForSolution( 1, 1, testCase.X, testCase.Y, testCase.moves.size(), 0);
...
class RecursiveSolver{
	...
	public void checkForSolution(int myX, int myY, int X, int Y, int numberOfMovesLeft, int numberOfMovesMade) {
		...
		if (myX == X && myY == Y) {
			...
			checkForSolution(myX2, myY, X, Y, numberOfMovesLeft, numberOfMovesMade);
			checkForSolution(myX, myY2, X, Y, numberOfMovesLeft, numberOfMovesMade);
			checkForSolution(myX1, myY, X, Y, numberOfMovesLeft, numberOfMovesMade);
			checkForSolution(myX, myY1, X, Y, numberOfMovesLeft, numberOfMovesMade);
			checkForSolution(myX, myY, X, Y, numberOfMovesLeft, numberOfMovesMade);
		}
	}
}

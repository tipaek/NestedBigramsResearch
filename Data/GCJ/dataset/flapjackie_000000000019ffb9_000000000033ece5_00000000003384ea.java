...
public class Solution {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int ca = 1 ; ca <= T ; ca++) {
			L = new BigInteger(sc.next()); R = new BigInteger(sc.next());
			...
			if (sum(high).compareTo(stacks[0]) == 1) {
			...
			//works(people, stacks[0], stacks[1]);
	...
	static boolean works(BigInteger people, BigInteger ansL, BigInteger ansR) {
		BigInteger ppl = ZERO;
		
		BigInteger curr = ONE;
		
		while (L.compareTo(curr) >= 0 || R.compareTo(curr) >= 0) {
			ppl = ppl.add(ONE);
		...
		if (people.compareTo(ppl) == 0 && ansL.compareTo(L) == 0 && ansR.compareTo(R) == 0) {
			return true;
		}
		

		System.out.printf("%s %s got %s %s %s should be %s %s %s\n", saveL, saveR, people, ansL, ansR, ppl, L, R);
		return false;
	}

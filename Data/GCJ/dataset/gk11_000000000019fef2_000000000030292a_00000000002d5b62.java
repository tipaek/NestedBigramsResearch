...
//    }
//
//    public static void test(String t,int X, int Y) {
        int d0 = abs(X)+abs(Y);
        int steps = pow2Above(d0);
        List<Point> q = new LinkedList<>();
        while (true) {
            Point start = new Point(X,Y,1<<(steps-1),new LinkedList<>());
            if (withinReach(start)) {
                q.add(start);
                steps++;
            }
            break;

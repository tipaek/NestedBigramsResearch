...
public class Solution {
            ...
            if (B > 10) {
                ...
                while (idx < arr.length / 2) {
                    ...
                    if (diff[0][0] != -1 && diff[1][0] != -1) {
                        ...
                        list.add(diff[0][0] == -1 ? diff[1][0] : diff[0][0]);
                        list.add(diff[1][0] == -1 ? diff[0][0] : diff[1][0]);
                    }
    ...
    public static void check(int[][] diff, int[] arr, int s, int e) {
        if (diff[0][0] != -1 && diff[1][0] != -1)
            return;
    ...
    public static void checkAndPerform(int[][] diff, int[] arr, List<Integer> values) {
        ...
        if (values.get(0) != arr[diff[0][0]] && values.get(1) != arr[diff[1][0]]) {
            complement(arr);
        } else if (values.get(0) == arr[diff[0][1]] && values.get(1) == arr[diff[1][1]]) {
            reverse(arr);
        } else if (values.get(0) != arr[diff[0][1]] && values.get(1) != arr[diff[1][1]]) {
            reverse(arr);
            complement(arr);

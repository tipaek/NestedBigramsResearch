	...
	// write your code here
        ...
        for(i = 1; i <= T; i++) {
            ...
            for(j = 0; j < N; j++) {
                ...
                num = S * maxV + E;
                sortQ.add(num);
                if(!indexMap.containsKey(num)) indexMap.put(num, new ArrayList<>());
                indexMap.get(num).add(j);
            }
            ...
            while (!sortQ.isEmpty()) {
                ...
                S = num / maxV;
                E = num % maxV;
                j = indexMap.get(num).get(indexMap.get(num).size() - 1);
                indexMap.get(num).remove(indexMap.get(num).size() - 1);

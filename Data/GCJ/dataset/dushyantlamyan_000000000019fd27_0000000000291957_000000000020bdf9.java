...
public class Solution {
    public static void main(String[] args) {
	  ...
	  for (int j = 0; j < countN; j++) {
          ...
          for (int j = 0; j < countN; j++) {
              for(int k = 0; k < 2; k++){
                  if(j==0 && k==1){
                     ...
                     if(current == true && a[j][0] >= chighest){
                       ...
                       chighest = a[j][1];
                         ...
                         if(a[j][1]<=jlowest || a[j][0] >=jhighest){
                           ...
                           output.append('J');
                           jhighest = a[j][1];
                     }else if(current == false && (a[j][0] <= clowest || a[j][0] >= chighest)){
                         if(a[j][1] <= clowest || a[j][0] >= chighest){
                           output.append('C');
                           current = true;
                           clowest = a[j][0];
                           chighest = a[j][1];
                         }else if(a[j][0] <= jlowest || a[j][0] >= jhighest){
                           output.append('J');
                           current = false;
                           jlowest = a[j][0];
                           jhighest = a[j][1];
                     ...
                     }else if(current == false && (a[j][0] <= jlowest || a[j][0] >= jhighest)){
                         if(a[j][1] <= clowest || a[j][0] >= chighest){
                           output.append('C');
                           current = true;
                           clowest = a[j][0];
          ...
          //System.out.println("Case ::: " + output.toString());

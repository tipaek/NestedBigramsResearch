...
public class Solution {
    ...
    static class Expogo {
        ...
        public void solve(int testNumber, FastInput in, FastOutput out) {
            ...
            long x = Math.abs(xx);
            long y = Math.abs(yy);
            if (isPossible(x, y) && (lowerset(x) > 0 || lowerset(y) > 0)) {
                out.println(findAnswer("", 0, x, y, xx < 0, yy < 0));
            } else if (lowerset(x) > 0 && isPossible((x + 1) | 1, y)) {
                out.println(findAnswer((xx < 0 ? "E" : "W"), 1, x + 1, y, xx < 0, yy < 0));
            } else if (lowerset(y) > 0 && isPossible(x, (y + 1) | 1)) {
                out.println(findAnswer((yy < 0 ? "N" : "S"), 1, x, y + 1, xx < 0, yy < 0));
            } else {
                out.println("IMPOSSIBLE");
            }
        }

        public String findAnswer(String init, int start, long x, long y, boolean xNeg, boolean yNeg) {
            StringBuilder out = new StringBuilder();
            if (start == 1) out.append(init);
            for (int i = start; x > 0 || y > 0; i++) {
                if (Bits.bitAt(x | y, i) == 1) {
                    if (Bits.bitAt(x, i) == 1) {
                        out.append(xNeg ? "W" : "E");
                    } else {
                        out.append(yNeg ? "S" : "N");
            ...
            for (int i = 0; i < out.length(); ) {
                if (out.charAt(i) == '#') {
                    ...
                    out.setCharAt(i - 1, comp.get(c));
                    int j = i;
                    while (j < out.length() && out.charAt(j) == '#') j++;
                    while (i < j - 1) out.setCharAt(i++, comp.get(c));
                    out.setCharAt(i++, c);
                } else i++;
            }
            return out.toString();
        ...
        public boolean isPossible(long x, long y) {
            for (int i = 0; x > 0 || y > 0; i++) {
                if (Bits.bitAt(x, i) == Bits.bitAt(y, i) && Bits.bitAt(x, i) == 1) return false;
                x = Bits.setBit(x, i, false);
                y = Bits.setBit(y, i, false);
            }
            return true;

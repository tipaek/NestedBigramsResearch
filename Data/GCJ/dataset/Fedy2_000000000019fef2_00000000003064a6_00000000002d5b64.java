...
public class Solution {
	...
	private static List<Move> find(int[] stack) {
		...
		while(!q.isEmpty()) {
			...
			int[] mstack = Arrays.copyOf(stack, stack.length);
			...
			for (int i = moves.size() - 1; i >= 0; i--) {
				Move move = moves.get(i);
				swapInPlace(mstack, move.a, move.b);
			}
			...
			String key = Arrays.toString(mstack);
			
			if (saw.contains(key)) continue;
			saw.add(key);
			
			for (int a = 1; a <= stack.length; a++) {
				for (int b = 1; b <= stack.length - a; b++) {
					//int[] ns = swap(m.stack, a, b);
					Move nm = new Move(a, b, m);
					q.add(nm);
				}
			}
		}

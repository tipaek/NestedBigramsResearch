            ...
            for (int j = 0; j < n; ++j) {
                if(j==0){
                    ...
                    for (int k = 0; k < j;k++){
                        ...
                        if((timetable[j][0]<=cp[k][0] && timetable[j][1]> cp[k][0]) || (timetable[j][0]<cp[k][1] && timetable[j][1]>= cp[k][1])){
                            break;
                        }
                        if(k+1==j){
                            cp[j][0]=timetable[j][0];
            ...
            for (int j = 0; j < n; ++j) {
                if(cp[j][0]==0&&cp[j][1]==0) {
                    for (int k = 0; k < j; k++) {
                        ...
                        if((timetable[j][0]<=jp[k][0] && timetable[j][1]> jp[k][0]) || (timetable[j][0]<jp[k][1] && timetable[j][1]>= jp[k][1])){
                            jpcounter=-100;
                            break;
                        }
                        if (k + 1 == j) {

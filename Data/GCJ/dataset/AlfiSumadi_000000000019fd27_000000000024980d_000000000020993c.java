    ...
    private static void solve(long x) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < n; j++) {
                ...
                if (i == j) k += currentVal;
                digitsFoundInRow.putIfAbsent(currentVal, 1l);
                digitsFoundInRow.put(currentVal, digitsFoundInRow.get(currentVal) + 1);
                digitsFoundInColumns.get(j).putIfAbsent(currentVal, 1l);
                digitsFoundInColumns.get(j).put(currentVal, digitsFoundInRow.get(currentVal) + 1);
            }

...
public class Solution {
    ...
    private static boolean startRound(Scanner scanner, int B, PrintWriter pw) {
        ...
        int index = 1;
        communicate(scanner, pw, bits, index, false);
        index++;

        for (int i = 2; i < 32; i++) {
            communicate(scanner, pw, bits, index, true);
        }

        communicate(scanner, pw, bits, B, false);
    ...
    private static void communicate(Scanner scanner, PrintWriter pw, int[] bits, int index, boolean ignore) {
        pw.println(index);
        pw.flush();
        if (!ignore) {

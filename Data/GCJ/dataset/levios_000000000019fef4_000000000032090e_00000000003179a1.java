...
public class Solution {
    ...
    private static void solve() {
        ...
        List<Integer> Qs = new ArrayList<>(); // query: in base 10 using digits 0 through 9, as usual
        List<String> Rs = new ArrayList<>(); // response
        char[] solution = new char[10];
        Map<Integer, Set<Character>> cannotBe = new HashMap<>();
...
//            canBe.put(i, new HashSet<>());
        ...
        for (int i = 0; i < 10000; i++) {
            ...
            if (r.length() == length) {
                ...
                cannotBe.get(0).add(firstChar);
            }
        }

        // 0
        Set<Character> zero = cannotBe.get(0);
        for (char c : chars) {
            if (!zero.contains(c)) {
                solution[0] = c;
                chars.remove(c);

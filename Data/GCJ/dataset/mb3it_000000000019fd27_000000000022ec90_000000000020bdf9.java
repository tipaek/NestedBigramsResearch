...
public class Solution {
	...
	private static String scheduleActivity(int [][] taskArray, int N){
		...
		for (int i = 0; i < taskArray.length; i++) {
			if(addActivity(cameronFreeSlot, taskArray[i][0], taskArray[i][1])){
				schedule += "C";
			}
			else if(addActivity(jamieFreeSlot, taskArray[i][0], taskArray[i][1])){
				schedule += "J";
			}
			else{
				return "IMPOSSIBLE";
	...
	private static boolean addActivity(TreeMap<Integer, Integer> freeSlotMap, int taskStart, int taskEnd){
		int S = 0, E = 0;
		...
		for(Map.Entry<Integer, Integer> slot : freeSlotMap.entrySet()) {
			...
			if(S <= taskEnd){
				if(S <= taskStart && taskEnd <= E){
					foundSlot = true;
					if(taskEnd < E){
						freeSlotMap.put(taskEnd, E);
					}
					break;
				}
			}
		...
		if(foundSlot){
			if(S == taskStart){
				freeSlotMap.remove(taskStart);
			}
			else{
				freeSlotMap.put(S, taskStart);
			}
			return true;
		}	

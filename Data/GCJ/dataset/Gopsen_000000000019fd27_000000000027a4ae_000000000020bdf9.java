    ...
    public static void main(String... args) {
        ...
        for (int i=1; i<=testCasesCount; i++) {
            ...
            for (int k=0; k<activityCount; k++) {
                if (jLastIndex != -1) {
                    if (intervals[k].isIntervalOverlapping(intervals[jLastIndex].lowerLimit, intervals[jLastIndex].upperLimit)) {
                        if (cLastIndex != -1) {
                            if (intervals[k].isIntervalOverlapping(intervals[cLastIndex].lowerLimit, intervals[cLastIndex].upperLimit)) {
                                chars = null;
                                break;
                            } else {
                                cLastIndex = k;
    ...
    private static class Interval implements Comparable<Interval> {
        ...
        private boolean isIntervalOverlapping(int lowerLimit, int upperLimit) {
            return (lowerLimit > this.lowerLimit && lowerLimit < this.upperLimit) ||
                    (lowerLimit < this.lowerLimit && upperLimit > this.lowerLimit);
        }

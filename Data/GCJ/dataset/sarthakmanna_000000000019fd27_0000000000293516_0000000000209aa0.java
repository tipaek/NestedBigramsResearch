...
class Solver {
    ...
    void solve() throws Exception {
        ...
        for (int tce = 1; tce <= tc; tce++) {
            hp.printsp("Case #" + tce + ":");
            samples = new TreeSet<>((a, b) -> Arrays.toString(a).compareTo(Arrays.toString(b)));

            N = hp.nextInt(); K = hp.nextInt() - N;
            int[] sample = new int[N];
            for (i = 0; i < N; ++i) sample[i] = i;

            while (samples.size() < hp.getFactorial(N)) {
                hp.shuffle(sample);
                samples.add(sample.clone());
            }

            present = new BitSet[N];
            for (i = 0; i < N; ++i) present[i] = new BitSet();

            matrix = new int[N][];
            if (recurse(0, 0)) {
                hp.println("POSSIBLE");
                for (int[] row : matrix) {
                    for (int itr : row) hp.printsp(itr + 1);
                    hp.println();
                }
            } else {
                hp.println("IMPOSSIBLE");
            }
        }
    ...
    boolean recurse(int row, int trace) {
        if (trace > K) return false;
        else if (row >= N) return trace == K;

        for (int[] sample : samples) {
            if (canFit(sample)) {
                matrix[row] = sample.clone();
                for (int j = 0; j < N; ++j) present[j].set(sample[j]);

                if (recurse(row + 1, trace + matrix[row][row])) {
                    return true;
                }

                for (int j = 0; j < N; ++j) present[j].clear(sample[j]);
                matrix[row] = null;
            }
        }
        return false;
    ...
    boolean canFit(int[] ar) {
        for (int i = 0; i < N; ++i) if (present[i].get(ar[i])) return false;
        return true;
    }
}

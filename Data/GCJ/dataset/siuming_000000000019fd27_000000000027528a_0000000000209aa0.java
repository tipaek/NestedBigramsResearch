...
public class Solution {
    ...
    private static boolean solveByBackTracking(int[][] matrix, int trace) {
        int dim = matrix.length;
        int trace2 = 0;
        boolean checkTrace = true;
        for (int i = 0; i < dim; i++) {
            if (matrix[i][i] == 0) {
                checkTrace = false;
                break;
        ...
        for (int num = 1; num <= dim; num++) { 
            if (isValidMove(matrix, row, col, num)) { 
                matrix[row][col] = num; 
                if (solveByBackTracking(matrix, trace)) { 
                    return true; 
                } else {
                    matrix[row][col] = 0;
                } 
    ...
    private static int[][] solve(int dim, int trace) {
        ...
        if (solveByBackTracking(retMatrix, trace)) {
            return retMatrix;
        }
        return new int[0][0];
    }

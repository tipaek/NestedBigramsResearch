...
class Solution
{
        ...
        for(v=1;v<=t;v++)
        {
            ...
            if(sum2==sum)
            {
                ...
                for(i=0;i<34;i++)
                    if(s1[i]!=' ') st+=s1[i];
                    st=st.trim();
                 if(checker(st,x,y)) sb.append("Case #"+v+": "+st+"\n");
                 else sb.append("Case #"+v+": IMPOSSIBLE\n");
                 continue;
            }
            else
            {
    ...
    static boolean checker(String s,int x,int y)
    {
        int i; long p=1,xp=0,yp=0;
        ...
        if((int)xp==x && (int)yp==y) return true;
        else return false;
    }
}

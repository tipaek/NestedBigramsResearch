...
public class Solution {
    ...
    static class Indicium {
        public void solve(int testNumber, InputReader in, PrintWriter out) {
            ...
            int[][] res = new int[n][n];
            boolean valid = helper(res, 0, k, n, new boolean[n][n], new boolean[n][n]);
            if (!valid) {
        ...
        boolean helper(int[][] arr, int i, int total, int n, boolean[][] row, boolean[][] col) {
            if (total == 0 && i == n) {
            ...
            int start = Math.max(1, total - n * (n - i - 1));
            int end = Math.min(n, total - (n - i - 1));
            for (int num = start; num <= end; num++) {
                arr[i][i] = num;
                row[i][num - 1] = true;
                col[i][num - 1] = true;
                if (helper(arr, i + 1, total - num, n, row, col)) {
                    return true;
                }
                row[i][num - 1] = false;
                col[i][num - 1] = false;

...
public class Solution {
    ...
    public static void main(String[] args) {
        ...
        for (int m = 0; m < testCases; m++){
            ...
            int currOpenings = 0;
            int[] noOfOpens = new int[chars.length];
            for (int i = 0; i < chars.length; i++){
                noOfOpens[i] = Integer.parseInt(chars[i]+"");
                if (currOpenings<=noOfOpens[i]){
                    for (int j = currOpenings; j < noOfOpens[i]; j++){
                        System.out.print(open);
                        currOpenings = noOfOpens[i];
                    }
                }
                else {
                    for (int j = currOpenings; j > noOfOpens[i]; j--){

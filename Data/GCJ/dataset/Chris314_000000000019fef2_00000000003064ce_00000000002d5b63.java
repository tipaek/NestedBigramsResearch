...
public class Solution {
  public static void solve(Scanner input, int A, int B, int upperLeft[], int bottomRight[], int size) {
     ...
     if (s.equals("HIT")) {
         solve(input, A, B, upperLeft, new int[]{  upperLeft[0] + (size+1)/2, upperLeft[0] - (size+1)/2}, (size+1)/2);
         return;
     }
     ...
     if (s.equals("HIT")) {
         solve(input, A, B, new int[]{upperLeft[0], bottomRight[0] + (size+1)/2}, new int[] {upperLeft[0] + (size+1)/2,bottomRight[1]}          ,(size+1)/2);
         return;
     }

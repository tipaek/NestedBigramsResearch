...
public class Solution {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < T; i++) {
            ...
            Collections.sort(activities, Comparator.comparing(integerIntegerSimpleEntry -> (integerIntegerSimpleEntry.getKey()+integerIntegerSimpleEntry.getValue())/2));
    ...
    private static boolean getNumber(List<AbstractMap.SimpleEntry<Integer,Integer>> s, int index,List<Integer> indexesC,List<Integer> indexesL) {
        ...
        for(int i=0;i<indexesC.size();i++){
            if(isOverlapping(s.get(index), s.get(indexesC.get(indexesC.size()-1-i)))){
                goFurther = false;
                break;
            }
        }
        ...
        for(int i=0;i<indexesL.size();i++){
            if(isOverlapping(s.get(index), s.get(indexesL.get(indexesL.size()-1-i)))){
                goFurther = false;
                break;
            }
        }

...
public class Solution implements Runnable {
    ...
    public static class Solver extends  SolverAbstract{
        ...
        protected void solve() throws IOException {
            // try to equalize first
            long l = L;
            long r = R;
            long steps = steps_initial(Math.abs(L-R));
            long removed = steps * (steps+1) / 2;
            if (l>=r){
                l-=removed;
            }else{
                r-=removed;
            ...
            // l >= r; i    #start with L,R,L,R etc

            long steps_L = steps_possible(l, i+1);
            long steps_R = steps_possible(r, i+2);
            long bi_steps = Math.min(steps_L, steps_R);
            l -= compute_removed_items(i+1, bi_steps);
            r -= compute_removed_items(i+2, bi_steps);
            // try L
            i += bi_steps * 2;
        ...
        long steps_initial(long delta){
            return (long) Math.floor( Math.sqrt(2*delta+1.0/4) - 1.0/2);
        }

        long steps_possible(long capacity, long start){
            double tmp = (start-1) / 2.0;
            long steps = (long) Math.floor( Math.sqrt(tmp * tmp + capacity) - tmp);
            return Math.max(steps, 0);
        }

        long compute_removed_items(long start, long steps){
            return start * steps + steps*(steps-1);
        }

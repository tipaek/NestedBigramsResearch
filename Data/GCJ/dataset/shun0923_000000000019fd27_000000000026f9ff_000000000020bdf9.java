...
public void solve() {
	...
	for(int query = 0; query < q; query ++) {
		...
		int num = ni();
		int a[][] = ni(num, 2);
		long sum[] = new long[24 * 60 + 1];
		for(int i = 0; i < num; i ++) {
			sum[a[i][0]] ++;
			sum[a[i][1]] --;
		}
		sum = sums(sum);
		if(max(sum) >= 3) { prtln("IMPOSSIBLE"); continue; }
		int a2[][] = new int[num][4];
		for(int i = 0; i < num; i ++) {
			a2[i][0] = a[i][0];
			a2[i][1] = a[i][1];
			a2[i][2] = i;
		...
		Arrays.sort(a2, (sort1, sort2) -> Integer.compare(sort1[1], sort2[1]));
		Arrays.sort(a2, (sort1, sort2) -> Integer.compare(sort1[0], sort2[0]));
		int r = -1;
		for(int i = 0; i < num; i ++) {
			if(a2[i][0] >= r) {
				a2[i][3] = 1;
		...
		Arrays.sort(a2, (sort1, sort2) -> Integer.compare(sort1[2], sort2[2]));
		for(int i = 0; i < num; i ++) {
			prt(a2[i][3] == 0 ? 'J' : 'C');
		}
		prtln();

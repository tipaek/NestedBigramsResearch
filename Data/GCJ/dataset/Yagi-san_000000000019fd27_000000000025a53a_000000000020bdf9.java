...
public class Solution {
	...
	public boolean estLibre(int heureDebut, int heureFin) {
		for(int i=0; i<listBegin.size(); i++) {
			if(heureDebut>listBegin.get(i) && heureDebut<listEnd.get(i)) {
				return false;
			}
			if(heureFin>listBegin.get(i) && heureFin<listEnd.get(i)) {
				return false;
			}
			if(heureDebut<=listBegin.get(i) && heureFin>=listEnd.get(i)) {
				return false;
			}
		}
		return true;

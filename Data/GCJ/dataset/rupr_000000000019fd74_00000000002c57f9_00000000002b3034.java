    ...
    private static class Finder {
        ...
        private void find(String current, int[] pointers) {
            ...
            for (int i = 0; i < pointers.length; i++) {
                ...
                while(index < patterns[i].length) {
                    if (patterns[i][index] == '*') {
                        if (maxA < patterns[i].length - pointers[i]) {
                            maxA = patterns[i].length - pointers[i];
                        }
                        non = false;
                        break;
                    }
                ...
                if (non) {
                    fna = true;
                    if (maxNA < patterns[i].length - pointers[i]) {
                        maxNA = patterns[i].length - pointers[i];
                    }
                }
            }

...
public class Solution {
	...
	static void solve() throws Exception {
		...
		for (int test = 1; test <= tests; test++) {
			...
			List<Integer> cameTaskBegs = new ArrayList<>();
			List<Integer> cameTaskEnds = new ArrayList<>();
			...
			for (int j = 1; j <= tasks; j++) {
				...
				if (isPossible) {
					boolean isAssignToCameron = isAssignedToPerson(cameTaskBegs, cameTaskEnds, lowerTime, upperTime);
					if (isAssignToCameron) {
						answer.append('C');
						continue;
					}
					boolean isAssignToJamie = isAssignedToPerson(jamieTaskBegs, jamieTaskEnds, lowerTime, upperTime);
					if (isAssignToJamie) {
						answer.append('J');
						continue;
					}
	...
	private static boolean isAssignedToPerson(List<Integer> personTaskBegs, List<Integer> personTaskEnds, int lowerTime,
			int upperTime) {
	...
	public static boolean areOverlapping(int a1, int b1, int a2, int b2) {

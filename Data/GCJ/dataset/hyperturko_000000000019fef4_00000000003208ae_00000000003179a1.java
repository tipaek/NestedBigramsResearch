...
public class Solution {
  ...
  private static String solve(final IO io) {
    ...
    for (int i = 1; i < 10; i++) {
      final Map<Character, Integer> sum = new HashMap<>();
      for (int p = 1; p < io.U; p *= 10) {
        for (int j = i*p; j < (i+1)*p; j++) {
          if (freq.containsKey(j)) {
            freq.get(j).forEach((key, val) -> sum.merge(key, val, Integer::sum));
          }
        }
      }

      final char c = getMax(sum, found);
      if (c != 0) {
        res[i] = c;
        found.add(c);
      }

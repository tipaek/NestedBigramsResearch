...
public class Solution{
	static class cases{
     ...
     public boolean Available( Activity act) {
    	 ...
    	 Integer EndTime=act.getStart();
    	 boolean bool=true;
    	 ArrayList<Activity>array=getArray();
    	 for(int i=0; i<array.size();++i) {
    		 Activity tmp=array.get(i);
    		 if((StartTime<tmp.getStart() && EndTime>tmp.getStart())) {
    			 bool=false;
    		 }
    		 if((StartTime<tmp.getStart() && EndTime <tmp.getEnd())) {
    			 bool=false;
    		 }
    		 if((StartTime>tmp.getEnd() && EndTime <tmp.getEnd())) {
    			 bool=false;
    		 }
    		 if((StartTime>tmp.getEnd() && EndTime >tmp.getEnd())) {
    			 bool=false;
	   ...
	   Scanner in= new Scanner(System.in);
	    ...
	    Activity [] arr=new Activity [Integer.parseInt(N)];
			for(int j=0; j<Integer.parseInt(N);j++) {
			 ...
			 Activity curr=new Activity(j+1,Integer.parseInt(tmp[0]),Integer.parseInt(tmp[1]) );
			 arr[j]=curr;
			}
			for(int j=0; j<arr.length; j++) {
				Activity current=arr[j];

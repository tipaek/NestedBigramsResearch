...
public class Solution {
    ...
    public void start(){
        ...
        for (int i = 1; i <= numberOfTestCases; i++) {
            ...
            activityList.sort(new StartTimeComparator());
            Activity mostRecenActivity = activityList.get(0);
            mostRecenActivity.owner = "J";
            jamieList.add(mostRecenActivity);
            mostRecenActivity = findMostRecentActivity(activityList,mostRecenActivity.end);
            while(mostRecenActivity != null){
                mostRecenActivity.owner = "J";
                jamieList.add(mostRecenActivity);
                mostRecenActivity = findMostRecentActivity(activityList,mostRecenActivity.end);
            }
            mostRecenActivity = findMostRecentActivity(activityList,0);
            while (mostRecenActivity != null){
                mostRecenActivity.owner = "C";
                cameronList.add(mostRecenActivity);
                mostRecenActivity = findMostRecentActivity(activityList,mostRecenActivity.end);
            }
            boolean owverNullExists = false;
            for(Activity activity : activityList){
                if(activity.owner == null){
                    System.out.println("Case #"+i+": "+ "IMPOSSIBLE");
                    owverNullExists = true;
                    break;
                }
            }
            if(owverNullExists){
    ...
    public Activity findMostRecentActivity(List<Activity> startTimeSortedList, int startTime){
        Integer minDif = Integer.MAX_VALUE;
        Activity minActivity = null;
        for(Activity activity : startTimeSortedList){
            if(activity.owner == null && activity.start>=startTime){
                int diff = activity.start-startTime;
                if(diff>-1 && diff<minDif){
                    minDif = diff;

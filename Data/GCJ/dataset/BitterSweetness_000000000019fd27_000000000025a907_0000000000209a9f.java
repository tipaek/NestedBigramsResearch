	...
	public static void main(String args[]) {
		...
		for (int i = 0; i < t; ++i) {
			...
			while (matcher.find()) {
				...
				if (res.charAt(0) == '0') {
					output += new String(new char[b]).replace("\0", ")") + res;
					//output += ")".repeat(b) + res;
				}
				...
				// No active bracket
					...
					if (Integer.parseInt(String.valueOf(res.charAt(0))) >= b) {
						output += new String(new char[Integer.parseInt(String.valueOf(res.charAt(0))) - b]).replace("\0", "(") + res;
			...
			output += new String(new char[b]).replace("\0", ")");

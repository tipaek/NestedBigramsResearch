 ...
 public class Solution
{
    public static void main(String args[])
    {
        ...
        while(test_case!=0)
        {
            ...
            if(N<=K && K<=N*N)
            {
               t++;
                int[][] squareMatrix=new int[N][N];
                 ...
                 squareMatrix=makeMatrix(N,K).clone();
            ...
            {
                System.out.println("Case #"+t+": "+"IMPOSSIBLE1212");
            }
    ...
    private static int[][] makeMatrix(int sizeMatrix,int trace) 
    {
        int R=0,C=0,sum=0;
        ...
        for(int i=0;i<sizeMatrix;i++)
                ...
                for(int row=0;row<sizeMatrix;row++)
                    {
                        sum+=newArray[row][row];
                    }
                if(R==0 && C==0 && sum==trace)
                {
                    return newArray;
                }
                else
                {
                    return  makeMatrix(sizeMatrix,trace);
                }
     }

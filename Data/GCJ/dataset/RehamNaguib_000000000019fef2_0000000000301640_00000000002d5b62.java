...
public class Solution {
    ...
    public static void solve(int x, int y,int currentX, int currentY, String ans,int step){
        ...
        if(!dp[currentX + 100][currentY + 100][step].equals("")){
            if(dp[currentX + 100][currentY + 100][step].length() <= ans.length()){
               return; 
            }
        }
        if(currentX == x && currentY == y){
            if(s.equals("") || ans.length() < s.length()){
                s = ans;
                dp[currentX + 100][currentY + 100][step] = ans;
                return;
            }
        }
        dp[currentX + 100][currentY + 100][step] = ans;

...
public class Solution {
    ...
    static String assignPrev(List<Activity> activities) {
    ...
    static String assign(List<Activity> activities) {
        List<Activity> sortedByStart = activities.stream()
            .sorted((a1, a2) -> a1.start - a2.start)
            .collect(Collectors.toList());
        ...
        for (Activity starting : sortedByStart) {
            if (currentC == null || currentC.end <= starting.start) {
                currentC = starting;
                starting.assignedTo = "C";
                continue;
            }
            
            if (currentJ == null || currentJ.end <= starting.start) {
                currentJ = starting;
                starting.assignedTo = "J";
                continue;
            } 

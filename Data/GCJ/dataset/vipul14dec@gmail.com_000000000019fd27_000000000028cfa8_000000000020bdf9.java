        ...
        for(int i=0; i< t;i++) {
            ...
            for(int k=0;k<n;k++) {
                if(schedule[k][0] >= cend) {
                    cend = schedule[k][1];
                    lkp.put(String.valueOf(schedule[k][0]) + String.valueOf(schedule[k][0]), "C");
                }
                else {
                    if(schedule[k][0] >= jend) {
                        jend = schedule[k][1];
                        lkp.put(String.valueOf(schedule[k][0]) + String.valueOf(schedule[k][0]), "J");
                    }
                    else {
                        result = "IMPOSSIBLE";
                        break;
            ...
            if(result == "IMPOSSIBLE")
                ...
                for(int k=0;k<n;k++) {
                    result += lkp.get(String.valueOf(original[k][0]) + String.valueOf(original[k][0]));
                }
                System.out.println("Case #" + (i+1) + ": " + result);
            }
        }

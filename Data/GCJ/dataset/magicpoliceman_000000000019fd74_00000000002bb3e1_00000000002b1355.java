...
public class Solution {
    ...
    private static int calculateInterest(int[][] floor) {
        ...
        for (int r = 0; r < floor.length; r++) {
            for (int c = 0; c < floor[0].length; c++) {
                ...
                if (dancer != -1) {
                    sum += dancer;
                    double average = calculateAverage(r, c, floor);
                    if (average > (double) dancer) {
                        toRemove.add(r + " " + c);
                    }
                }
            }
    ...
    private static double calculateAverage(int r, int c, int[][] floor) {
        ...
        return (double) sum / (double) dancers.size();
    }

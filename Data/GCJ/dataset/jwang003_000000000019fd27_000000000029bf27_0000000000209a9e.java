...
public class Solution {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < cases; i++) {
            if (size == 10) {
                ...
                //do the first ten
                for (int j = 1; j <= 10; j++) {
                    System.out.println((j%2==0)? (size - j/2) : j/2+1);
                ...
                while (count < size) {
                    if (count%10 == 0) {
                        int result = evaluate(checkers);
                        order = changes(order, result);
                        //count += 4;
                        //changeCount++;
                    } else {
                        System.out.println(((count)%2==0)? (size - count/2) : count/2+1);
    ...
    static int evaluate(int[] check) {
        ...
        if (newValues[0] == check[0] && newValues[2] != check[2]) {
            return 2;
        } else if (newValues[0] != check[0] && newValues[2] != check[2]) {
            return 1;
        } else if (newValues[0] != check[0] && newValues[2] == check[2]) {
            return 3;
        } else {
    ...
    static int[] changes(int[] order, int result) {
        switch (result) {
            case 1:
                ...
                break;
            case 2:
                for (int j = 0; j < order.length/2; j+=2) {
                    int temp = order[j];
                    order[j] = order[j+1];
                    order[j+1] = temp;
                }

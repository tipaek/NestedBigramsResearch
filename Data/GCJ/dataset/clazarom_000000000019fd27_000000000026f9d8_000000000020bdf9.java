...
public class Solution {
	...
	public String computeSchedule(String[] activities, int numActivities) {
		String schedule = "";
		Parent Cameron = new Parent(_CAMERON, activities.length);
		Parent Jamie = new Parent(_JAMIE, activities.length);
			...
			for (int[] timeActivity: timeActivities) {
				...
				if (timeActivity.length == 2) {
					if(Cameron.isAvailable(timeActivity)) {
						...
						if (assignResult == Parent._UNAVAILABLE && Jamie.isAvailable(timeActivity))
							assignResult = Jamie.assignActivity(timeActivity);
					}else 
						//Try the next
						if(Jamie.isAvailable(timeActivity))
							assignResult = Jamie.assignActivity(timeActivity);
				}

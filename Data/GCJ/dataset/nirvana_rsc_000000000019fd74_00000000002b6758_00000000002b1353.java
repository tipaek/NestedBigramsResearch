...
public class Solution {
    ...
    static class Path {
        ...
        Path(int r, int k, int d, List<List<Integer>> path) {
            this.r = r;
            this.k = k;
            this.d = d;
            this.path = path;
            ...
            while (!pq.isEmpty()) {
                ...
                for (int[] dir : DIRS) {
                    ...
                    if (0 <= nr && nr < 30 && 0 <= nk && nk < triangle.get(nr).size()) {
                        final int nd = curr.d + triangle.get(nr).get(nk);
                        if (nd > n || visited.add(nr + "," + nk + ',' + nd)) {
                            final List<List<Integer>> npath = new ArrayList<>(curr.path);
                            npath.add(Arrays.asList(nr + 1, nk + 1));
                            pq.offerLast(new Path(nr, nk, nd, npath));
                        }
                    }
                }
            }

...
public class Solution {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= t; ++i) {
            ...
            System.out.println("Case #" + i + ": " + calculateTime(i, people, in, totActivities));
        }
    }

    private static String calculateTime(int t, ArrayList<Person> people, Scanner scanner, int totActivities) {
        String order = "";
    ...
    private static String assignActivity(Person p, int start, int end) {
        if(p.getActivities().isEmpty()) {
            ...
            return p.getName();
                ...
                ArrayList<Integer> startTimes = new ArrayList<>(p.getActivities().keySet());
                ArrayList<Integer> endTimes = new ArrayList<>(p.getActivities().values());
                if ((start < Collections.min(startTimes) && end <= Collections.max(startTimes)) ||

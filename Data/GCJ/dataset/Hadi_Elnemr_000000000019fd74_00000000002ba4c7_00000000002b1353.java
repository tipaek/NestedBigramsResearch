...
public class Solution {
	...
	static String pascal(int i, int j, int sum, String s, boolean[]visited) {
		...
		for(int idx = 0 ; idx<visited.length;idx++)
			a[idx] = visited[idx];
		if (i == -Infinity || j == -Infinity || i <= 0 || j <= 0 || j > i || sum > n)
			return "";
		if (sum == n)
			return s;
		if (i != 1 && j != 1)
		...
		for (int k = 1; k < 5; k++) {
			x[k] = pascal(i + ro[k], j + co[k], sum + value(i, j), s,visited);
			if (x[k].length() != 0)
				return x[k];
		}
		x[0] = pascal(i + ro[0], j + co[0], sum + value(i, j), s,visited);
		x[5] = pascal(i + ro[5], j + co[5], sum + value(i, j), s,visited);
		if(x[0].length()!=0)
			return x[0];
		if(x[5].length()!=0)
			return x[5];

...
class Solution {
	...
	void solve(int TC) {
		...
		ans=null;
		dfs(0, 0, dx, dy, 1, "");
		pn("Case #"+TC+": " + (ans==null?"IMPOSSIBLE":ans));
		
	}
	static String ans;
	static int[][]dir= {{0,1},{-1,0},{1,0},{0,-1}};
	static String[]p={"N","W","E","S"};
	void dfs(long cx, long cy, long dx, long dy, long j, String path) {
		if(cx==dx&&cy==dy) {
			if(ans==null)ans = path;
			else {
		...
		for(int i=0;i<4;i++) {
			int[]d=dir[i];
			long cxd=cx+(long)Math.pow(2, j-1)*d[0];
			long cyd=cy+(long)Math.pow(2, j-1)*d[1];
			dfs(cxd, cyd, dx, dy, j+1, path+p[i]);
		}
	}

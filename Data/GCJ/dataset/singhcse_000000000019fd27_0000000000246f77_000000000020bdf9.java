    ...
    for(int t =1;t<=T;t++){
        ...
        for(Map.Entry<Integer,Integer> entry : map.entrySet()){
            ...
            if(jobMap.size() < 2){
                ...
                resultMap.put((entry.getKey()+""+entry.getValue()),whoDoJob);continue;
            }
            int min = Math.min(jobMap.get('J'),jobMap.get('C'));
            if(key >= jobMap.get('J') && jobMap.get('J') == min){
                jobMap.put('J',entry.getValue());
                resultMap.put((entry.getKey()+""+entry.getValue()),'J');
            }
            else if(key >= jobMap.get('C') && jobMap.get('C') == min){

...
public class Solution {
    ...
    private boolean solve(int[][] board, int x, int y, int trace, int curr){

        if(isSolved(board))
            return curr == trace;
        if(x < 0 || y < 0 || x >= board.length || y >= board[0].length || curr > trace)
            return false;

        if(board[x][y] != 0)
            return solve(board, x + (y + 1) / board.length, (y + 1)%board.length, trace, curr);
...
//                if(x == y)
//                    curr += board[x][y];
                if(solve(board, x + (y + 1) / board.length, (y + 1)%board.length, trace, curr+(x == y? board[x][y]:0)))
                    return true;

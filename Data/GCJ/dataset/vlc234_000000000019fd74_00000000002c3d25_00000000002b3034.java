...
//        solve(1, patterns.size(), patterns);
    ...
    private static void solve(int t, int n, List<String> patterns) {
        ...
        int patternsStartCaseIndex = 0;
        int patternsEndCaseIndex = 0;
        int index = -1;
        for (String pattern : patterns) {
            index++;
//            System.out.println("--- chenking pattern: " + pattern);
//            int asteriskIndex = pattern.indexOf("*");
            
            //Process start
            int startAsteriskIndex = pattern.indexOf("*");
            if (startAsteriskIndex > 0) {
                String patternStart = pattern.substring(0, startAsteriskIndex);
//                System.out.println("Checking start: " + patternStart);
            ...
            //Process end
            int endAsteriskIndex = pattern.lastIndexOf("*");
            if (endAsteriskIndex < pattern.length() - 1) {
                String patternEnd = pattern.substring(endAsteriskIndex + 1);
//                System.out.println("Checking end: " + patternEnd);
        ...
        String result = combine(patternsStart, patternsContain, patternsEnd, patternsStartCaseIndex, patternsEndCaseIndex);
    ...
    private static String combine(String patternsStart, String patternsContain, String patternsEnd, int patternsStartCaseIndex, int patternsEndCaseIndex) {
//        System.out.println("combine: " + patternsStart + ","  + patternsContain + "," + patternsEnd);
        ...
        if (!patternsEnd.isEmpty()) {
            if (patternsStartCaseIndex == patternsEndCaseIndex) {
                result = result + patternsEnd;
            } else {
                resultEndIndex = 0;

...
public class Solution {
	...
	List<Dance> tod = new LinkedList<Dance>();
	
	for(int i=0;i<r;++i){
	    for(int j=0;j<c;++j){
		tod.add(skill[i][j]);
		if(i>0)
		    skill[i][j].l=skill[i-1][j];
		if(j>0)
		    skill[i][j].t=skill[i][j-1];
		if(i<r-1)
		    skill[i][j].r=skill[i+1][j];
		if(j<c-1)
		    skill[i][j].b=skill[i][j+1];
	    }
	}
	
	while(!tod.isEmpty()){
	    interest+=currint;
	    List<Dance> dest = new LinkedList<Dance>();
	    List<Dance> up = new LinkedList<Dance>();    
	    for(Dance d: tod){
		//check if destroy
		d.upd=false;
		if(bad(d))dest.add(d);

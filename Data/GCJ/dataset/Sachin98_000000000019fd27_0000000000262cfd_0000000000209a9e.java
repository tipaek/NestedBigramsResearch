...
//	      Scanner sc = new Scanner(System.in);
	      ...
	      for( int testcase=1 ; testcase<=t ; ++testcase ) {
	    	  queryCount = 1;
	    	  int sameBit,sameBitLeftPosition=1,diffBitLeft,difBitLeftPosition=1;
	    	  int prevSameBit=0,prevDifLeftBit=0;
	    	  ...
	    	  while( (!isSameBitAvailable || !isDifBitAvailable) && pairNum<=(b/2) ) {
	    		  if( (queryCount%10)==1 ){
	    			  if( isSameBitAvailable) {
	    				  write(sameBitLeftPosition);
		    			  sameBit = sc.nextInt();
		    			  write(pairNum);
			    		  leftBit = sc.nextInt();
		    			  if( prevSameBit!=sameBit ) {
		    				  result = compliment(result);
		    			  }
		    			  prevSameBit = sameBit;
	    			  }else if ( isDifBitAvailable ) {
	    				  write(difBitLeftPosition);
		    			  diffBitLeft = sc.nextInt();
		    			  write(pairNum);
			    		  leftBit = sc.nextInt();
		    			  if( prevDifLeftBit!=diffBitLeft ) {
		    				  result = compliment(result);
		    			  }

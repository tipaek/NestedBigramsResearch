	...
	public static void main(final String[] args) {
		try (PrintWriter pw = new PrintWriter(System.out, false)) {
			...
			for (int ii = 0; ii < t; ii++) {
				...
				final Set<P> s = new HashSet<>();
				P p;
				final Map<P, Integer> m = new HashMap<>();
				long q = 0, tt = 0;
				for (int i = 0; i < r; i++) {
					for (int j = 0; j < c; j++) {
						s.add(p = new P(i, j));
				...
				for (;;) {
					final int l = s.size();
					final Set<P> ts = new HashSet<>();
					for (final P v : new HashSet<>(s)) {
						final int x = v.x;
						final int y = v.y;
						int ss = 0, cc = 0;
						for (int i = x + 1; i < r; i++) {
							if (s.contains(p = new P(i, y))) {
								ss += m.get(p);
								cc++;
								break;
							}

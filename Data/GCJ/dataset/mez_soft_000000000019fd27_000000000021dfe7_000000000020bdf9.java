...
public class Solution {
	...
	private static String solve() {
		...
		String t = "";
		for(int i = 0; i < n; i++) {
			int k = end.get(i) - begin.get(i);
			if(c < 2 & !cam.contains(end.get(i)) & !cam.contains(begin.get(i))) {
				t+="C";
				c++;
				if(k > 2)cam.addAll(num.subList(begin.get(i)+1, end.get(i)-1));
				else if(k == 2) cam.add(begin.get(i)+1);
				j = 0;
			}else if(j < 2 & !jam.contains(end.get(i)) & !jam.contains(begin.get(i))) {
				t+="J";
				j++;
				if(k > 2)jam.addAll(num.subList(begin.get(i)+1, end.get(i)-1));
				else if(k == 2) jam.add(begin.get(i)+1);
				c = 0;
			}else {
				t = "IMPOSSIBLE";
				break;

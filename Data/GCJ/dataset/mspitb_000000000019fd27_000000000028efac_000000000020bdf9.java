...
public class Solution {
   ...
   private static List<Shift> merge(List<Shift> shiftsList) {
      MyList modified = new MyList();
      shiftsList.forEach(modified::addShift);
      return modified;
   }
   ...
   private static class MyList extends ArrayList<Shift> {

      public boolean addShift(Shift shift) {
         if (this.isEmpty()) {
            return super.add(shift);
         }
         for (int i = 0; i < this.size(); i++) {
            if (this.get(i).getStart() == shift.getEnd() && !this.get(i).isOverlap(shift)) {
               int newStart = shift.getStart();
               int newEnd = this.get(i).getEnd();
               Shift newShift = new Shift(newStart, newEnd);
               newShift.setSubShifts(Arrays.asList(this.get(i), shift));
               this.set(i, newShift);
               return true;
            } else if (this.get(i).getEnd() == shift.getStart() && !this.get(i).isOverlap(shift)) {
               int newStart = this.get(i).getStart();

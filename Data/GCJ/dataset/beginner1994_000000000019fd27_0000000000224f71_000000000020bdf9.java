...
class Solution {
	...
	static class Wrapper{
		...
		public boolean equals(Object o){
			Wrapper w = (Wrapper)o;
			if(this.start==w.start && this.end==w.end)return true;
			return false;
		}
		
		@Override
		public int hashCode() {
			return start*33333+end*22733;
		}
	}
	...
	public static void main(String[] args) throws Exception {
		...
		outer: for(int _i=1;_i<=_t;_i++)
		{
			...
			for(int i=0;i<n;i++){
				...
				wList.add(new Wrapper(result[i][0],result[i][1]));
			}
			...
			for(int i=0;i<n;i++){
				Wrapper w = new Wrapper(result[i][0],result[i][1]);
				sb.append(map.get(w));
			}

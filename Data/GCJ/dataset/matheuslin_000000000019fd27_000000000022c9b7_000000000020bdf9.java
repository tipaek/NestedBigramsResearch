...
public class Solution {
	...
	public static String schedule(List<Activity> activities) {
		...
		int verifyRemaining = entriesCount;
		int i = 0;
		
		while(verifyRemaining > 0) {
			Activity activity = colliders.get(i);
			if(shouldSwap(activity, collisionMap.get(activity))) {
				activity.swap();
				verifyRemaining = entriesCount;
			}
			verifyRemaining--;
			i = (i + 1) % entriesCount;
		}
	...
	private static boolean shouldSwap(Activity key, Set<Activity> collided) {
		Set<Character> collidedAssign = new HashSet<>();
		for(Activity a : collided) {
			collidedAssign.add(a.assigned);
		}
		
		if(collidedAssign.size() > 1) {
			return true;
		} else if(collidedAssign.contains(key.assigned)) {
			return true;
		}

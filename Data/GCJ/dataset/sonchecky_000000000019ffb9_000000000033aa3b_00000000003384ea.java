...
public class Solution {
  ...
  public static void main(String[] args) {
    ...
    for (int counter = 1; counter <= t; counter ++) {
      ...
      long i = (long)(Math.sqrt(2*Math.abs(difference))); // number of customers served so far
      if (difference >= 0)
        left -= i*(i+1)/2;
      else
        right -= i*(i+1)/2;
      ...
      long kBig = (long)((-i + Math.sqrt(i*i + 4*big))/(2.0));
      long kSmall = (long)((-(i+1) + Math.sqrt((i+1)*(i+1) + 4*small))/(2.0));

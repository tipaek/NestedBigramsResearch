...
public class Solution {
    ...
    private static String parenting(List<Interval> originals) {
        Map<Interval, Character> actors = new HashMap<>();

        List<Interval> intervals = new ArrayList<>(originals);
        ...
        for (Interval interval : intervals) {
            ...
            actors.put(interval, activity.actor);
        }


        StringBuilder schedule = new StringBuilder(intervals.size());

        for (Interval interval : originals) {
            schedule.append(actors.get(interval));
        }

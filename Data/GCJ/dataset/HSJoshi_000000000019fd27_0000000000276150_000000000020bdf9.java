...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int qItr = 0; qItr < t; qItr++) {
        	...
        	System.out.println("Case #" + (qItr+1) + ": " + result);
        }
	...
	private static char getChar(Timestamp st, Timestamp et) {
		...
		for(int i=0;i<startTimes.size();i++) {
			if(st.equals(startTimes.get(i)) || et.equals(endTimes.get(i))
					|| (st.before(startTimes.get(i)) && et.after(startTimes.get(i)))
					|| (st.before(endTimes.get(i)) && et.after(endTimes.get(i)))

...
import codejam.y2020.r1b.b.Solution.Gary.Answer;
...
public class Solution {
    ...
    private boolean solve(Gary gary, int a, int b) {
        if (a == b && a == 1_000_000_000 - 5) {
            for (int x = -5; x <= 5; x++) {
                for (int y = -5; y <= 5; y++) {
                    Answer answer = gary.say(x, y);
                    if (answer == Answer.CENTER) {
                        return true;
                    }
                }
            }
    ...
    static class RemoteGary implements Gary {
        ...
        @Override
        public Answer say(long x, long y) {
            queries++;
            out.println(x + " " + y);
            out.flush();
            return Answer.valueOf(in.next());
        }
    ...
    static class LocalGary implements Gary {
        ...
        public Answer say(long x, long y) {
            queries++;
            if (x == xc && y == yc) return Answer.CENTER;
            if ((x - xc) * (x - xc) + (y - yc) * (y - yc) <= radius) return Answer.HIT;
            return Answer.MISS;
        }

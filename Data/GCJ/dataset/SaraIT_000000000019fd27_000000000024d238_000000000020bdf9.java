			...
			for (int j = 0; j < N; j++) {
				...
				if (!timesMap.containsKey(Integer.parseInt(times[0]))) {
					...
					if (secondMap.containsKey(Integer.parseInt(times[0]))) {
						...
						duplicatKey = Integer.parseInt(times[0]);
						duplicatValue = Integer.parseInt(times[1]);
						secondMap.put(Integer.parseInt(times[0]), Integer.parseInt(times[1]));
					}
				}
			}
			// Duplicate Key
			if (timesMap.containsValue(duplicatKey)) {
				if (timesMap.containsValue(duplicatKey+1)) isImpo = true; 
				else timesMap.put(duplicatKey+1, duplicatValue);
			}

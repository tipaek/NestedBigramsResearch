...
public class Solution {
    ...
    public static void solveTasks(int quizId,int actNum){
        ...
        int j=0;
        while(j<jamieActArray.size()){
            println("jamie " + j + ": [" +jamieActArray.get(j).startTime + "," + jamieActArray.get(j).endTime + "]");
            j++;
        }
        j=0;
        while(j<cameronActArray.size()){
            println("cameron " + j + ": [" +cameronActArray.get(j).startTime + "," + cameronActArray.get(j).endTime + "]");
            j++;
        }
*/

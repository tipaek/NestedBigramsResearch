...
public class Solution {
    ...
    private static String getSolution(BigInteger x, BigInteger y, String s, int move) {
        if(x.compareTo(X)==0 && y.compareTo(Y)==0) {
            ...
            did = true;
        } else if(x.compareTo(X.multiply(X.subtract(new BigInteger("1")))) > 0 || y.compareTo(Y.multiply(Y.subtract(new BigInteger("1")))) > 0) {
            // nothingness
        } else if(x.compareTo(X.multiply(X.subtract(new BigInteger("1"))).negate()) < 0 || y.compareTo(Y.multiply(Y.subtract(new BigInteger("1"))).negate()) < 0) {
            // nothingness
        } else {

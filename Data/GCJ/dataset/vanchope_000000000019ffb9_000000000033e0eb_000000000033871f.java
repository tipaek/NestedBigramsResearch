...
public class Solution implements Runnable {
    ...
    public static class Solver extends  SolverAbstract{
        ...
        public void readInput() throws IOException {
            ...
            C = scanner.nextInt(); // 100
            D = scanner.nextInt(); // 1000
            A = new Data[C];
            for (int i = 0; i < A.length; i++) {
                Data dat = new Data();
                dat.index = i;
                dat.time_reached = MAX;
                A[i] = dat;
            }
            A[0].time_reached = 0;
            A[0].x = 0;
        ...
        int[] time_reached;
        int[] latency; // must be  >0

        @Override
        protected void solve() throws IOException {
            time_reached = new int[C];
            latency = new int[C];
            Arrays.fill(latency, 1000000);
            int time = 0;

            PriorityQueue<Data> q = new PriorityQueue<>(Comparator.comparingInt(o -> -o.x));
            for(Data d: A){
                q.add(d);
            }
            Data first = q.poll(); // remove first
            while(!q.isEmpty()){
                time++;
                int x_curr = q.peek().x;
                List<Data> to_finalize = new LinkedList<>();
                while (!q.isEmpty() && (q.peek().x == x_curr)){
                    Data dat = q.poll();
                    int v = dat.index;
                    for (int u = 0; u < C; u++) if (u!= dat.index) {
                        if (A[u].finalized>=1 && G[u][v]!=MAX){
                            dat.time_reached = time;
                            latency[G[u][v]] = time - A[u].time_reached;
                            to_finalize.add(A[u]);
                        }
                    }
                }
                // finalize
                for(Data d : to_finalize){
                    d.finalized = 1;
                }
            }
            for (int i = 0; i < D; i++) {

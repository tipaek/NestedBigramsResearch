...
public class Solution {
    ...
    private static Integer[][] mirrorLeft(Integer[][] array) {
        int n = array.length;
        Integer[][] newArray = new Integer[n][n];
        for(int i=0;i<n;i++){
            for(int j=0;j<n;j++){
                newArray[i][n-j-1]= array[i][j];
            }
        }
        return newArray;
    }

    private static Integer[][] mirrorTop(Integer[][] array) {
        int n = array.length;
        Integer[][] newArray = new Integer[n][n];
        for(int i=0;i<n;i++){
            for(int j=0;j<n;j++){
                newArray[n-i-1][j]= array[i][j];
            }
        }
        return newArray;
    }

...
public class Solution {
	  public static void main(String[] args) {
		    ...
		    for (int xx = 1; xx <= t; xx++) {
		    	...
		    	int max=0;
		    	Set<Double> maxSlope = new HashSet<>();
		    	for (int i=0; i<n; i++)
		    		for (int j=i+1; j<n; j++) {
		    			if (count.containsKey(slope[i][j])) {
		    				...
		    				if (c>max) {
		    					max=c;
		    					maxSlope = new HashSet<>();
		    					maxSlope.add(slope[i][j]);
		    				} else if (c==max) maxSlope.add(slope[i][j]);
		    	...
		    	for (double mx: maxSlope) {
		    	    ...
		    	    if (points.size()>pts.size()) pts=points;
		    	}

		    	
		    	int result = pts.size()+2;

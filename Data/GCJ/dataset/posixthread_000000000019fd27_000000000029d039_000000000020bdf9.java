...
public class Solution {
    ...
    public static void main(String[] args) {
        ...
        for (int k = 0; k < numCases; k++) {
            ...
            if(!figureOutSchedule2(acts)) {
                System.out.println("IMPOSSIBLE");
            } else {
                for (int j = 0; j < acts.length; j++) {
                    System.out.print(finalSchedule[j]);
                }
                System.out.println();
            }
        }
    ...
    public static boolean figureOutSchedule2(Activity[] acts) {
        int endC = 0;
        int endJ = 0;
        
        for (int i = 0; i < acts.length; i++) {
            
            if (endC > acts[i].start && endJ > acts[i].start) {
                return false;
            }

            else if (endC <= acts[i].start) {
                endC = acts[i].end;
                finalSchedule[acts[i].order] = 'C';
            } else if (endJ <= acts[i].start) {
                endJ = acts[i].end;
                finalSchedule[acts[i].order] = 'J';
            }
        }
    ...
    public static boolean figureOutSchedule(Activity[] acts, int actNum, int endC,
            int endJ) {

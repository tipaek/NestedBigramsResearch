...
public class Solution {
    ...
    private static void print(List<AbstractMap.SimpleEntry<Integer,AbstractMap.SimpleEntry<Integer,Integer>>> s){
        ...
        List<Integer>[] overlap = new List[s.size()];
        for(int i=0;i<overlap.length;i++){
            overlap[i]=new ArrayList<>();
        }
        for(int i=0;i<s.size()-1;i++){
            for(int j=i+1;j<s.size();j++){
                if(isOverlapping(s.get(i).getValue(),s.get(j).getValue())){
                    overlap[i].add(j);
                    overlap[j].add(i);
                }
            }
        ...
        for(int i=0;i<overlap.length;i++){
            for(Integer o:overlap[i]){
                for(Integer o2:overlap[o])
                if(overlap[i].contains(o2)) {

...
public class Solution {
	...
	class Variant implements Comparable<Variant>{
		...
		public boolean possible() {
			long all = x|xminus|y|yminus;
			long big = big(all);
			return ((x&xminus)==0)&&((y&yminus)==0)&&(((x|xminus)&(y|yminus)) ==0)&&((big-all)==1);
		}
	...
	private void calculate() {
		...
		v.add(new Variant(x, y, 0, 0, xcnt+ycnt));
		bigx = big(x);
		while(bigx<=0x80000000L) {
			bigxcnt = 1 + bitCount(bigx - x);
			v.add(new Variant(bigx, y, bigx - x, 0, bigxcnt+ycnt));
			bigx*=2;
		}
		bigy = big(y);
		while(bigy<=0x80000000L) {
			bigycnt = 1 + bitCount(bigy - y);
			v.add(new Variant(x, bigy, 0, bigy-y, xcnt+bigycnt));
			bigy*=2;
		}
		...
		while(bigx<=0x80000000L) {
			...
			while(bigy<=0x80000000L) {
				bigycnt = 1 + bitCount(bigy - y);
				v.add(new Variant(bigx, bigy, bigx-x, bigy-y, bigxcnt+bigycnt));
				bigy*=2;
			}
			bigx*=2;
		}

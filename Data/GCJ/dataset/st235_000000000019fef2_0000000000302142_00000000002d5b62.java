...
public class Solution {
    ...
    private static Map<Point, Map<Double, String>> lookup = new HashMap<>();
    ...
    private static String solve(Set<Point> visited, String path, int iter, long currentX, long currentY, int x, int y) {
        ...
        if (lookup.containsKey(current) && lookup.get(current).containsKey(jump)) {
            return lookup.get(current).get(jump);
        }
        ...
        if (!lookup.containsKey(current)) {
            lookup.put(current, new HashMap<>());
        }

        lookup.get(current).put(jump, result);

...
public class Solution {
    ...
    private static String getMoves(int targetX, int targetY) {
        ...
        if (targetX == 1) {
            return getNoDetourPath(targetX, targetY, 1, 0, 1,  "E");
        } else if (targetX == -1) {
            return getNoDetourPath(targetX, targetY, -1, 0, 1,  "W");
        } else if (targetY == 1) {
            return getNoDetourPath(targetX, targetY, 0, 1, 1,  "N");
        } else if (targetY == -1) {
            return getNoDetourPath(targetX, targetY, 0, -1, 1,  "E");
        }

        if ((differenceX == 1 && Math.abs(targetX) < Math.abs(targetY))
                || (differenceY == 1 && Math.abs(targetY) < Math.abs(targetX))) {
            return getNoDetourPath(targetX, targetY, 0, 0, 0,  "");
        } else {
            String prefix = "";
            int currentX = 0;
            int currentY = 0;
            ...
            return getNoDetourPath(targetX, targetY, currentX, currentY, 1, prefix);
        }
    }

    private static String getNoDetourPath(int targetX, int targetY, long currentX, long currentY, int power, String prefix) {

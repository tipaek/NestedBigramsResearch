...
public class Solution
{
  ...
  {
    Deque<Integer> traceNumbers = new ArrayDeque<>(matrixRange / 2);
    Deque<Integer> nonTraceNumbers = new ArrayDeque<>(matrixRange / 2);
  ...
  private static String getMatrixStringOdd(int matrixRange, int trace)
  {
    ...
    for (int i = 1; i < matrixRange; i++)
    {
      matrix[0][i] = goUp ? getDown(matrix[0][i - 1], matrixRange) : getUp(matrix[0][i - 1], matrixRange);
    }
    for (int i = 0; i < matrixRange; i++)
    {
      for (int j = 0; j < matrixRange; j++)
      {
        ...
        int el = goUp ? getUp(matrix[i - 1][j], matrixRange) : getDown(matrix[i - 1][j], matrixRange);
        matrix[i][j] = el;
        sb.append(el).append(" ");
      }
      if (i != matrixRange - 1)

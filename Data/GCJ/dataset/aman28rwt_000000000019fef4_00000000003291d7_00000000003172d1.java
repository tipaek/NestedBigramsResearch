...
public class Solution implements Runnable {
	...
	void solve() {
		...
		for (int tt = 1; tt <= T; tt++) {
			...
			HashMap<Long, Long> cut = new HashMap<>();
			HashMap<Long, ArrayList<Long>> lol = new HashMap<>();
			for (long a : arr) {
			...
			for (int i = 0; i < arr.length; i++) {
				for (long d = 1; d * d <= arr[i]; d++) {
					if (arr[i] % d == 0) {
						long x = arr[i] / d, y = arr[i] / x;

						if (!lol.containsKey(x)) {
							lol.put(x, new ArrayList<>());
						}
						cut.merge(x, y, Long::sum);
						lol.get(x).add(y - 1);
						
						if(x != y) {
							if (!lol.containsKey(y)) {
								lol.put(y, new ArrayList<>());
							}
							cut.merge(y, x, Long::sum);
							lol.get(y).add(x - 1);
						}
					}
				}
			...
			for(long a : lol.keySet()) {
				Collections.sort(lol.get(a));
				if(cut.get(a) >= k) {
					int add = k - cnt.getOrDefault(a, 0), x = 0;
					for(long z : lol.get(a)) {
						if(z + 1 < add) {
							x += z;
							add -= z + 1;
						} else {

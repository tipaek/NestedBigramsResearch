...
class Solution {
    ...
    public static void main(String[] args) {
        ...
        for(int q = 1; q <= t; q++){
            ...
            for(int i = 0; i < n; i++){
                ...
                int e = sc.nextInt();
                Pair p = new Pair(s, e);
                // Here we can change the priority of names...
                if (p.canInsert(c)){
                    c.add(p);
                    output.append("C");
                    continue;
    ...
    static class Pair{
        ...
        public boolean canInsert(List<Pair> pairs){
            for(Pair pair: pairs){
                if(this.intersect(this, pair)){
                    return false;
                }
            }
            return true;
        }
        private boolean intersect(Pair p1, Pair p2){
            if(p1.end < p2.start){
                return false;
            }
            if(p1.start > p2.end){
            ...
            if(p1.end == p2.start || p1.start == p2.end){
                if(p1.start == p2.start || p2.start == p2.end){
                    return true;
                }
                return false;
            }

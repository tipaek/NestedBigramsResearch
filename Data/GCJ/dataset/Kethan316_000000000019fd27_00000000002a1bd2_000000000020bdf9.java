    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 1; i <= x; i++) {
            ...
            for (int j = 0; j < y; j++) {
                ...
                if (activities.get(i).get(1) < endC && activities.get(i).get(1) < endJ) {
                    notAvail = true;
                    break;
                } else {
                    if (activities.get(i).get(1) >= endC) {
                        ans[activities.get(i).get(0)] = 'C';
                        endC = activities.get(i).get(2);
                    } else {
                        ans[activities.get(i).get(0)] = 'J';
                        endJ = activities.get(i).get(2);
                    }
                }
            }
            if (notAvail)
                ...
                System.out.println("Case #" + j + ": " + new String(ans));
        }
    }
}
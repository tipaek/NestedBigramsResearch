...
public class Solution {
    ...
    public static void main(String[] args)throws IOException {
        ...
        while(t --> 0) {
            int n = f.nextInt();
            int st[] = new int[n]; int en[] = new int[n];
            Pair arr[] = new Pair[n];
            for(int i = 0; i < n; ++i) {
                st[i] = f.nextInt(); en[i] = f.nextInt();
                arr[i] = new Pair(st[i] , en[i] , i);
            }
            Compare obj = new Compare();
            arr = obj.compare(arr , n);
            //for(int i = 0; i < n; ++i) {pn(arr[i].x + " " + arr[i].y + " " + arr[i].ind);}
            int end = -1;
            char ans[] = new char[n];
            Arrays.fill(ans , 'U');
            HashSet<Integer> h = new HashSet<Integer>();
            for(int i = 0; i < n; ++i) {
                if(arr[i].x >= end) {end = arr[i].y; ans[arr[i].ind] = 'C'; h.add(i);}
            }
            ...
            for(int i = 0; i < n; ++i) {
                if(!h.contains(i)) {
                    if(arr[i].x >= end) {end = arr[i].y; ans[arr[i].ind] = 'J'; h.add(i);}
                }
            }
            p("Case " + "#" + (x++) + ": ");
            if(h.size() != n) {p("IMPOSSIBLE");}
            else for(int i = 0; i < n; ++i) {p(ans[i]);}
            pn("");
        }
        pw.flush();
        pw.close();  

...
public class Solution {
    ...
    private static void solve(int caseNumber, List<Activity> activities) {
        ...
        String result;
        if (isPossible) {
            result = activities.stream().sorted(Comparator.comparingInt(a -> a.i)).map(Activity::getOwner).collect(Collectors.joining());
        } else {
            result = "IMPOSSIBLE";
        }
    ...
    private static boolean dfs(List<Activity> activities, int i, Activity C, Activity J) {
        if (i == activities.size()) return true;
        ...
        if (!current.isOverlap(C) && !current.isOverlap(J)) {
            current.owner = "J";
            boolean jResult = dfs(activities, i + 1, C, current);
            if (!jResult) {
                current.owner = "C";

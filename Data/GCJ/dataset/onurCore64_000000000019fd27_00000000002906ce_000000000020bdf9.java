...
class Solution{
  public static String isBipartite(Node[] node) throws NullPointerException{
    ...
    for(int i = 0;i < first.size() - 1;i++){
      for(int y = 0;y < node[first.get(i)].getNode().length;y++){
      if(node[first.get(i)].getNode()[y] == first.get(i + 1).intValue()){
        return "IMPOSSIBLE";
      }
      }}
      for(int j = 0;j < sec.size() - 1;j++){
        for(int y = 0;y < node[sec.get(j)].getNode().length;y++){
      if(node[sec.get(j)].getNode()[y] == sec.get(j + 1).intValue()){
        return "IMPOSSIBLE";
      }
        }}
    return s;
    ...
    for(int i = 0;i < n;i++){
      ...
      if(!re.equals("IMPOSSIBLE")){
        ...
        for(int t = 0;t < map.size();t++){
          rep = rep.substring(0,t) + re.substring(map.get(t),map.get(t) + 1) + rep.substring(t + 1,rep.length()); 
        }
      }else{
        rep = "IMPOSSIBLE";
      }

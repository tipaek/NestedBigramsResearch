...
public class Solution {
    ...
    private static String solve(int[] s){
        ...
        if(s[start] > 0){
           ...
           replacePos(s, sb, start, end);
        } else {
            int end = findEndOfLongestNegative(s, start);
            sb.replace(end+1, s.length, solve(Arrays.copyOfRange(s, end+1, s.length)));
            replaceNeg(s, sb, start, end);
        }

        return sb.toString();
    }

    private static void replaceNeg(int[] s, StringBuilder sb, int start, int end) {
        String sub = subNegative(s, start, end);
        sb.replace(start, end+1, sub);
    }

    private static void replacePos(int[] s, StringBuilder sb, int start, int end) {

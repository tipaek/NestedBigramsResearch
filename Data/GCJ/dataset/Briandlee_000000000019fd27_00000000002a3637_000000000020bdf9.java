  ...
  public static void calculations(){
    ...
    for (int i = 0; i < T; i++){
      ...
      for (int jjjj = 0; jjjj < N; jjjj++){
        if (cameronList.get(startingTimes[jjjj]) == 0 && cameronList.get(endingTimes[jjjj] - 1) == 0){
          for (int k = startingTimes[jjjj]; k < endingTimes[jjjj]; k++){
            cameronList.set(k, 1);
          }
          parentsList.add("C");
          parentsString = parentsString + "C";
        } else if (jamieList.get(startingTimes[jjjj]) == 0 && jamieList.get(startingTimes[jjjj] - 1) == 0){

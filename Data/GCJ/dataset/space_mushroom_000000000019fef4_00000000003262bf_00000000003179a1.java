...
public class Solution{
    ...
    private static void goForExclusion(Map<Character, List<Integer>> map) {
        ...
        for(char c: map.keySet()){
            ...
            for(int el: list){
                if(!numberCount.containsKey(el))
                    numberCount.put(el, 0);
                numberCount.put(el, numberCount.get(el)+1);
            }
        }
            ...
            if(numberCount.get(num) == 1){
                for(char c: map.keySet()){
                    ...
                    if(list.contains(num)){
                        map.put(c, new ArrayList<>(Arrays.asList(num)));
                        break;
                    }
                }
            }
    ...
    private static void updateMapV2(Couple couple, Map<Character, List<Integer>> map) {
        ...
        if(couple.str.length() > 1 && map.get(firstChar).size() > 1)
            canBe(couple, map);
    ...
    private static void canBe(Couple couple, Map<Character, List<Integer>> map) {
        String numAsString = String.valueOf(couple.num);
        int firstCharNum = (int) numAsString.charAt(0) - '0';
        int secondCharNum = (int) numAsString.charAt(1) - '0';

        int max = 0;
        for(int num: map.get(couple.str.charAt(0))){
            if(num > max)
                max = num;
        }

        int min = 10;
        for(int num: map.get(couple.str.charAt(1))){
            if(num < min)
                min = num;
        }

        int num = Integer.parseInt("" + firstCharNum + secondCharNum);
        int strNum = Integer.parseInt(""+max+""+min);

        if(strNum > num)
            map.get(couple.str.charAt(0)).remove(Integer.valueOf(max));
        
        //if(strNum == num && couple.str.length() > 2)
          //  canBe(new Couple(Long.parseLong(numAsString.substring(1)), couple.str.substring(1)), map);

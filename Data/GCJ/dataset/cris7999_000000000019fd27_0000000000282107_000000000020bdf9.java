	...
	public static void main(String[] args) {
		...
		for(int test=0;test<numTest;++test) {
			...
			for(int i=0 ; i <numActivities; ++i) {
				...
				if(iCanDoThatActivitie(cameronList, start, finish) && !impossibleCombination) {
					...
					if(iCanDoThatActivitie(jaimeList, start, finish) && !impossibleCombination) {
						List<Integer> newTask = new ArrayList<>();
						newTask.add(start);
						newTask.add(finish);
	...
	public static boolean iCanDoThatActivitie(List<List<Integer>> myList, int start, int finish) {	
		...
		for(int i = 0; i<myList.size();++i) {
			...
			if(finish >myList.get(i).get(0) && finish < myList.get(i).get(1))
				return false;
			if(start < myList.get(i).get(0) && finish > myList.get(i).get(1))
				return false;
		}

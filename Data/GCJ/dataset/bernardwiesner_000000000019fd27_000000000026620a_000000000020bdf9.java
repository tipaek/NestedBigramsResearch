...
//                    break;
    ...
    static class Activity {
        ...
        boolean inRange(Activity activity) {
            int c = activity.start;
            int d = activity.end;
            return (c >= start && c < end) || (d > start && d < end);
        }

        boolean notInRange(Activity activity) {
            int c = activity.start;
            int d = activity.end;
            return (c < start && c >= end) || (d <= start && d > end);
        }
    ...
    static class Parent {
        ...
        boolean canTakeActivity(Activity activity){
            for (Activity a : activities) {
                if (a.inRange(activity) || activity.inRange(a)) return false;
            }

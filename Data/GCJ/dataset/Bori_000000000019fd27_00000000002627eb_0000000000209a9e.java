    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        if(noBits==10) {
            ...
            for(int t=1; t<=noTests && !exit; t++){
                //find bits 1-5 and 16-20
                findBitsInInterval(bits, 1, 5, input);
                findBitsInInterval(bits, 96, 100, input); //10 queries

                mirroredBit = findMirroredBit(bits, 1, 5, 96, 100);
                unMirroredBit = findUNMirroredBit(bits, 1, 5, 96, 100);

                performNeededOperations(bits, 100, 5, mirroredBit, unMirroredBit, input); //query nr 11, 12


                //in all the other rounds I perform 2 checks to find ut what operations were performed on the array
                // //and 8 others to find out 8 new bits --> 11 such rounds

                for(int i=6; i<=49; i+=4){
                    findBitsInInterval(bits, i, i+3, input);
                    findBitsInInterval(bits, 100-i-2, 100-i+1, input); //10 queries

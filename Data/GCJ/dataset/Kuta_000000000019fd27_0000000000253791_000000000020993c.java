...
public class Solution {
        ...
        for(int i =0;i<grid.length;i++)
        {
            ...
            for(int j=0;j<grid[i].length;j++)
            {
                ...
                if(i==0)
                {
                    ...
                    HashSet<Integer> keyCount=new HashSet<>();
                    dfs(keyCount,grid,colDoubles,i,j);
                    if(keyCount.size() > 0)
                    {
                        ++repeat_in_col;
                    }
    ...
    public static void dfs(HashSet<Integer> keyCount,int [][] grid,HashSet<Integer> doubles,int i,int j)
    {
     if(i>=grid.length)
     {
         return ;
     ...
     dfs(keyCount,grid,doubles,i+1,j);
     return ;
    }

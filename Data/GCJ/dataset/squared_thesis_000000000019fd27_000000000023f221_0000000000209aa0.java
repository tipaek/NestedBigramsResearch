...
public class Solution {
            ...
            for(int j=0; j<size; j++){
                ...
                for(int k=0; k<size; k++){
                    for(int l=0; l<size; l++){
                        Integer[][] matrix1 = getMatrixWithSwappedCols(matrix, k, l);
                        if(calculateTrace(matrix1) == sum) {
                            return matrix1;
                        }
                    }
                }
    ...
    private static Integer[][] getLatinSquaredWithSwappedRows(int size, int row, int swapRowWith){
        Integer[][] square = generateLatinSquare(size);
        for(int i=0; i<size; i++){
            int temp = square[row][i];
            square[row][i] = square[swapRowWith][i];
            square[swapRowWith][i] = temp;
        }
        return square;
    }

    private static Integer[][] getMatrixWithSwappedCols(Integer[][] matrix, int col, int swapColWith){
        Integer[][] square = new Integer[matrix.length][matrix.length];
        for(int i=0; i<matrix.length; i++){
        ...
        for(int i=0; i<square.length; i++){
            int temp = square[i][col];
            square[i][col] = square[i][swapColWith];
            square[i][swapColWith] = temp;
        }
        return square;
    }

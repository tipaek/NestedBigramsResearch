                ...
                for(int k = j+1; k < activities.size(); k ++){
                    if(activities.get(j)[0] < activities.get(k)[1] && activities.get(k)[0] < activities.get(j)[1]){
                        for(int m = k+1; m < activities.size(); m ++){
                            if(activities.get(m)[0] < activities.get(j)[1] && activities.get(j)[0] < activities.get(m)[0]){
                                if(activities.get(m)[0] < activities.get(k)[1] && activities.get(k)[0] < activities.get(m)[0]) impossible = true;
                            }
                        }
                    }
                }

...
class Solution {
	...
	private static boolean isBusy(Set<StartEnd> taskSet, StartEnd startEnd) {
		for (Iterator<StartEnd> iterator = taskSet.iterator(); iterator.hasNext();) {
			...
			if((startEnd.getStart()>camStartEnd.getStart() && startEnd.getStart()<camStartEnd.getEnd())
					|| (startEnd.getEnd()>camStartEnd.getStart() && startEnd.getEnd()<camStartEnd.getEnd())) {
				return true;
			}
			
			if((startEnd.getStart() == camStartEnd.getStart()) && (startEnd.getEnd() == camStartEnd.getEnd())) {
				return true;
			}
		}
		return false;

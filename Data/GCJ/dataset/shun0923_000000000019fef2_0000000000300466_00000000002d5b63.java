...
public void solve() {
	...
	long a2 = nl();
	if(a == a2) {
		int size = (int)abs((long)1e9 - a);
		end: for(int query = 0; query < q; query ++) {
			boolean b[][] = new boolean[size * 2 + 1][size * 2 + 1];
			fill(b, true);
			int sign[] = {1, -1, 1, -1};
			for(int i = 0; i < 4; i ++) {
				long ok = size * sign[i];
				long ng = (- size - 1) * sign[i];
				while (abs(ok - ng) > 1) {
					long mid = (ok + ng) / 2;
					long p[] = {mid - a * sign[i], 0};
					if(i >= 2) swap(p, 0, 1);
					int tmp = ask(p);
...
void cal(int size, boolean[][] b, long[] p, boolean isOK, long a) {
	for(int i = - size; i <= size; i ++) {
		for(int j = - size; j <= size; j ++) {
			long dist = pow(i - p[0], 2) + pow(j - p[1], 2);
			b[i + size][j + size] &= (dist > pow(a, 2)) ^ isOK;
		}
	}
}
...
int ask(long... p) {
	prtln(p); flush();
	String str = ns();
	if(str.equals("CENTER")) return -1;
	if(str.equals("HIT")) return 0;
	if(str.equals("MISS")) return 1;
	return -2;
}

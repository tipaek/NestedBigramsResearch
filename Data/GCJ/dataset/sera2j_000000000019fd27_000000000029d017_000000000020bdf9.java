...
public class Solution {
	...
	Scanner console = new Scanner(System.in);
		...
		for(int i = 0; i < numberOfMatrixes; i++) {
			...
			List<List<Integer>> jj = new ArrayList<>();
			String result = "";
			...
			for(int j = 0; j < numberOfPeriods; j++) {
				...
				if(!impossible) {
					...
					Iterator<List<Integer>> cIterator = cc.iterator();
					...
					while (cIterator.hasNext()) {
						...
						if(Math.max(entry.get(0), oldEntry1.get(0)) < Math.min(entry.get(1), oldEntry1.get(1))){							
							cCanTakeIt = false;
							break;
						}else if((entry.get(0)==oldEntry1.get(0)) && (entry.get(1)==oldEntry1.get(1))) {
							cCanTakeIt = false;
							break;
						}
					}
					...
					if(!cCanTakeIt) {
						Iterator<List<Integer>> jIterator = jj.iterator();
						...
						while (jIterator.hasNext()) {
							...
							if(Math.max(entry.get(0), oldEntry2.get(0)) < Math.min(entry.get(1), oldEntry2.get(1))){	
								...
								impossible = true;
								break;
							}else if((entry.get(0)==oldEntry2.get(0)) && (entry.get(1)==oldEntry2.get(1))) {
								jCanTakeIt = false;
								impossible = true;
								break;
							}

...
class Scheduler {
	...
	public String organize() {
		//#1 find and assign earliest activity to Jamie
		int early = this.findEarliest();
		Jamie.add(activities[early]);
		output[early]='J';
		...
		for(int a=0; a<activities.length;a++) {
			if(activities[a].getStart()==-1) {
				...
				if(isFree(activities[a], Jamie)){
					Jamie.add(activities[a]);
					output[a]='J';
					activities[a]= new Tuple(-1,-1);
				}else if(isFree(activities[a],Cam)) {
		...
		String solution = new String(output);
		return solution;
	}

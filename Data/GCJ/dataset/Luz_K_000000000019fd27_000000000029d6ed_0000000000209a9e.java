...
public class Solution {
  public static void main(String[] args) {
    ...
    if(numberOfBits == 10) {
	    for (int i = 1; i <= t; ++i) {
	    	Integer[] bits10 = new Integer[numberOfBits];
	    	for(int x = 0; x < numberOfBits; x++) {
	    		System.out.println(x + 1);
	    		bits10[x] = Integer.parseInt(in.next());
	    	}
	    	StringBuilder output10 = new StringBuilder();
	    	for(int z = 0; z < bits10.length; z++) {
	    		output10.append(bits10[z]);
	    	}
  ...
  public static Integer[] changeBits(int start, Integer[] bits,boolean diffList[], boolean changeDiff) {
	  ...
	  if(changeDiff == false) {
		  for(int i = 0; i < 5;i++) {
			  if(bits[start+i] == 0 && !diffList[start + i]) {
				  bits[start+i] = 1;
			  }else if(!diffList[start + i]) {
				  bits[start+i] = 0;
			  }
			  if(bits[bits.length - 1 -start - i] == 0 && !diffList[start + i]) {

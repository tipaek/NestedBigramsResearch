...
public class Solution {
	static class Activity implements Comparable<Activity>{
		...
		public int compareTo(Activity a) {
			if(this.start < a.start && this.end <= a.start)
				return -1;
			if(this.start >= a.start && a.end <= this.start)
				return 1;
			return 0;
		}
	}
    public static void main(String [] args){
        ...
        for(int k = 0; k< test_cases; k++){
        	...
        	for(int i = 0; i< activities.length; i++) {
        		...
        		if(JHandling != null && JHandling.compareTo(acc) != 0)
        			JHandling = null;
        		if(CHandling != null && CHandling.compareTo(acc) != 0)
        			CHandling = null;

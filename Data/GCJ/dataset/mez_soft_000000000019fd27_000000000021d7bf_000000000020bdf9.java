	...
	private static String solve() {
		...
		for(int i = 0; i < n; i++) {
			if(c < 2 & !cam.contains(end.get(i)) & !cam.contains(begin.get(i))) {
				t+="C";
				c++;
				if(end.get(i) - begin.get(i) > 2)cam.addAll(num.subList(begin.get(i)+1, end.get(i)-1));
				else if(end.get(i) - begin.get(i) == 2) cam.add(begin.get(i)+1);
				j = 0;
			}else if(j < 2 & !jam.contains(end.get(i)) & !jam.contains(begin.get(i))) {
				t+="J";
				j++;
				if(end.get(i) - begin.get(i) > 2)jam.addAll(num.subList(begin.get(i)+1, end.get(i)-1));
				else if(end.get(i) - begin.get(i) == 2) jam.add(begin.get(i)+1);
				c = 0;
			}else {
				t = "IMPOSSIBLE";
				break;

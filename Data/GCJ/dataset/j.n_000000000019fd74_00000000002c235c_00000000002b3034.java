...
public class Solution {
    public static void solve(int t, List<String> patterns, int mxlen){
        ...
        //System.out.println(patterns.get(0).substring(patterns.get(0).lastIndexOf('*')));
        //check start
        String x = patterns.get(0);
        String mxpref = "";
        int idx = x.indexOf('*');

        mxpref = idx != -1 ? x.substring(0,idx) : mxpref;
        for (int i = 1; i < patterns.size(); i++){
            x = patterns.get(i);
            String suf = "";
            idx = x.indexOf('*');
            suf = idx != -1 ? x.substring(0, idx) : suf;

            String tmp = suf;
            if (mxpref.length() < suf.length()){
                tmp = mxpref;
                mxpref = suf;
            }
            if (!mxpref.startsWith(tmp)){
        ...
        resString = mxpref;
        //System.out.println("mxpref" + mxpref);
       ...
       /* while (resString.length() < mxlen && resString != "IMPOSSIBLE") {
            for (int i = 0; i < patterns.size(); i++) {
                ...
                if (patterns.get(i).charAt(position[i]) == '*') {
                    stars++;
                    position[i] = patterns.get(i).length();
                    continue;
                }
            ...
            //System.out.println(resString + " " + stars);
        }*/
        x = patterns.get(0);
        String mxsuf = "";
        idx = x.lastIndexOf('*');
        ...
        //System.out.println("mxsuf" + mxsuf);
        if (resString == "IMPOSSIBLE"){
        ...
        for (int i = 0; i < patterns.size(); i++){
            ...
            suf = idx != -1 ? x.substring(idx + 1) : suf;
            idx = suf.lastIndexOf('*');
            suf = (idx != -1) ? suf.substring(0, idx + 1) : suf;
            suf = suf.replaceAll("\\*","");
            resString += suf;
        }
        resString += mxsuf;

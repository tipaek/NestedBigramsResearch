...
public class Solution {
    ...
    void solve(int TC) throws Exception{
        int N = ni(), M = ni();
        int[] cntBefore = new int[N], time = new int[N];
        Arrays.fill(cntBefore, -1);
        cntBefore[0] = 0;
        for(int i = 1; i< N; i++)cntBefore[i] = Math.abs(ni());
        int[] from = new int[M], to = new int[M];
        for(int i = 0; i < M; i++){
            from[i] = ni()-1;
            to[i] = ni()-1;
        }
        int[][][] g = makeS(N, from, to, M, true);
        int[] ans = new int[M];
        Arrays.fill(ans, (int)1e6-1);
        Arrays.fill(time, (int)1e6);
        time[0] = 0;
        int D = 1;
        int[][] list = new int[N-1][];
        for(int i = 1; i< N; i++)list[i-1] = new int[]{i, cntBefore[i]};
        Arrays.sort(list, (int[] i1, int[] i2) -> Integer.compare(i1[1], i2[1]));
        for(int i = 0, j = 0; i< N-1; i = j){
            while(j< N-1 && list[j][1] == list[i][1])j++;
            for(int k = i; k< j; k++){
                int u = list[k][0];
                for(int[] v:g[u]){
                    if(time[v[0]] < D){

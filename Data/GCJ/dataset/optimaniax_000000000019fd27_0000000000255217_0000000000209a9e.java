...
public class Solution {
    ...
    static Set<Integer> negativeVal = new HashSet<>();
    ...
    static void flip () {
        for (int i=1;i<present.size();i++) {
            int oldval = present.get(i);
            if(oldval==0) present.set(i, 1);
            else if (oldval==1) present.set(i,0);
        }
    }
    ...
    static void setNegativeVal() {
        negativeVal.clear();
        for (int i=1;i<present.size();i++) if (present.get(i)<0) negativeVal.add(i);
    }
    ...
    public static void main(String[] args) {
        ...
        for (int z=1;z<=t;z++) {
            ...
            while (negativeVal.size() > 0) {

                if (qcoun%10==1) {
                    if (diferent.size()==0) {
                        int bitval = getibit(1); qcoun++;
                        for (int j=1;j<=b;j++) present.set(j, bitval);
                    } else if (same.size()==0) {
                ...
                int nextInd = negativeVal.iterator().next();
                negativeVal.remove(nextInd);
                present.set(nextInd, getibit(nextInd)); qcoun++;
                //out.println(present.subList(1, b+1)); out.flush();
            }

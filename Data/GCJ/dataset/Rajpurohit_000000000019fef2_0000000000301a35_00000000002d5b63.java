...
public class Solution {
    ...
    private static void process(int a, int b) throws IOException {
        ...
        Point ref = null;
        outer:
        for (int x = MIN + side; x <= MAX - side; x += side) {
            for (int y = MIN + side; y <= MAX - side; y += side) {
                ref = new Point(x, y);
                status = validate(ref);
        ...
        if (status != Status.CENTER) throw new RuntimeException("incorrect center");
    }
    ...
    private static Point findDwonBoundry(Point ref, int side) throws IOException {
        if (validate(new Point(ref.x, ref.y - 1)) == Status.MISS) return null;
        int a = ref.y - 2 * side, b = ref.y, m = (a + b) / 2;
        int x = ref.x;
        a = Math.max(MIN, a);
        while (b > a) {
    ...
    private static Point findUpBoundry(Point ref, int side) throws IOException {
        if (validate(new Point(ref.x, ref.y + 1)) == Status.MISS) return null;
        int a = ref.y, b = a + 2 * side, m = (a + b) / 2;
        int x = ref.x;
        a = Math.min(MAX, a);
        while (b > a) {
            Status status = validate(new Point(x, m));
    ...
    private static Point findRightBoundry(Point ref, int side) throws IOException {
        int a = ref.x, b = ref.x + 2 * side, m = (a + b) / 2;
        b = Math.min(MAX, b);
        int y = ref.y;
        while (b > a) {
    ...
    private static Point findLeftBoundry(Point ref, int side) throws IOException {
        int a = ref.x - 2 * side, b = ref.x, m = (a + b) / 2;
        a = Math.max(a, MIN);
        int y = ref.y;
        while (b > a) {
            Status status = validate(new Point(m, y));

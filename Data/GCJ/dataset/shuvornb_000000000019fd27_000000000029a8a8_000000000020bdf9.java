...
public class Solution {
    ...
    private static String getCandidate(Task currentTask, List<Task> taskJ, List<Task> taskC) {
        boolean flag = true;
        for(Task task: taskC) {
            if ((currentTask.startTime >= task.startTime && currentTask.startTime < task.endTime) ||
                    (currentTask.endTime <= task.endTime && currentTask.endTime > task.startTime) ||
                    (currentTask.startTime >= task.startTime && currentTask.endTime <= task.endTime)) {
                flag = false;
                break;
            }
        }

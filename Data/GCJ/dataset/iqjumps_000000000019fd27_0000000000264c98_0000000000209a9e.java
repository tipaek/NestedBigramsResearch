...
public class Solution {
    ...
    static int determine(int same, int different, int[] arr) {
        if (different == -1) {
            return determineSame(same, arr);
        }
        if (same == -1) {
            return determineDifferent(different, arr);
        }
        int num1 = query(same);
    ...
    static int determineSame(int same, int[] arr) {
        int num1 = query(same);
        int num2 = query(arr.length - 1 - same);
    ...
    static int determineDifferent(int different, int[] arr) {
        int num3 = query(different);
        int num4 = query(arr.length - 1 - different);

        if (arr[different] == num3 && arr[arr.length-1-different] == num4) {
            return 0; // nothing
        }
        if (arr[different] != num3 && arr[arr.length-1-different] != num4) {
            return 1; // reversed

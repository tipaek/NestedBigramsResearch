...
{	
   ...
   public static void solve(int t)
   {
       ...
       for(int i=0;i<n;i++)
	    {
	   	 if(i==0) temp.append(ch[i]);
	   	 else
	   		 {
	   			 if(ch[i]==ch[i-1]) temp.append(ch[i]);
	   			 else
	   				 {
	   					 if(ch[i-1]>ch[i])
	   						 {
	   							int close=ch[i-1]-ch[i];
	   							for(int k=0;k<close;k++) temp.append(")");
	   							rem-=close;
	   							temp.append(ch[i]);
	   						 }
	   					 else
	   						 {
	   							   int op=ch[i]-ch[i-1];
		   							for(int k=0;k<op;k++) temp.append("(");
		   							rem+=op;
		   							temp.append(ch[i]); 
	   						 }
	   				 }
       ...
       for(int k=0;k<rem;k++) temp.append(")");
       f.append(temp);

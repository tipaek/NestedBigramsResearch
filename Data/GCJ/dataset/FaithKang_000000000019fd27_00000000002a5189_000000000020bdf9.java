...
//			System.out.println("works "+ work1[0]+" "+work2[0]);
			return false;
			
		}
		else {
//			System.out.println("check 1" + work1[0]+" "+work2[0]);
			return true;
		}
	}
	public boolean possible(int[][] S, LinkedList<Integer>[] overlap_work, int idx) {
		if(overlap_work[idx].size()>=2) {
			for(int i=0;i<overlap_work[idx].size();i++) {
				
				for(int j = i+1;j<overlap_work[idx].size();j++) {
					if(overlap(S, overlap_work, overlap_work[idx].get(i), overlap_work[idx].get(j))) {
						return false;
					}
				}
			}
...
//						System.out.println("overlap isze"+overlap_work[i].size());
						answer = "IMPOSSIBLE";
						impossible = true;
					}
					else {
//						System.out.println("let's code");
						...
						for(int j = 0;j<overlap_work[i].size();j++) {
							l.add(overlap_work[i].get(j));
//							System.out.println("?? "+ overlap_work[i].get(j));
						}
						more.add(l);
					}

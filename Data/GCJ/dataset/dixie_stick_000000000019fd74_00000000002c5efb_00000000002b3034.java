...
public class Solution {
	...
	public static void main(String[] args) {
		...
		for(int t = 0; t < T; t++) {
			...
			String[][] allSplits = new String[pats.length][1];
			for(int i = 0; i < pats.length; i++) {
				allSplits[i] = getSplit(pats[i]);
				if(allSplits[i].length == 1 && allSplits[i][0].length() < min) {
					index = i;
					min = allSplits[i][0].length();
				}
			}
			...
			for(String pat : pats) {
				...
				/*
				if(rest.length == 1) {
					cs = new char[rest[0].length()];
					for(int i = 0; i < rest[0].length(); i++) {
						cs[i] = rest[0].charAt(i);
					}
					System.out.println("Cs NOW " + Arrays.toString(cs));
				}
				*/

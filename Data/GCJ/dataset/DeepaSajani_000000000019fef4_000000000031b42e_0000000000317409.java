...
public class Solution {
    ...
    private static int solution(int x, int y, String sequence) {
        ...
        for (int i = 0; i < positions.size(); i++) {
            boolean canReach = canReach(0, 0, positions.get(i).get(0), positions.get(i).get(1), i + 1);
            if (canReach) return i + 1;
        }
        return -1;
    }

    private static boolean canReach(int x, int y, int cx, int cy, int time) {
        if (x == cx && y == cy && time == 0) return true;
        if (time < 0) return false;
        boolean north = canReach(x + 1, y, cx, cy, time - 1);
        boolean south = north || canReach(x - 1, y, cx, cy, time - 1);
        boolean east = south || canReach(x, y + 1, cx, cy, time - 1);
        boolean west = east || canReach(x, y - 1, cx, cy, time - 1);
        boolean stand = west || canReach(x, y, cx, cy, time - 1);
        return stand;
    }
}

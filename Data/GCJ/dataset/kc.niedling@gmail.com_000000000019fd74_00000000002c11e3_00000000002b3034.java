...
public class Solution {
    ...
    public static void main(String[] args) {
        ...
        for(int c = 1; c <= tc; c++) {
            ...
            for (int i = 0; i < n; i++) {
                String s = in.next();
                if (s.startsWith("*")) {
                    startsWithPatterns.add(s);
                } else if (s.endsWith("*")) {
                    endsWithPatterns.add(s);
                } else {
                    String[] array = s.split("\\*");
                    startsWithPatterns.add(array[0]);
    ...
    private static String solve(ArrayList<String> startsWithPatterns, ArrayList<String> endsWithPatterns) {
        ...
        for (int i = 0; i < n-1; i++) {
            String smallerString = startsWithPatterns.get(i).substring(1, startsWithPatterns.get(i).length());
            for (int j = i+1; j < n; j++) {
        ...
        //Check to make sure they are all substrings of the longer strings
        for (int i = 0; i < n-1; i++) {
            String smallerString = endsWithPatterns.get(i).substring(0, endsWithPatterns.get(i).length()-1);
            for (int j = i+1; j < n; j++) {

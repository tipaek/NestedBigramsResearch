...
public class Solution {
    ...
    public static void main(String...args) {
        ...
        for (int t=1; t<=testCases; t++) {
            ...
            int[][] matrix = null;
            List<Integer[]> diags = calculateDiags(n, k);
            if (diags != null && !diags.isEmpty()) {
                for (Integer[] diag: diags) {
                    matrix = new int[n][n];
                    reset(matrix, diag);
                    matrix = solve(matrix);
                    if (matrix != null)
    ...
    private static List<Integer[]> calculateDiags(int length, int diagSum) {
        if (diagSum < length || diagSum > length*length)
            return null;

        List<Integer[]> diags = new LinkedList<>();

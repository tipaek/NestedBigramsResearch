...
//			  char x[][]=new char[n][];
			  ...
			  boolean flag=true;
			  ans=smax.substring(0,max)+smin.substring(min+1);
//			  pt.println(ans);
			  for(String x:str) {
				  int p=x.indexOf('*');
//				  System.out.println(x);
				  String l=x.substring(0,p);
				  String r=x.substring(p+1);
//				  pt.println(cleft(l,ans));
//				  pt.println(cright(r,ans));
				  if(cleft(l,ans)&&cright(r,ans)) {
					  continue;
				  }
				  else
					  flag=false;
	...
	static boolean cleft(String a, String b) {
	...
	static boolean cright(String a, String b) {
		int l=Math.min(a.length(), b.length());
		int p=Math.max(a.length(), b.length())-l;

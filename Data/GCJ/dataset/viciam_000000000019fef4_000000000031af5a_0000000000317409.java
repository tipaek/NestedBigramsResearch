...
public class Solution {
	void solveCase(int caseNr) {
		...
		int iterations = solve(y,x,seq.trim(),0);
		
		
		if (iterations <0){
			System.out.println("Case #"+caseNr+": IMPOSSIBLE");
		} else 
			System.out.println("Case #"+caseNr+": "+iterations);
		
	}
	int solve(int x, int y, String seq, int iters) {
		if (x==0 && y==0)
				return iters;
		if (seq.length()==0)
			return -1;
		...
		int minIters = (1+Math.abs(x)+ Math.abs(y))/2;
		if (minIters > seq.length())
			return -1;
		int xADD =x, yADD=y;
		...
		if (Math.abs(xADD) > Math.abs(yADD)){
			if (xADD > 0) {
				return solve(nextX-1, nextY, seq.substring(1), iters+1);
			} else {
				return solve(nextX+1, nextY, seq.substring(1), iters+1);
			}
		} else {
			if (yADD > 0) {
				return solve(nextX, nextY-1, seq.substring(1), iters+1);
			} else {
				return solve(nextX, nextY+1, seq.substring(1), iters+1);
			}			
		}

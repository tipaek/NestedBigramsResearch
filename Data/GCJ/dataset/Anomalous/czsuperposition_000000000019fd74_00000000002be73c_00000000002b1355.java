...
public class Solution {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < T; i++) {
            ...
            for (int r = 0; r < R; r++) {
                for (int c = 0; c < C; c++) {
                    if (r > 0) grid[r][c].up = grid[r - 1][c];
                    if (r < R - 1) grid[r][c].down = grid[r + 1][c];
                    if (c > 0) grid[r][c].left = grid[r][c - 1];
                    if (c < C - 1) grid[r][c].right = grid[r][c + 1];
                }
            }

            int interest = 0;
            while (true) {
                int sum = 0, toEliminate = 0;

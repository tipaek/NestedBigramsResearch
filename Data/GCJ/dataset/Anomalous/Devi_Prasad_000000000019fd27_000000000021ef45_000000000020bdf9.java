    ...
    private static int solve(Activity[] activities) {
        ...
        for (FreeTime ft : freeTimes) {
            if (ft.parentingBefore == ft.parentingAfter) {
                ...
                int partnerTime = Math.min(parentingTimes[ft.parentingBefore], ft.length);
                parentingTimes[ft.parentingBefore] -= partnerTime;
                parentingTimes[ft.parentingBefore ^ 1] -= (ft.length - partnerTime);
            }
        }

...
public final class Solution {
    ...
    public static void main(final String[] args) {
        try (PrintWriter pw = new PrintWriter(System.out, false)) {
            ...
            for (int ii = 0; ii < t; ii++) {
                ...
                final P[] a = new P[n];
                for (int i = 0; i < n; i++) {
                    a[i] = new P(sc.nextInt(), sc.nextInt(), i);
                }
                Arrays.sort(a, (x, y) -> x.x == y.x ? x.y - y.y : x.x - y.x);
                final StringBuilder sb = new StringBuilder();
                P c = new P(-1, -1, -1), j = new P(-1, -1, -1);
                sb.append('C');
                c = a[0];
                for (int i = 1; i < n; i++) {
                    P p = a[i];
    ...
    private static final class P {
        ...
        public P(final int x, final int y, final int z) {
            this.x = x;
            this.y = y;
            this.z = z;
        }

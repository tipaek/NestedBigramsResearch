...
public class Solution {
    ...
    private void checkPossibleTrace(int[][] mat, int curIdx, int nCount, int kLeft,
        ...
        for (int val = nCount; val >= 1 && val <= kLeft; val--) {
    ...
    private void checkPossibleLatMat(int[][] mat, int curRow, int curCol, int nCount,
                                     Set<Integer>[] rowSetArr, Set<Integer>[] colSetArr) {
    ...
    private int[][] cloneMatrix(int[][] mat, int nCount) {
        int[][] clone = new int[nCount + 1][nCount + 1];
        for (int i = 1; i <= nCount; i++) {

...
public class Solution extends PrintWriter {
    ...
    void solve(int n, int m, Node[] nodes) {
        ...
        for (Node node : nodes) {
            if (node.time >= 0) times.add(node);
        }
        times.sort(Comparator.comparingInt(node -> node.time));
    ...
    public static boolean nextPermutation(int[] permutation) {
        ...
        while (a >= 0 && permutation[a] >= permutation[a + 1]) a--;
        if (a == -1) return false;
        int b = n - 1;
        while (permutation[b] <= permutation[a]) b--;
        swap(permutation, a, b);
        for (int i = a + 1, j = n - 1; i < j; i++, j--) swap(permutation, i, j);
        return true;
    }

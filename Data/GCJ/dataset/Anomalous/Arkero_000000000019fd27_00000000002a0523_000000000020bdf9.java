...
public class Solution {
    ...
    private static boolean isAvailable(HashMap<Integer, Integer> schedule, int start, int end) {
        for (Map.Entry<Integer, Integer> entry : schedule.entrySet()) {
            int scheduledStart = entry.getKey();
            int scheduledEnd = entry.getValue();
            if ((end > scheduledStart && end <= scheduledEnd) || (start >= scheduledStart && start < scheduledEnd)) {
                return false;
            }
        }

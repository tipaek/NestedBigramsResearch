...
public class Solution {
    public static void main(String[] args) {
        ...
        for (int testcase = 1; testcase <= t; testcase++) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (canAssignTask(cameronTasks, currentTask)) {
                    cameronTasks.add(currentTask);
                    schedule.append('C');
                } else if (canAssignTask(jamieTasks, currentTask)) {
    ...
    private static boolean canAssignTask(Set<Interval> taskSet, Interval newTask) {
        for (Interval task : taskSet) {

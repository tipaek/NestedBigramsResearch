...
public class Solution {
    ...
    static String findPath(Point target) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                for (int j = 0; j < 4; j++) {
                    int nextX = current.x + deltasX[j] * (1 << step);
                    int nextY = current.y + deltasY[j] * (1 << step);
    ...
    static boolean isBeyondTarget(Point target, int step) {
        int totalDistance = Math.abs(target.x) + Math.abs(target.y);
        return totalDistance < (1 << step);
    }
}
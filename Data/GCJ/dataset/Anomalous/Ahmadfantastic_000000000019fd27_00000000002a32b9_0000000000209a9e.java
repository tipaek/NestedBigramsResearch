...
class Solution {
    ...
    public static void main(String[] args) {
        ...
        for (int t = 1; t <= T; t++) {
            ...
            for (int q = 1; q <= 150; q++) {
                ...
                if (q % 10 == 1 && q != 1) {
                    isChecking = true;
                    checkPos = (knownCount - sameCount - 1) > 0 ? sameCount - 1 : (knownCount - diffCount - 1 > 0 ? diffCount - 1 : 0);
                ...
                if (isChecking) {
                    ...
                    if (knownCount <= checkPos) {
                        if (array[checkPos] == array[B - checkPos - 1] && isSame) sameCount++;
                        else isSame = false;

                        if (array[checkPos] != array[B - checkPos - 1] && isDiff) diffCount++;
                        else isDiff = false;
                    }
                    ...
                    if (q % 2 == 0) {
                        ...
                        int isCount = 0;
                        if (isI) isI = isI(array[checkPos], array[B - checkPos - 1], arrayNew[checkPos], arrayNew[B - checkPos - 1]);
                        if (isI) isCount++;
                        if (isS) isS = isS(array[checkPos], array[B - checkPos - 1], arrayNew[checkPos], arrayNew[B - checkPos - 1]);
                        if (isS) isCount++;
                        if (isIS) isIS = isIS(array[checkPos], array[B - checkPos - 1], arrayNew[checkPos], arrayNew[B - checkPos - 1]);
                        if (isIS) isCount++;
                        if (isN) isN = isN(array[checkPos], array[B - checkPos - 1], arrayNew[checkPos], arrayNew[B - checkPos - 1]);
                        if (isN) isCount++;
                    ...
                    if (q % 2 == 0) {
                        if (array[knownCount] == array[B - knownCount - 1] && isSame) sameCount++;
                        else isSame = false;

                        if (array[knownCount] != array[B - knownCount - 1] && isDiff) diffCount++;
                        else isDiff = false;
    ...
    private static void print(short[] array) {
        ...
        for (short s : array) value.append(s);

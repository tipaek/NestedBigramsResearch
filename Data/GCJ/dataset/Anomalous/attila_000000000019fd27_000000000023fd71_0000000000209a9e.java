...
public class Solution {
    ...
    public static void main(String[] args) {
        ...
        for (int testCase = 0; testCase < testCases; testCase++) {
            ...
            while (true) {
                ...
                boolean diffResponse = false;
                boolean sameResponse = false;

                if (firstDiffIndex != -1) {
                    diffResponse = queryBit(firstDiffIndex);
                } else {
                    queryBit(firstSameIndex);
                }

                if (firstSameIndex != -1) {
                    sameResponse = queryBit(firstSameIndex);
                } else {
                    queryBit(firstDiffIndex);
                }

                if (firstDiffIndex != -1 && firstSameIndex == -1) {
                    if (diffResponse != bits[firstDiffIndex]) {
                        complementBits(bits, nextQuery);
                    }
                }

                if (firstDiffIndex == -1 && firstSameIndex != -1) {
                    if (sameResponse != bits[firstSameIndex]) {
                        complementBits(bits, nextQuery);
                    }
                }
            ...
            System.out.println(buildResult(bits));
    ...
    private static String buildResult(boolean[] bits) {
    ...
    private static int findIndex(boolean[] bits, int max, boolean findDifference) {
        for (int i = 1; i < max; i++) {
            int oppositeIndex = bits.length - i;
            if ((bits[i] != bits[oppositeIndex] && findDifference) || 
                (bits[i] == bits[oppositeIndex] && !findDifference)) {
                return i;
            }
        }
        return -1;

    ...
    private static String solve(Scanner input) {
        ...
        if (single && leftMost) {
            ...
            for (int strIdx = 0; strIdx < n; strIdx++) {
                ...
                leftStrs[strIdx] = strs[strIdx].charAt(0) == '*' ? "" : split[0];
                rightStrs[strIdx] = strs[strIdx].charAt(strs[strIdx].length() - 1) == '*' ? "" : reverse(split[1]);
            ...
            for (int strIdx = 0; strIdx < n; strIdx++) {
                if (!leftStrs[leftLongest].startsWith(leftStrs[strIdx]) ||
                    !rightStrs[rightLongest].startsWith(rightStrs[strIdx])) {
                    return "*";
                }
            }

...
class Solution {
    ...
    public static String check(String[] strings, int n) {
        String pattern = strings[n - 1].substring(1);
        for (int i = n - 2; i >= 0; i--) {
            String currentSubstring = strings[i].substring(1);
            if (!pattern.endsWith(currentSubstring)) {
                return "*";
            }
        }
        return pattern;
    ...
    public static void main(String[] args) {
        ...
        while (t-- > 0) {
            ...
            System.out.println("Case #" + caseNumber + ": " + check(strings, n));
            caseNumber++;
        }
    }
}
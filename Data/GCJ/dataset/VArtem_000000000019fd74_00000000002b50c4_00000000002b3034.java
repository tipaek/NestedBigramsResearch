...
public class Solution {
    ...
    static class PatternMatching {
        public void solve(int testNumber, FastScanner in, PrintWriter out) {
            ...
            for (int i = 0; i < n; i++) {
                str[i] = new ArrayList<>();
                StringTokenizer st = new StringTokenizer(pats[i], "*");
                while (st.hasMoreElements()) {
                    str[i].add(st.nextToken());
                }
            ...
            out.print(maxPrefix.substring(1));
            for (int i = 0; i < n; i++) {
                for (int j = 1; j < str[i].size() - 1; j++) {
                    out.print(str[i].get(j));
                }
            }
            out.println(maxSuffix.substring(0, maxSuffix.length() - 1));
        }
